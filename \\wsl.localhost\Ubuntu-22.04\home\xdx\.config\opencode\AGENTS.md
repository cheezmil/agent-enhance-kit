...
- `pty_spawn` - 创建交互式终端会话（后台持续运行）
- `pty_write` - 向会话发送输入（命令、特殊键）
- `pty_read` - 读取会话输出（支持偏移、正则过滤）
- `pty_list` - 查看所有活跃会话
- `pty_kill` - 终止并清理会话
...
- **回车必须用 `\r`**，不是 `\n`。`\n` 只会换行不执行命令
- Ctrl+C 中断用 `\x03`，可终止卡住的命令
- Ctrl+D (EOF) 用 `\x04`
- Ctrl+Z (挂起) 用 `\x1a`
- Tab 键用 `\t`
- 方向键用转义序列：`\x1b[A` = 上，`\x1b[B` = 下，`\x1b[C` = 右，`\x1b[D` = 左

## 读取输出注意事项

- 输出包含大量 ANSI 转义序列（颜色、光标控制），肉眼看很乱是正常的
- 命令执行后可能需要再读一次才能拿到完整输出，不是发完就立刻有
- 缓冲区上限 50000 行，老的输出会被丢弃
- 用 `pattern` 参数可正则过滤，只看需要的行
- 用 `ignoreCase=true` 可忽略大小写匹配

## 会话管理注意事项

- 没设 `timeoutSeconds` 的话进程会一直跑，用完必须手动 `pty_kill`
- 设了 `notifyOnExit=true` 时，进程退出会收到系统通知，适合监控进程结束的场景
- 同时可以开多个会话，互不干扰
- `cleanup=true` 会删除会话和缓冲区，`cleanup=false` 只停止但保留（方便后续查看日志）
...
|---|---|---|
| 执行方式 | 同步等结果 | 异步后台运行 |
| 适用场景 | 一次性命令 | 长时间运行/需交互的进程 |
| 交互性 | 无 | 可随时发输入/读输出 |
| 例子 | git status, npm install | ssh, dev server, 需确认的程序 |

## 常见问题排查

1. **命令无反应**：检查是否用了 `\n` 而不是 `\r`
2. **输出为空**：命令可能还在执行，等一会再读或检查进程状态
3. **乱码**：ANSI 转义序列，不影响实际内容，可用正则提取关键信息
4. **会话卡死**：发送 `\x03` 中断，或直接 `pty_kill` 终止
