/**
 * Host half of aek-dsh. Three responsibilities:
 *
 * 1. Materialize a `wsl-<mode>` variant for every healthy roster preset
 *    under `<aekHome>/.agent-presets/` (the roster's auto-scanned user
 ...
 *
 * 2. Serve the browser dialog's data route (`/aek-wsl-workspace/api`):
...
export const DEFAULT_ROUTE = '/aek-wsl-workspace/api'
...
export function apply(ctx: Context, config: Config): void {
  ...
  const aekHome = process.env.AEK_HOME ?? join(homedir(), '.dsh')
  ...
  if (agentPresets !== undefined) {
    ctx.effect(() => {
      void materializeVariants(agentPresets, aekHome, shellPath, fsPath).catch((error) => {
  ...
  if (skills !== undefined && typeof skills.registerProvider === 'function') {
    ...
    ), 'aek-dsh: WSL workspace skills provider')
  }
