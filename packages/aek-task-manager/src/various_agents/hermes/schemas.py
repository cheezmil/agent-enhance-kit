"""Tool schema definitions for AekTaskManager."""
...
TASK_CREATE_SCHEMA = {
    "name": "aek_task_create",
    "description": "Create a new aek task with phased steps and approval gates. "
    "Initializes a dedicated Git repo under <cwd>/.aek/aek-task-manager/<task_id>/. "
    ...
    "parameters": {
        ...
        "properties": {
            "aek_task_id": {
                "type": "string",
                "description": "Unique aek task identifier (alphanumeric + hyphens, e.g. 'sc-mrp-v1'). "
        ...
        "required": ["aek_task_id", "name", "phases"],
    },
}

TASK_STATUS_SCHEMA = {
    "name": "aek_task_status",
    "description": "Show the current status of a aek task — phase, step, pending gates, and recent artifacts.",
    "parameters": {
        "type": "object",
        "properties": {
            "aek_task_id": {
...
TASK_PAUSE_SCHEMA = {
    "name": "aek_task_pause",
    "description": "Pause the active aek task and save a PausedExecutionContext snapshot. "
    "The task can be resumed later with aek_task_resume. "
    "This snapshots pending tool calls, artifacts, and session context so the agent "
    "can recover mid-step without losing work.",
    "parameters": {
        "type": "object",
        "properties": {
            "aek_task_id": {"type": "string", "description": "Sag long term task to pause. Omit to pause the active task."},
...
TASK_RESUME_SCHEMA = {
    "name": "aek_task_resume",
    "description": "Resume a paused aek task from its PausedExecutionContext snapshot. "
    "Restores pending tool calls, artifacts, and context. "
    "After resuming, use aek_task_advance to proceed after an approval gate.",
    "parameters": {
        "type": "object",
        "properties": {
            "aek_task_id": {"type": "string", "description": "Sag long term task to resume. Omit to resume the active task."},
        },
        "required": [],
    },
}
...
TASK_ADVANCE_SCHEMA = {
    "name": "aek_task_advance",
    "description": "Advance the aek task to the next Step or Phase after completing the current one. "
    ...
    "parameters": {
        "type": "object",
        "properties": {
            "aek_task_id": {"type": "string", "description": "Sag long term task to advance. Omit for the active task."},
            "commit_message": {
                "type": "string",
            ...
            "artifacts_summary": {
                "type": "string",
                "description": "Brief summary of artifacts produced in this step (for aek task_state).",
            },
        },
        "required": [],
    },
...
TASK_APPROVE_SCHEMA = {
    "name": "aek_task_approve",
    "description": "Submit an approval decision for a pending gate. "
    "Used when a user approves/rejects a step or requests changes. "
    "After approval, the aek task status is updated and the approver's decision is recorded.",
    "parameters": {
        "type": "object",
        "properties": {
            "aek_task_id": {"type": "string", "description": "Sag long term task with pending gate. Omit for the active task."},
...
TASK_LIST_SCHEMA = {
    "name": "aek_task_list",
    "description": "List all aek tasks under <cwd>/.aek/aek-task-manager/, showing task_id, status, current phase/step, and last active time.",
    "parameters": {
        "type": "object",
        "properties": {
            "status_filter": {
...
TASK_COMMIT_SCHEMA = {
    "name": "aek_task_commit",
    "description": "Commit current working state to the aek task's Git repo. "
    "Auto-stages all tracked files. Use before aek_task_advance or aek_task_pause to persist work.",
    "parameters": {
        "type": "object",
        "properties": {
            "aek_task_id": {"type": "string", "description": "Sag long term task whose repo to commit. Omit for the active task."},
            "message": {
                "type": "string",
...
TASK_BRANCH_SCHEMA = {
    "name": "aek_task_branch",
    "description": "Create a new Git branch for the next step of the aek task. "
    "The branch name follows: step/<phase_id>/<step_id>-<short-description>. "
    "Automatically switches to the new branch.",
    "parameters": {
        "type": "object",
        "properties": {
            "aek_task_id": {"type": "string", "description": "Sag long term task whose repo to branch. Omit for the active task."},
            "branch_name": {
                "type": "string",
...
TASK_GIT_LOG_SCHEMA = {
    "name": "aek_task_git_log",
    "description": "Show the Git commit history for a aek task, most recent first.",
    "parameters": {
        "type": "object",
        "properties": {
            "aek_task_id": {"type": "string", "description": "Sag long term task whose git log to show. Omit for the active task."},
            "max_count": {
                "type": "integer",
                "default": 20,
...
TASK_RELATE_SCHEMA = {
    "name": "aek_task_relate",
    "description": "Declare a cross-pollination relationship between this aek task and another. "
    ...
    "parameters": {
        "type": "object",
        "properties": {
            "aek_task_id": {"type": "string", "description": "Sag long term task to add a relationship to. Omit for the active task."},
            "related_task_id": {"type": "string", "description": "The task_id of the related aek task to link."},
            "relationship": {
                "type": "string",
...
TASK_VERIFY_SCHEMA = {
    ...
    "Must pass before aek_task_advance if verification.must_pass is True.",
    "parameters": {
        "type": "object",
        "properties": {
            "aek_task_id": {
...
TASK_PLAN_SCHEMA = {
    ...
    "Creates .aek_plans/<step_id>.json with bite-sized subtasks. "
