...
describe('packaged static collect', () => {
  it('collects from a packed npm install for a fresh owner without an owner venv, cache, sync, or network', async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [
        path.join(consumer, 'node_modules', '@cheezmil', 'aek-task-manager', 'bin', 'comet.js'),
        'eval',
        skill,
        '--collect',
        '--project',
  ...
  it('proves taskless collect is zero-workload at the real packaged CLI boundary', async () => {
    ...
    const harnessRoot = path.join(consumer, 'node_modules', '@cheezmil', 'aek-task-manager', 'eval');
    ...
    const cli = path.join(consumer, 'node_modules', '@cheezmil', 'aek-task-manager', 'bin', 'comet.js');
