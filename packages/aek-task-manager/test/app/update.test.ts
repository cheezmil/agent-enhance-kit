...
async function writeFakeCometPackage(packageRoot: string, version: string): Promise<void> {
  ...
  await fs.writeFile(
    ...
    JSON.stringify({ name: '@cheezmil/aek-task-manager', version, bin: { comet: 'bin/comet.js' } }),
  );
...
describe('update command helpers', () => {
  ...
  beforeEach(async () => {
    ...
    await writeFakeCometPackage(path.join(fakeGlobalNpmRoot, '@cheezmil', 'aek-task-manager'), '0.4.0-beta.7');
      ...
      queueMicrotask(async () => {
        try {
          ...
          if (npmArgs[0] === 'root' && npmArgs[1] === '--global') {
            ...
            const packageSpec = npmArgs.find((arg) => arg.startsWith('@cheezmil/aek-task-manager@'))!;
            const requestedVersion = packageSpec.slice('@cheezmil/aek-task-manager@'.length);
            await writeFakeCometPackage(
              path.join(prefix, 'node_modules', '@cheezmil', 'aek-task-manager'),
              candidateVersionOverride ?? requestedVersion,
            );
            if (candidateBinEscapesPackage) {
              const packageRoot = path.join(prefix, 'node_modules', '@cheezmil', 'aek-task-manager');
              ...
              await fs.writeFile(
                path.join(packageRoot, 'package.json'),
                JSON.stringify({
                  name: '@cheezmil/aek-task-manager',
            ...
            const packageRoot = npmArgs.includes('-g')
              ? path.join(fakeGlobalNpmRoot, '@cheezmil', 'aek-task-manager')
              : path.join(
  ...
  it('detects project package scope from local node_modules install path', async () => {
    const projectDir = path.join(tmpDir, 'project');
    const packageRoot = path.join(projectDir, 'node_modules', '@cheezmil', 'aek-task-manager');
  ...
  it('detects project package scope from package.json dependencies', async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ devDependencies: { '@cheezmil/aek-task-manager': '^0.2.4' } }),
      'utf-8',
    );
  ...
  it('builds npm update args preserving package install scope with official registry', () => {
    expect(buildNpmUpdateArgs('global')).toEqual([
      'install',
      '-g',
      '@cheezmil/aek-task-manager@latest',
      '--registry',
  ...
  it('covers self-update semver and package-scope decision branches', async () => {
    ...
    await fs.mkdir(path.join(project, 'node_modules', '@cheezmil', 'aek-task-manager'), { recursive: true });
    await expect(
      detectCometPackageScope(project, path.join(project, 'node_modules', '@cheezmil', 'aek-task-manager')),
    ).resolves.toBe('project');
    await fs.rm(path.join(project, 'node_modules'), { recursive: true, force: true });
    await fs.writeFile(
      path.join(project, 'package.json'),
      JSON.stringify({ optionalDependencies: { '@cheezmil/aek-task-manager': '^0.4.0' } }),
    );
  ...
  it('compares against the actual installed global package instead of the running CLI version', async () => {
    await fs.mkdir(path.join(tmpDir, '.claude', 'skills', 'comet'), { recursive: true });
    await fs.writeFile(path.join(tmpDir, '.claude', 'skills', 'comet', 'SKILL.md'), '# Comet');
    await writeFakeCometPackage(path.join(fakeGlobalNpmRoot, '@cheezmil', 'aek-task-manager'), '0.4.0-beta.9');
    mockedGetLatestVersion.mockResolvedValue('0.4.0-beta.8');
  ...
  it('allows current-project to opt into a validated self-update explicitly', async () => {
    ...
    try {
      ...
      expect(result.npm).toMatchObject({
        scope: 'global',
        status: 'updated',
        command: 'npm install -g @cheezmil/aek-task-manager@0.4.0-beta.8 --registry https://registry.npmjs.org',
      });
    } finally {
      log.mockRestore();
      homedirSpy.mockRestore();
    ...
    expect(mockedSpawn.mock.calls.at(-1)?.[1]?.slice(1)).toEqual([
      'install',
      '-g',
      '@cheezmil/aek-task-manager@0.4.0-beta.8',
      '--registry',
      'https://registry.npmjs.org',
    ]);
  ...
  it('restores the exact current version when the npm install command fails', async () => {
    ...
    expect(installCalls.map((call) => call[1]?.[3])).toEqual([
      '@cheezmil/aek-task-manager@0.4.0-beta.8',
      '@cheezmil/aek-task-manager@0.4.0-beta.7',
    ]);
  });
  ...
  it('restores project package metadata byte-for-byte after a failed project install', async () => {
    ...
    const packageJson = '{\n  "devDependencies": { "@cheezmil/aek-task-manager": "^0.4.0-beta.7" }\n}\n';
    ...
    await writeFakeCometPackage(
      path.join(projectDir, 'node_modules', '@cheezmil', 'aek-task-manager'),
      '0.4.0-beta.7',
    );
    targetInstallFailureVersion = '0.4.0-beta.8';
  ...
  it('reads a hoisted project package and restores workspace-root metadata after failure', async () => {
    ...
    await writeFakeCometPackage(
      path.join(projectNpmRootOverride, '@cheezmil', 'aek-task-manager'),
      '0.4.0-beta.7',
    );
    targetInstallFailureVersion = '0.4.0-beta.8';
    mutateProjectMetadataOnFailure = true;
  ...
  it('formats the npm update command for friendly console output', () => {
    expect(formatNpmUpdateCommand('global')).toBe(
      'npm install -g @cheezmil/aek-task-manager@latest --registry https://registry.npmjs.org',
    );
    expect(formatNpmUpdateCommand('project')).toBe(
      'npm install @cheezmil/aek-task-manager@latest --registry https://registry.npmjs.org',
    );
  });
