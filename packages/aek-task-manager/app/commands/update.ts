...
const PACKAGE_NAME = '@cheezmil/aek-task-manager';
...
async function detectCometPackageScope(
  ...
  const localPackageRoot = path.join(projectPath, 'node_modules', '@cheezmil', 'aek-task-manager');
...
async function readInstalledCometPackage(
  ...
  if (scope === 'project') {
    ...
    const installedPackage = await readCometPackage(path.join(npmRoot, '@cheezmil', 'aek-task-manager'));
    return {
      ...installedPackage,
  ...
  return readCometPackage(path.join(npmRoot, '@cheezmil', 'aek-task-manager'));
}
  ...
  try {
    ...
    if (!install.success) {
      ...
      const candidate = await readCometPackage(
        path.join(validationDir, 'node_modules', '@cheezmil', 'aek-task-manager'),
      );
