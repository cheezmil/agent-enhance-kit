...
// SettingsPath returns ~/.aek/websearch/websearch-settings.jsonc.
func SettingsPath() string {
	return filepath.Join(aekDir(), "websearch", "websearch-settings.jsonc")
}

// KeysDir returns ~/.aek/websearch/.
func KeysDir() string {
	return filepath.Join(aekDir(), "websearch")
}

// stripJSONC removes // line comments from JSONC content, skipping strings.
func stripJSONC(data []byte) []byte {
	...
	for _, line := range lines {
		...
		out = append(out, strings.TrimRight(line[:end], " \t"))
	}
	return []byte(strings.Join(out, "\n"))
}

// ReadKeys reads all non-comment, non-empty lines from ~/.aek/websearch/<name>.txt.
...
// ReadKey returns the first key from ~/.aek/websearch/<name>.txt, or "".
...
// Load reads ~/.aek/websearch/websearch-settings.jsonc, falls back to defaults.
func Load() Config {
	if globalConfig != nil {
		return *globalConfig
	}
