...
func GetPackageDetails(c *gin.Context) {
	...
	case "npm":
		...
		if isInstalled && mcpConfig != nil {
			...
			// 2. 从 settings.jsonc 加载用户配置覆盖默认配置
			if userID != 0 {
				settingsEnvs, settingsErr := common.GetServiceEnvsFromSettingsByName(installedService.Name)
				if settingsErr == nil && len(settingsEnvs) > 0 {
					for k, v := range settingsEnvs {
						finalEnvValues[k] = v // settings.jsonc 配置覆盖默认配置
					}
				}
			}
...
func ListInstalledMCPServices(c *gin.Context) {
	...
	for _, svc := range services {
		...
		// 2. 从 settings.jsonc 加载用户配置覆盖默认配置
		if userID != 0 {
			settingsEnvs, settingsErr := common.GetServiceEnvsFromSettingsByName(svc.Name)
			if settingsErr == nil && len(settingsEnvs) > 0 {
				for k, v := range settingsEnvs {
					finalEnvVars[k] = v // settings.jsonc 配置覆盖默认配置
				}
			}
		}
