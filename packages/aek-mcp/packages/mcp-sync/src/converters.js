// 头尾注释标记，用于标识 MCP 配置中由程序管理的部分
// 每次重新同步时只替换标记内的内容，保留用户手写的外部内容
export const HEAD_MARKER = '# head-aek-mcp';
export const END_MARKER = '# end-aek-mcp';
...
function parseCodexTool(rawContent) {
  ...
  let match;
  let managedContent = rawContent;

  // 检测头尾注释标记，提取被管理的内容区块
  const headIdx = rawContent.indexOf(HEAD_MARKER);
  const endIdx = rawContent.indexOf(END_MARKER);
  let preamble = '';
  let epilogue = '';

  if (headIdx !== -1 && endIdx !== -1 && endIdx > headIdx) {
    preamble = rawContent.slice(0, headIdx);
    managedContent = rawContent.slice(headIdx, endIdx + END_MARKER.length);
    epilogue = rawContent.slice(endIdx + END_MARKER.length);
  } else if (headIdx !== -1 || endIdx !== -1) {
    // 只有一侧标记，不完整——忽略标记，将整个内容视为被管理块
    preamble = '';
    managedContent = rawContent;
    epilogue = '';
  }
  ...
  // 无标记时：保留首个 section 之前和最后一个 section 之后的原始内容
  if (headIdx === -1 && endIdx === -1) {
    innerEpilogue = lastIndex > 0 ? managedContent.slice(lastIndex) : '';
  }
...
function formatCodexTool(canonical, meta) {
  ...
  const innerPreamble = meta?.innerPreamble ?? '';
  const innerEpilogue = meta?.innerEpilogue ?? '';
  ...
  // 有标记时：用 innerPreamble 的内容（通常是标记行本身）
  if (meta?.preamble !== undefined && meta?.preamble !== '') {
    // 有标记，用标记包裹
    sections.push(HEAD_MARKER);
  } else if (sections.length > 0 || orderedIds.length > 0) {
    // 无标记但首次创建：添加标记
    sections.push(HEAD_MARKER);
  }

  // 无标记时：保留原始的 innerPreamble（首个 section 之前的注释等）
  if (meta?.preamble === '' || meta?.preamble === undefined) {
    if (innerPreamble.trim()) {
      sections.push(innerPreamble.trimEnd());
    }
  }
  ...
  // 无标记时：保留原始的 innerEpilogue
  if ((meta?.preamble === '' || meta?.preamble === undefined) && innerEpilogue.trim()) {
    sections.push(innerEpilogue.trimStart());
  }
