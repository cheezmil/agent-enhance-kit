...
async function main() {
  ...
  try {
    ...
    console.error(`[mcp-sync] ${error.message ?? error}`);
    process.exitCode = 1;
  }
}

function printUsage() {
  console.log('Usage: npx mcp-sync <source_keyword> <target_keyword>');
...
async function runDirectSync(sourceKeyword, targetKeyword) {
  ...
  console.log(`[mcp-sync] Syncing from ${sourceTool.name} -> ${targetTool.name}`);
...
async function runInteractiveSync() {
  ...
  console.log("\x1b[41m ***** Welcome to mcp-sync interactive mode.***** \x1b[0m")
...
async function resolveSourceSelection(selection) {
  ...
  if (inferredTool) {
    console.log(`[mcp-sync] Detected ${inferredTool.name} configuration.`);
  }
...
async function performSync({ sourcePath, targetPath, sourceTool, targetTool }) {
  ...
  if (resolvedSource === resolvedTarget) {
    console.log('[mcp-sync] Source and target files are identical. No action taken.');
    return;
  }
    ...
    try {
      ...
      if (await fileExists(resolvedTarget)) {
        try {
          ...
          console.warn(`[mcp-sync] Warning: unable to parse existing ${targetTool.name} configuration. A fresh file will be generated.`);
        }
      }
  ...
  console.log(`[mcp-sync] ${action} ${sourceLabel} -> ${targetLabel}.`);
  if (backupLocation) {
    console.log(`[mcp-sync] Backup stored at ${backupLocation}`);
  }
}
...
async function backupFile(targetPath) {
  ...
  const backupRoot = path.join(os.homedir(), '.mcp-sync', 'backup');
