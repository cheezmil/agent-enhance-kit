# AEK Browser AutoFix — Automatic Adapter Self-Repair

When an `aekb` command fails because a website changed its DOM, API, or response schema, **automatically diagnose, fix the adapter, and retry** — don't just report the error.
...
- **`BROWSER_CONNECT`** (exit code 69) — **STOP.** Do not modify code. Tell the user to run `aekb doctor`.
- **CAPTCHA / rate limiting** — **STOP.** Not an adapter issue.

**Scope constraint:**
- **Only modify the file at `adapterSourcePath` in the trace `summary.md` front matter** — this is the authoritative adapter location (may be `clis/<site>/` in repo or `~/.aek/browser/system/clis/<site>/` for npm installs)
...
```bash
aekb doctor    # Verify extension + daemon connectivity
```

## When to Use This Skill

Use when `aekb <site> <command>` fails with repairable errors:
...
- **Retry with an alternative query or entry point.** If `aekb xiaohongshu search "X"` returns 0 but `aekb xiaohongshu search "X 攻略"` returns 20, the adapter is fine — the platform was shaping results for the first query.
- **Spot-check in a normal Chrome tab.** If the data is visible in the user's own browser but the adapter comes back empty, the issue is usually authentication state, rate limiting, or a soft block — not a code bug. The fix is `aekb doctor` / re-login, not editing source.
...
```bash
aekb <site> <command> [args...] --trace retain-on-failure 2>trace-error.yaml
```
...
trace:
  ...
  aek-browserVersion: "..."
  traceId: "..."
  dir: "/path/to/.aek/browser/system/profiles/default/traces/..."
  summaryPath: "/path/to/.aek/browser/system/profiles/default/traces/.../summary.md"
  receiptPath: "/path/to/.aek/browser/system/profiles/default/traces/.../receipt.json"
```
...
Use `aekb browser` to inspect the live website. **Never use the broken adapter** — it will just fail again.

### DOM changed (SELECTOR errors)

```bash
# Open the page and inspect current DOM
aekb browser open https://example.com/target-page && aekb browser state
...
aekb browser click <N> && aekb browser network

# Narrow to the request you care about by the fields its body should have
aekb browser network --filter author,text,likes

# Inspect specific API response (key is the `key` field from the default JSON output)
aekb browser network --detail <key>
```

## Step 4: Patch the Adapter

Read the adapter source file at `adapterSourcePath` from the trace summary front matter and make targeted fixes. This path is authoritative — it may be in the repo (`clis/`) or user-local (`~/.aek/browser/system/clis/`).
...
If the retry **passes**, the local adapter has drifted from upstream. File a GitHub issue so the fix flows back to `cheezmil/AEK Browser`.
...
```markdown
## Summary
AEK Browser autofix repaired this adapter locally, and the retry passed.

## Adapter
- Site: `<site>`
- Command: `<command>`
- AEK Browser version: `<version from aekb --version>`
...
_Issue filed by AEK Browser autofix after a verified local repair._
```
...
```bash
gh issue create --repo cheezmil/AEK Browser \
...
- **Major redesign** — needs full adapter rewrite via `references/adapter-author.md` skill
...
```
1. User runs: aekb zhihu hot
   → Fails: SELECTOR "Could not find element: .HotList-item"

2. AI runs: aekb zhihu hot --trace retain-on-failure 2>trace-error.yaml
...
4. AI explores: aekb browser open https://www.zhihu.com/hot && aekb browser state
...
6. AI verifies: aekb zhihu hot
   → Success: returns hot topics

7. AI prepares upstream issue draft, shows it to the user

8. User approves → AI runs: gh issue create --repo cheezmil/AEK Browser --title "[autofix] zhihu/hot: SELECTOR" --body "..."
```
