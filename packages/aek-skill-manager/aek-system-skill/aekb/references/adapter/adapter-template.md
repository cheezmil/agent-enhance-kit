...
import { CliError } from '@cheezmil/aek-browser/errors';
...
cli({
  ...
  description: '...',         // 一句话，出现在 `aekb list` 和 `aekb <site> -h`
...
```javascript
import { ArgumentError, CommandExecutionError, EmptyResultError } from '@cheezmil/aek-browser/errors';
...
```javascript
import { cli, Strategy } from '@cheezmil/aek-browser/registry';
import { ArgumentError, AuthRequiredError, CommandExecutionError, EmptyResultError } from '@cheezmil/aek-browser/errors';
...
为什么不在 daemon 端固定 Chrome locale？因为 aekb **不启动 Chrome**——daemon 是连用户已经在跑的 Chrome（CDP via extension），用户可能就是中文 UI / 中文资料检索需求。强制 en-US 会破坏用户的正当工作流。
...
## Verify fixture（每个 adapter 配一份 `~/.aek/browser/system/sites/<site>/verify/<name>.json`）

verify fixture 是"adapter 产出长什么样"的结构锚点。没有它，`aekb browser verify` 只能证"adapter 能跑完不抛"，证不出数据没错位。**必写**。
...
aekb browser verify 1point3acres/hot --write-fixture

# 2. 手改 ~/.aek/browser/system/sites/1point3acres/verify/hot.json
#    - patterns: 加 URL / 日期 / ID 正则
#    - notEmpty: 加核心字段（title / author / url）
#    - rowCount: 收紧到业务合理区间

# 3. 再跑 verify，fixture 吃得动就 OK
aekb browser verify 1point3acres/hot
```
...
aekb 1point3acres thread 1173710 --limit 2 --format json | head

# 2. 照着响应手写 ~/.aek/browser/system/sites/1point3acres/verify/thread.json
#    （args 一定用数组: ["1173710", "--limit", "2"]）

# 3. 跑 verify 核对
aekb browser verify 1point3acres/thread
```
...
```
~/.aek/browser/system/clis/<site>/<name>.js    # 私人
clis/<site>/<name>.js               # repo 贡献
```

**两者在 `cli({...})` 层面完全一样**。差别只在运行入口：

- 私人：写完立即可跑（`aekb <site> <name>`）
- repo：要 `npm run build` 才被注册

先在 `~/.aek/browser/system/clis/` 调通再拷贝到 `clis/`。
