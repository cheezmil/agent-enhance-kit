...
This skill is for **driving a live browser** to accomplish an agent task. If you are building a reusable adapter under `~/.aek/browser/system/clis/<site>/` use `references/adapter-author.md` instead.
...
- `aekb *` (or `aekb browser *`) commands require a `<session>` positional. You can omit `browser`/`b` — `aekb <session> <cmd>` is the preferred form. Use the same session name for a multi-step flow; use a different name to isolate parallel browser work.
- Use a stable session name for any multi-command or human-paced browser workflow. Example: `aekb fb-yaya-warmup open https://example.com`, then reuse `aekb fb-yaya-warmup state`, `extract`, `click`, etc.
- Owned browser sessions keep a tab lease alive between calls. Release it with `aekb <session> close` or let the idle timeout expire.
- `aekb <session> bind` binds the Chrome tab you already have open to that session. Use this for logged-in pages, SSO flows, or pages you manually positioned before handing control to the agent.
- `--window foreground|background` (or `OPENCLI_WINDOW=foreground|background`) chooses whether AEK Browser creates/focuses a foreground browser window or uses a background browser window for owned sessions.
...
aekb gmail bind
aekb gmail state
aekb gmail click "Search"
aekb gmail network
aekb gmail unbind
```

Binding never owns the user window and never closes the user tab. It fails closed if the tab is closed or becomes non-debuggable. Re-run `aekb <session> bind` when you switch to a different real tab.

Navigation is allowed on bound sessions because the session now represents explicit agent ownership of that tab. Tab mutation (`tab new`, `tab select`, `tab close`) is still blocked for bound sessions. Use an owned session when you want AEK Browser to manage tab lifecycle.

Bound sessions have no AEK Browser idle-close timer; the binding lasts until `unbind`, tab close, window close, or daemon restart.
...
2. **Prefer site adapters before raw browser driving.** If `aekb <site> <command>` already covers the task, use that adapter command first (`aekb facebook notifications`, `aekb reddit read`, `aekb chatgpt model <level>`, etc.). Use `aekb browser ...` only for gaps, debugging, or one-off UI flows the adapter does not expose.
...
If `browser open` or `browser analyze` returns `sitemap.available: true`, switch to `references/browser-sitemap.md` before continuing a multi-step site flow. The sitemap is prior context for pages, actions, workflows, APIs, and pitfalls; it is not truth. If the browser state disagrees with the sitemap, trust the browser and mark the sitemap stale via `references/sitemap-author.md`.
...
List entries look like `{key, method, status, url, ct, size, shape, body_truncated?}`. Detail envelope is `{key, url, method, status, ct, size, shape, body, body_truncated?, body_full_size?, body_truncation_reason}`. Cache lives in `~/.aek/browser/system/cache/browser-network/` so you can re-inspect without re-triggering the request.
...
```bash
aekb hn open "https://news.ycombinator.com" \
  && aekb hn state \
  && aekb hn click 3
```
...
```bash
aekb login open "https://example.com/login"
aekb login state                          # find [N] for email, password, submit
aekb login type 4 "me@example.com"
aekb login type 5 "hunter2"
aekb login get value 4                    # verify (autocomplete can eat chars)
aekb login click 6                        # submit
aekb login wait selector "[data-testid=account-menu]" --timeout 15000
aekb login state                          # fresh refs on the logged-in page
```

### Pick from a long dropdown

```bash
aekb form state                          # sidebar shows [12] <select name=country>
aekb form find --css "select[name=country]"
# the compound.options_total is 137, but compound.current is "" — unselected.
aekb form select 12 "Uruguay"
aekb form get value 12                   # { value: "uy", match_level: "exact" }
```
...
```bash
aekb mercury state                          # find category trigger ref
# If the trigger/option is not clear, use AX:
aekb mercury state --source ax              # look for combobox/button/listbox/option names
aekb mercury click 7                        # click category trigger
aekb mercury state --source ax              # fresh refs after the portal/listbox opens
aekb mercury click 12                       # click option
aekb mercury get text 7                     # verify visible selected label
```
...
```bash
aekb compare state --compare-sources
```
...
```bash
aekb hn open "https://news.ycombinator.com"
aekb hn network --filter "title,score"
# -> find the /topstories entry, note its key
aekb hn network --detail topstories-a1b2
```

### Read a long article in chunks

```bash
aekb browser article open "https://blog.example.com/long-post"
aekb browser article extract --chunk-size 8000
# -> content + next_start_char: 8000
aekb browser article extract --start 8000 --chunk-size 8000
...
```bash
aekb browser checkout frames
# -> [{"index": 0, "url": "https://checkout.stripe.com/...", ...}]
aekb browser checkout eval "(() => document.querySelector('input[name=cardnumber]')?.value)()" --frame 0
```
...
| Network cache seems stale | Bump `--ttl` down, or let it expire. The cache lives at `~/.aek/browser/system/cache/browser-network/`. |

---

## See also

- `references/adapter-author.md` — turning what you just figured out into a reusable `~/.aek/browser/system/clis/<site>/<command>.js`.
- `references/browser-sitemap.md` — consuming site sitemap context while driving a browser task.
- `references/sitemap-author.md` — creating or updating sitemap knowledge when you discover a durable path or stale entry.
- `references/autofix.md` — when an existing adapter breaks, this skill walks you through `--trace retain-on-failure` evidence and filing a fix.
