...
根据话题和场景，将查询路由到最佳的 aekb 搜索源。此 skill 的核心目标不是记忆命令，而是先定位数据源，再让 Agent 通过 `aekb` 自己读取实时帮助，避免文档漂移。
...
- 运行 `aekb <site> -h` 查看该站点有哪些子命令
- 若已锁定某个子命令，再运行 `aekb <site> <command> -h` 查看参数、输出列、策略

不要在 skill 文档里硬编码参数或假设命令签名；以 `aekb ... -h` 的实时输出为准。
...
- `aekb list -f yaml`、`aekb <site> -h`、`aekb <site> <command> -h` 属于预检与帮助，不计入搜索次数
- 一次真正的 `aekb <site> ...` 搜索/查询执行，计为该站点 1 次调用
...
- 始终以 `aekb list -f yaml` 与 `aekb <site> -h` 的实际结果为准
...
- **`references/search/sources-ai.md`** — AI 默认源
- **`references/search/sources-tech.md`** — 技术 / 学术
- **`references/search/sources-social.md`** — 社交媒体
- **`references/search/sources-media.md`** — 媒体 / 娱乐
- **`references/search/sources-info.md`** — 资讯 / 知识
- **`references/search/sources-shopping.md`** — 购物
- **`references/search/sources-travel.md`** — 旅游
- **`references/search/sources-other.md`** — 其他垂直源
