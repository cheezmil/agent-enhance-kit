...
aek-browser arxiv search "Direct Preference Optimization" --limit 5 -f json
aek-browser arxiv paper 2305.18290 -f json

# 2. dblp bibliography record + co-authors + venue history
aek-browser dblp search "Direct Preference Optimization" --limit 5 -f json

# 3. Community uptake on Hugging Face: trending Daily Papers that mention DPO
aek-browser hf top --period monthly --limit 50 -f json | jq '.[] | select(.title | test("DPO|preference"; "i"))'

# 4. Conference review record (if posted to OpenReview)
aek-browser openreview search "Direct Preference Optimization" --limit 5 -f json
```
...
- Each adapter alone is just "search a website". The value is the chain. Four `aek-browser` calls feed into one LLM call. No browser, no copy-paste.
- Output is identifier-rich (arxiv id, dblp key, venue id, HF paper id). I can re-feed any of those into the next call, e.g. once I find a follow-up arxiv id from HF Daily Papers I run `aek-browser arxiv paper <new-id>` immediately.
...
Without `aek-browser dblp search` (added in #1299) and `aek-browser openreview search` (added in #1294), this whole pipeline used to require either web scraping in agent code or paying for a research-paper API. Both adapters being public-strategy means they slot in cleanly.
