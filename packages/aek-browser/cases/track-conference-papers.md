...
aek-browser openreview venue "ICLR 2024 oral" --limit 200 -f json > /tmp/iclr-2024.json
...
aek-browser openreview reviews KS8mIvetg2 -f json > /tmp/reviews.json

# 3. Single paper metadata if needed
aek-browser openreview paper KS8mIvetg2 -f json
```
...
For each shortlisted forum id, run `aek-browser openreview reviews <forum-id>` and feed the JSON to an LLM with the prompt:
...
- Forum ids returned from `venue` are stable and reusable across calls. Easy to keep a personal reading list as `forum-ids.txt` and run `for id in $(cat forum-ids.txt); do aek-browser openreview reviews $id; done`.
- The whole loop is public-strategy. No login required for venues with public reviewing.

`aek-browser openreview` (added in #1294) is the lever. Before this adapter existed, the same workflow needed either OpenReview's Python client or HTML scraping inside agent code. Both have higher friction than `aek-browser openreview reviews <forum-id>` returning structured JSON in one shot.
