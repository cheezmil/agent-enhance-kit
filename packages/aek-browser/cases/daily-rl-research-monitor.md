...
aek-browser arxiv recent cs.LG --limit 30 -f json > /tmp/lg.json
aek-browser arxiv recent cs.AI --limit 30 -f json > /tmp/ai.json
...
aek-browser openreview venue "NeurIPS 2025 oral" --limit 50 -f json > /tmp/neurips.json

# 3. Hugging Face Daily Papers (community-upvoted research)
aek-browser hf top --period daily --limit 20 -f json > /tmp/hf.json
```
...
- Four pages of clicking and scrolling collapses into four `aek-browser` calls.
...
- Zero token cost on the AEK Browser side. The only paid step is the digest call at the end.
