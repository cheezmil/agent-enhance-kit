...
async function ensureAttached(tabId, aggressiveRetry = false) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTACH_RETRIES; attempt++) {
    try {
      ...
      if (attempt < MAX_ATTACH_RETRIES) {
        console.warn(`[aek-browser] attach attempt ${attempt}/${MAX_ATTACH_RETRIES} failed: ${lastError}, retrying in ${RETRY_DELAY_MS}ms...`);
  ...
  if (lastError) {
    ...
    console.warn(`[aek-browser] attach failed for tab ${tabId}: url=${finalUrl}, windowId=${finalWindowId}, error=${lastError}`);
...
var JOURNAL_KEY = "aek-browser_command_journal_v1";
...
var CONTEXT_ID_KEY = "aek-browser_context_id_v1";
var currentContextId = "default";
var contextIdPromise = null;
var connectInFlight = null;
var workerReady = Promise.resolve();
var workerRecovered = true;
...
async function connectAttempt() {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`[aek-browser] daemon ping failed: HTTP ${res.status}`);
      scheduleReconnect();
      return;
    }
    reconnectAttempts = 0;
  ...
  thisWs.onopen = () => {
    console.log("[aek-browser] onopen fired, thisWs.readyState:", thisWs.readyState);
    console.log("[aek-browser] Connected to daemon");
    reconnectAttempts = 0;
    if (reconnectTimer) {
    ...
    console.log("[aek-browser] Sending hello:", JSON.stringify(helloMsg));
    const sent = safeSend(thisWs, helloMsg);
    console.log("[aek-browser] Hello sent:", sent);
  ...
  thisWs.onmessage = async (event) => {
    ...
    try {
      ...
      console.error("[aek-browser] Message handling error:", err);
    }
  };
  thisWs.onclose = () => {
    stopWsKeepalive(thisWs);
    if (ws !== thisWs) return;
    console.log("[aek-browser] Disconnected from daemon");
    ws = null;
    scheduleReconnect();
  };
  thisWs.onerror = () => {
...
var WS_KEEPALIVE_INTERVAL_MS = 2e4;
var wsKeepaliveTimer = null;
var wsKeepaliveSocket = null;
...
var REGISTRY_KEY = "aek-browser_target_lease_registry_v2";
var LEASE_IDLE_ALARM_PREFIX = "aek-browser:lease-idle:";
var CONTAINER_TAB_GROUP_TITLE = {
  interactive: "AEK Browser Browser",
  // Retained for registry/type compatibility. Adapter automation no longer
  // creates or discovers a visible tab group.
  automation: "AEK Browser Adapter"
};
...
var interactiveGroupLedger = /* @__PURE__ */ new Set();
...
function getSessionName(session) {
  ...
  if (!raw) throw new CommandFailure(
    ...
    "Pass a browser session name, e.g. aek-browser browser <session> <command>."
  );
  return raw;
}
...
async function safeDetach(tabId) {
  try {
    const detach2 = detach;
    if (typeof detach2 === "function") await detach2(tabId);
  } catch {
  }
}
...
async function ensureOwnedContainerGroupUnlocked(role, fallbackWindowId, ids) {
  try {
    ...
    console.warn(`[aek-browser] Failed to ensure ${role} tab group: ${err instanceof Error ? err.message : String(err)}`);
    throw err;
  }
}
...
async function ensureOwnedContainerWindowUnlocked(role, initialUrl, mode = "background") {
  ...
  await persistRuntimeState();
  console.log(`[aek-browser] Created owned ${role} window ${container.windowId} (start=${startUrl})`);
...
async function findReusableOwnedContainerTab(windowId, ownedGroupId) {
  try {
    ...
    const reusable = tabs.find(
      (tab) => tab.id !== void 0 && initialTabIsAvailable(tab.id) && isDebuggableUrl2(tab.url) && (ownedGroupId === void 0 || ownedGroupId !== null && tab.groupId === ownedGroupId || !isSafeNavigationUrl(tab.url ?? ""))
    );
    return reusable?.id;
  } catch {
    return void 0;
  ...
  if (existing) {
    if (!existing.owned) {
      throw new CommandFailure(
        ...
        `Session "${existing.session}" is bound to a user tab and does not own an AEK Browser tab lease.`,
    ...
    try {
      ...
      if (tabId !== null) {
        const tab = await chrome.tabs.get(tabId);
        if (isDebuggableUrl2(tab.url)) return tab.windowId;
      }
...
chrome.windows.onRemoved.addListener(async (windowId) => {
  ...
  for (const [leaseKey, session] of automationSessions.entries()) {
    if (session.windowId === windowId) {
      console.log(`[aek-browser] ${session.surface} container closed (session=${session.session})`);
      if (session.idleTimer) clearTimeout(session.idleTimer);
      automationSessions.delete(leaseKey);
      sessionOverrides.delete(leaseKey);
      scheduleIdleAlarm(leaseKey, IDLE_TIMEOUT_NONE);
...
chrome.tabs.onRemoved.addListener(async (tabId) => {
  ...
  for (const [leaseKey, session] of automationSessions.entries()) {
    if (session.preferredTabId === tabId) {
      ...
      sessionOverrides.delete(leaseKey);
      scheduleIdleAlarm(leaseKey, IDLE_TIMEOUT_NONE);
      console.log(`[aek-browser] Session ${session.session} detached from tab ${tabId} (tab closed)`);
    }
  }
  await persistRuntimeState();
});
...
function initialize() {
  ...
  registerListeners();
  try {
    const registerFrameTracking2 = registerFrameTracking;
    registerFrameTracking2?.();
  } catch {
  }
  try {
  ...
  workerReady = (async () => {
    ...
    console.warn(`[aek-browser] Startup recovery failed: ${err instanceof Error ? err.message : String(err)}`);
  }).finally(() => {
    workerRecovered = true;
  });
  void workerReady.then(() => connect());
  console.log("[aek-browser] AEK Browser extension initialized");
}
...
async function resolveCommandTabId(cmd) {
  if (cmd.page) return resolveTabId(cmd.page);
  return void 0;
}
async function resolveTab(tabId, leaseKey, initialUrl) {
  ...
  if (tabId !== void 0) {
    try {
      ...
      if (isDebuggableUrl2(tab.url) && matchesSession) return { tabId, tab };
      if (session && !session.owned) {
        throw new CommandFailure(
          ...
          'Run "aekb browser <session> bind" again on a debuggable http(s) tab.'
        );
      }
      if (session && !matchesSession && session.preferredTabId === null && isDebuggableUrl2(tab.url)) {
        console.warn(`[aek-browser] Tab ${tabId} drifted to window ${tab.windowId}, moving back to ${session.windowId}`);
        try {
          await chrome.tabs.move(tabId, { windowId: session.windowId, index: -1 });
          const moved = await chrome.tabs.get(tabId);
          if (moved.windowId === session.windowId && isDebuggableUrl2(moved.url)) {
            return { tabId, tab: moved };
          }
        } catch (moveErr) {
          console.warn(`[aek-browser] Failed to move tab back: ${moveErr}`);
        }
      } else if (!isDebuggableUrl2(tab.url)) {
        console.warn(`[aek-browser] Tab ${tabId} URL is not debuggable (${tab.url}), re-resolving`);
      }
    } catch (err) {
      ...
      if (existingSession && !existingSession.owned) {
        ...
        throw new CommandFailure(
          ...
          'Run "aek-browser browser bind" again, then retry the command.'
        );
      }
      console.warn(`[aek-browser] Tab ${tabId} no longer exists, re-resolving`);
    }
  }
  ...
  if (existingSession && existingPreferredTabId !== null) {
    ...
    try {
      ...
      if (isDebuggableUrl2(preferredTab.url)) return { tabId: preferredTab.id, tab: preferredTab };
      if (!session.owned) {
        throw new CommandFailure(
          "bound_tab_not_debuggable",
          `Bound tab for session "${session.session}" is not debuggable (${preferredTab.url ?? "unknown URL"}).`,
          'Switch the tab to an http(s) page or run "aek-browser browser bind" on another tab.'
        );
      }
    } catch (err) {
      if (err instanceof CommandFailure) throw err;
  ...
  if (reuseTab?.id) {
    ...
    try {
      ...
      if (isDebuggableUrl2(updated.url)) return { tabId: reuseTab.id, tab: updated };
      console.warn(`[aek-browser] data: URI was intercepted (${updated.url}), creating fresh tab`);
    } catch {
    }
  }
...
async function resolveTabId2(tabId, leaseKey, initialUrl) {
...
async function listAutomationWebTabs(leaseKey) {
  ...
  return tabs.filter((tab) => isDebuggableUrl2(tab.url));
}
...
async function handleExec(cmd, leaseKey) {
  ...
  const tabId = await resolveTabId2(cmdTabId, leaseKey);
  try {
...
async function handleNavigate(cmd, leaseKey) {
  ...
  await new Promise((resolve) => {
    ...
    timeoutTimer = setTimeout(() => {
      timedOut = true;
      console.warn(`[aek-browser] Navigate to ${targetUrl} timed out after 15s`);
      finish();
    }, 15e3);
  });
  ...
  if (postNavigationSession && tab.windowId !== postNavigationSession.windowId) {
    console.warn(`[aek-browser] Tab ${tabId} drifted to window ${tab.windowId} during navigation, moving back to ${postNavigationSession.windowId}`);
    try {
      await chrome.tabs.move(tabId, { windowId: postNavigationSession.windowId, index: -1 });
      tab = await chrome.tabs.get(tabId);
    } catch (moveErr) {
      console.warn(`[aek-browser] Failed to recover drifted tab: ${moveErr}`);
    }
  }
...
async function handleTabs(cmd, leaseKey) {
  ...
  if (session && !session.owned && cmd.op !== "list") {
    return {
      ...
      error: `Session "${session.session}" is bound to a user tab; tab new/select/close requires an owned AEK Browser session.`,
      errorHint: "Unbind the session first, or use a different session for owned AEK Browser tabs."
    };
  }
  switch (cmd.op) {
    case "list": {
  ...
  if (session.owned) {
    ...
    if (tabId !== null) {
      ...
      if (hasOtherOwnedLease) {
        ...
        console.log(`[aek-browser] Released owned tab lease ${tabId} (session=${session.session}, surface=${session.surface}, ${reason})`);
      } else {
        try {
          ...
          console.log(`[aek-browser] Released owned tab lease ${tabId} as reusable placeholder (session=${session.session}, surface=${session.surface}, ${reason})`);
        } catch {
          await chrome.tabs.remove(tabId).catch(() => {
          });
          console.log(`[aek-browser] Released owned tab lease ${tabId} (session=${session.session}, surface=${session.surface}, ${reason})`);
      ...
      console.log(`[aek-browser] Released legacy owned window lease ${session.windowId} without closing container (session=${session.session}, surface=${session.surface}, ${reason})`);
    }
  } else if (session.preferredTabId !== null) {
    await safeDetach(session.preferredTabId);
    console.log(`[aek-browser] Detached borrowed tab lease ${session.preferredTabId} (session=${session.session}, surface=${session.surface}, ${reason})`);
  }
  automationSessions.delete(leaseKey);
  sessionOverrides.delete(leaseKey);
  await persistRuntimeState();
...
async function reconcileTargetLeaseRegistry() {
  ...
  for (const [leaseKey, stored] of Object.entries(registry.leases)) {
    ...
    try {
      const tab = await chrome.tabs.get(tabId);
      if (!isDebuggableUrl2(tab.url)) continue;
  ...
  try {
    ...
    console.warn(`[aek-browser] Startup interactive group convergence failed: ${err instanceof Error ? err.message : String(err)}`);
  }
  await persistRuntimeState();
}
async function handleBind(cmd, leaseKey) {
  ...
  const boundTab = activeTabs.find((tab) => isDebuggableUrl2(tab.url)) ?? fallbackTabs.find((tab) => isDebuggableUrl2(tab.url));
  if (!boundTab?.id) {
    return {
      id: cmd.id,
      ok: false,
  ...
  resetWindowIdleTimer(leaseKey);
  console.log(`[aek-browser] Session ${getSessionFromKey(leaseKey)} explicitly bound to tab ${boundTab.id} (${boundTab.url})`);
...
var __test__ = {
  ...
  getSession: (leaseKey = "default") => automationSessions.get(leaseKey) ?? null,
  getAutomationWindowId: (leaseKey = "default") => automationSessions.get(leaseKey)?.windowId ?? null,
  setAutomationWindowId: (leaseKey, windowId) => {
    if (windowId === null) {
      const session = automationSessions.get(leaseKey);
      if (session?.idleTimer) clearTimeout(session.idleTimer);
      automationSessions.delete(leaseKey);
