/**
 * AEK Browser — Service Worker (background script).
 *
 * Connects to the aek-browser daemon via WebSocket, receives commands,
 * dispatches them to Chrome APIs (debugger/tabs/cookies), returns results.
 */

declare const __AEK_BROWSER_COMPAT_RANGE__: string;
...
const CONTEXT_ID_KEY = 'aek-browser_context_id_v1';
...
async function connectAttempt(): Promise<void> {
  ...
  thisWs.onopen = () => {
    // ws may be null on first connection; check thisWs only
    ...
    const helloMsg = {
      ...
      compatRange: __AEK_BROWSER_COMPAT_RANGE__,
    };
...
const REGISTRY_KEY = 'aek-browser_target_lease_registry_v2';
const LEASE_IDLE_ALARM_PREFIX = 'aek-browser:lease-idle:';
...
async function ensureOwnedContainerGroup(
  ...
  // a visible "AEK Browser Adapter" tab group. Its ownership anchors are the
...
async function resolveTab(tabId: number | undefined, leaseKey: string, initialUrl?: string): Promise<ResolvedTab> {
  ...
  // content in a window where an AEK Browser group may have disappeared.
...
async function reconcileTargetLeaseRegistry(): Promise<void> {
  ...
  // of accumulating as untitled "AEK Browser Browser" duplicates (#2097). Best
