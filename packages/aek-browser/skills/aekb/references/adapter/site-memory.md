...
## Layer 2 — 本地工作目录（`~/.aek-b/sites/<site>/`）
...
~/.aek-b/sites/<site>/
  ...
  verify/                — `aek-b browser verify` 期望值（值级校验锚点，每个 adapter 一份）
    <cmd>.json
...
每个 adapter 一份，`aek-b browser verify <site>/<cmd>` 会自动读。**没有这份 = verify 只能证"能跑"，证不出数据对**——所以是必填产物。
...
Step 10 verify 首轮通过后 → 写 ~/.aek-b/sites/<site>/verify/<cmd>.json
                            - 先 `--write-fixture` 拿种子，再手改 patterns / notEmpty / rowCount
                            - 没这份后续 verify 挡不住数据错位，**必填**

Step 11 肉眼对比通过后 → 写 ~/.aek-b/sites/<site>/
...
调试过程里的临时 dump（`.dbg-*.html` / `raw-*.json` / `sample-*` / `trace-*.txt`）**只能**落在系统 `/tmp/`；只有通过上面数据分级、准备长期保留的安全样本才进入 `~/.aek-b/sites/<site>/fixtures/`。PR diff 会把 repo 根目录和 `clis/<site>/` 下的文件一起带走；任务结束还要删除原始 capture/cache。

---

## 没有 site-memory 时

新站点没对应 `.md`，也没本地目录 → 完整走 recon + discovery，跑完直接写 `~/.aek-b/sites/<site>/`，后面就有了。
