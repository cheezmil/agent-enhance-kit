...
aek-b browser eval "window.location.origin + '/<path>'"
```
...
2. **用两个已知含义的值各抓一份响应**
   ...
   ```bash
   aek-b browser eval "fetch('<url>&fid=f2&po=1').then(r=>r.json()).then(d=>d.data.diff.slice(0,3))"
   aek-b browser eval "fetch('<url>&fid=f3&po=1').then(r=>r.json()).then(d=>d.data.diff.slice(0,3))"
   ```
...
4. **用第三个参数交叉验证**
   ...
   ```bash
   aek-b browser eval "fetch('<url>&fid=f6&po=1').then(r=>r.json()).then(d=>d.data.diff.slice(0,3))"  # 成交额
   ```
...
```bash
# 按价格排，拿一条观察
aek-b browser eval "fetch('https://push2.eastmoney.com/api/qt/clist/get?fs=b:MK0354&pn=1&pz=1&fid=f2&po=1&fltt=2&fields=f12,f14,f2,f3,f236,f237,f239').then(r=>r.json()).then(d=>d.data.diff[0])"
# 返回 {f12:'123456', f14:'XX转债', f2:180.5, f3:2.1, f236:98.5, f237:83.2, f239:-4.1}

# 按 f237 排
aek-b browser eval "fetch('...&fid=f237&po=1...').then(...)"
# 第一条变了，f237 值变成 400+
...
1. **先数一次嵌套路径**
   ...
   ```bash
   aek-b browser eval "fetch('<url>').then(r=>r.json()).then(j=>({keys:Object.keys(j), type:Array.isArray(j)?'array':'object'}))"
   ```

2. **一层一层剥**

   ```bash
   aek-b browser eval "fetch('<url>').then(r=>r.json()).then(j=>{const d=j.data; return {keys:Object.keys(d), sample: d[Object.keys(d)[0]]}})"
   ```
...
4. **同一字段在不同条目间是否变化**
   ...
   aek-b browser eval "fetch('<url>').then(r=>r.json()).then(d=>d.data.diff.slice(0,3).map(x=>({f2:x.f2,f3:x.f3,f152:x.f152})))"
   ```
...
1. **补进 `references/adapter/field-conventions.md`**：找到对应站点的表格加一行。下次直接查。
2. **在 adapter 代码里留一条注释**：如果是实测推出来的不常见代号，写一行 `// f237 = convertible premium rate (verified 2026-04-20 against page)` 方便复核。
3. **通过 `aek-b browser verify` 验一次**：字段值能对上网页上眼见的数字。
