...
1. **Capture** the page's HTML from a live verify run:
   ```bash
   aek-b browser open https://www.example.com/<page>
   # In another shell, dump page.content():
   aek-b browser eval 'document.documentElement.outerHTML' \
...
- `references/adapter/adapter-template.md` — basic adapter file structure
- `references/adapter/output-design.md` — column naming for the post-extract mapping
- `references/adapter/success-rate-pitfalls.md` — broader "verify can pass while
