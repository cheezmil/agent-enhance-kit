...
async function main() {
  ...
  if (!taskName) {
    ...
    for (const task of tasks) {
      try { exec('aek-browser browser close'); } catch {}
      let lastOutput = '';
...
Do NOT fix the code — just diagnose. Use aek-browser browser commands to investigate.`;

    try {
      const result = execSync(
        `claude -p --dangerously-skip-permissions --allowedTools "Bash(aek-browser:*),Bash(npm:*),Read,Grep,Glob" --output-format text --no-session-persistence "${prompt.replace(/"/g, '\\"')}"`,
