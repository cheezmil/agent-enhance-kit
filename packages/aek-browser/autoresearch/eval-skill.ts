 ...
 *
 * Spawns Claude Code with the aek-browser-adapter-author skill. Claude Code
...
const SKILL_PATH = join(__dirname, '..', 'skills', 'aek-browser-adapter-author', 'SKILL.md');
...
const TASKS: SkillTask[] = [
  ...
  // Form
  { name: "form-fill-basic", task: "Fill the Customer Name with 'AEK Browser' and Telephone with '555-0100'. Do not submit.", url: "https://httpbin.org/forms/post", judge_context: ["The agent must type 'AEK Browser' into a name field", "The agent must type '555-0100' into a phone field", "The form must NOT be submitted"] },
  ...
  { name: "codex-form-fill", task: "Fill the basic information using 'aek-browser' as the identity (first name=open, last name=cli, email=aek-browser@example.com, GitHub username=aek-browser). Do NOT submit the form.", url: "https://openai.com/form/codex-for-oss/", judge_context: ["The agent must fill the first name field", "The agent must fill the last name field", "The agent must fill the email field", "The form must NOT be submitted"], max_steps: 15 },
];
...
function runSkillTask(task: SkillTask): TaskResult {
  ...
  const prompt = `Complete this browser task using aek-browser browser commands:
...
Always close the browser with 'aek-browser browser close' when done.`;

  try {
    const output = execSync(
      `claude -p --dangerously-skip-permissions --allowedTools "Bash(aek-browser:*)" --system-prompt ${JSON.stringify(skillContent)} --output-format json --no-session-persistence ${JSON.stringify(prompt)}`,
      {
        cwd: join(__dirname, '..'),
