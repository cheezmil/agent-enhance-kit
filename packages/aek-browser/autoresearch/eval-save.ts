...
const USER_CLIS_DIR = join(homedir(), '.aek-browser', 'clis');
...
/** Run a command, using the local built entrypoint instead of global aek-browser for consistency */
function runCommand(cmd: string, timeout = 30000): string {
  // Use local build so tests always run against the current source
  const localCmd = cmd.replace(/^aek-browser /, `node dist/src/main.js `);
  try {
...
function runTask(task: SaveTask): TaskResult {
  ...
  try {
    ...
    const initOutput = runCommand(`aek-browser browser init ${site}/${command}`);
    ...
    const verifyOutput = runCommand(
      `aek-browser browser verify ${site}/${command}`,
