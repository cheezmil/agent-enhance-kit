 ...
 * Uses a real GitHub plugin (aek-browser-plugin-hot-digest) to verify the full
...
const TEST_HOME = fs.mkdtempSync(path.join(os.tmpdir(), 'aek-browser-plugin-e2e-'));
const AEKB_HOME = path.join(TEST_HOME, '.aek-browser');
const PLUGINS_DIR = path.join(AEKB_HOME, 'plugins');
const PLUGIN_SOURCE = 'github:ByteYue/aek-browser-plugin-hot-digest';
const PLUGIN_NAME = 'hot-digest';
const PLUGIN_DIR = path.join(PLUGINS_DIR, PLUGIN_NAME);
const LOCK_FILE = path.join(AEKB_HOME, 'plugins.lock.json');
...
describe('plugin management E2E', () => {
  ...
  it('plugin install clones and sets up a real plugin', async () => {
    ...
    expect(lock[PLUGIN_NAME].source.url).toContain('aek-browser-plugin-hot-digest');
