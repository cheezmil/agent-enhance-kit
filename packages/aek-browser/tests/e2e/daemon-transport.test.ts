...
const HEADERS = { 'X-AEK Browser': '1', 'Content-Type': 'application/json' };
...
describe('daemon transport contracts (real daemon)', () => {
  ...
  beforeAll(async () => {
    if (await isPortBusy()) {
      skipReason = `Port ${PORT} is already in use; stop the local aek-browser daemon before running this suite`;
