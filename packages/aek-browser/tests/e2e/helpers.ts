 ...
 * Runs the built aek-browser binary as a subprocess.
 */
...
/**
 * Default stdout cap for `runCli`. `aek-browser list -f json` already weighs
...
/**
 * Run `aek-browser` as a child process with the given arguments.
 * Without PLAYWRIGHT_MCP_EXTENSION_TOKEN, aek-browser auto-launches its own browser.
 */
export async function runCli(
  ...
  try {
    const runtime = process.env.AEKB_TEST_RUNTIME || 'node';
