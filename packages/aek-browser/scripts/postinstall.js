...
const BASH_COMPLETION = `# Bash completion for aek-b (auto-installed)
_aek-b_completions() {
  local cur words cword
  _get_comp_words_by_ref -n : cur words cword

  local completions
  completions=$(aek-b --get-completions --cursor "$cword" "\${words[@]:1}" 2>/dev/null)

  COMPREPLY=( $(compgen -W "$completions" -- "$cur") )
  __ltrim_colon_completions "$cur"
}
complete -F _aek-b_completions aek-b
`;

const ZSH_COMPLETION = `#compdef aek-b
# Zsh completion for aek-b (auto-installed)
_aek-b() {
  local -a completions
  local cword=$((CURRENT - 1))
  completions=(\${(f)"$(aek-b --get-completions --cursor "$cword" "\${words[@]:1}" 2>/dev/null)"})
  compadd -a completions
}
_aek-b
`;

const FISH_COMPLETION = `# Fish completion for aek-b (auto-installed)
complete -c aek-b -f -a '(
  set -l tokens (commandline -cop)
  set -l cursor (count (commandline -cop))
  aek-b --get-completions --cursor $cursor $tokens[2..] 2>/dev/null
)'
`;
...
function main() {
  ...
  try {
    switch (shell) {
      case 'zsh': {
        ...
        const completionFile = join(completionsDir, '_aek-b');
      ...
      case 'bash': {
        ...
        const completionFile = join(userCompDir, 'aek-b');
      ...
      case 'fish': {
        ...
        const completionFile = join(completionsDir, 'aek-b.fish');
        ensureDir(completionsDir);
  ...
  if (!existsSync(spotifyEnvFile)) {
    ...
    console.log(`  Edit the file and add your Client ID and Secret, then run: aek-b spotify auth`);
  }
  ...
  console.log('  1. Download: https://github.com/cheezmil/aek-b/releases');
  console.log('  2. In Chrome or Chromium, open chrome://extensions → enable Developer Mode → Load unpacked');
  console.log('');
  console.log('  Then run \x1b[36maek-b doctor\x1b[0m to verify.');
  console.log('');
