...
export function fetchAdapters() {
  ...
  // Skip if already installed at the same version (unless forced via AEKB_FETCH=1)
  const isForced = process.env.AEKB_FETCH === '1';
  ...
  let tmpCleaned = 0;
  try {
    for (const entry of readdirSync(AEKB_DIR)) {
      if (entry.startsWith('.plugins.lock.json.tmp-')) {
        try {
          unlinkSync(join(AEKB_DIR, entry));
          tmpCleaned++;
        } catch { /* ignore */ }
      }
    }
...
function main() {
  ...
  const isExplicit = process.env.AEKB_FETCH === '1';
  const isFirstRun = process.env._AEKB_FIRST_RUN === '1';
