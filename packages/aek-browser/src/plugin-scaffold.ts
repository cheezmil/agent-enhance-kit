 ...
 *
 * Usage: aekb plugin create <name> [--dir <path>]
 *
 * Creates:
 *   <name>/
 *     aekb-plugin.json   — manifest with name, version, description
 *     package.json          — ESM package with aekb peer dependency
...
export function createPluginScaffold(name: string, opts: ScaffoldOptions = {}): ScaffoldResult {
  ...
  const manifest = {
    name,
    version: '0.1.0',
    description: opts.description ?? `An aekb plugin: ${name}`,
    aekb: `>=${PKG_VERSION}`,
  };
  writeFile(targetDir, 'aekb-plugin.json', JSON.stringify(manifest, null, 2) + '\n');
  files.push('aekb-plugin.json');

  // package.json
  const pkg = {
    name: `aekb-plugin-${name}`,
    version: '0.1.0',
    type: 'module',
    description: opts.description ?? `An aekb plugin: ${name}`,
    peerDependencies: {
      '@cheezmil/aek-browser': `>=${PKG_VERSION}`,
    },
  };
...
`;
  ...
  // README.md
  const readme = `# aekb-plugin-${name}

${opts.description ?? `An aekb plugin: ${name}`}

## Install

\`\`\`bash
# From local development directory
aekb plugin install file://${targetDir}

# From GitHub (after publishing)
aekb plugin install github:<user>/aekb-plugin-${name}
\`\`\`
