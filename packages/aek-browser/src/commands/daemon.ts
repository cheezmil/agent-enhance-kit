 ...
 *   AEK Browser daemon status — show daemon state
 *   AEK Browser daemon stop   — graceful shutdown
 *   AEK Browser daemon restart — graceful shutdown, then start a fresh daemon
 */
...
export async function daemonStatus(): Promise<void> {
  ...
  if (status.extensionConnected) {
    ...
    extensionLabel = `${count} ${count === 1 ? 'profile' : 'profiles'} connected, none selected — run \`aekb profile use <name>\``;
  } else if (status.profileDisconnected) {
    extensionLabel = 'requested profile not connected — run `aekb profile use <name>`';
  } else {
  ...
  console.log(`Version: ${daemonVersion}${stale ? ` (CLI v${PKG_VERSION}; run: AEK Browser daemon restart)` : ''}`);
