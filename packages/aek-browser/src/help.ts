...
export function formatRootAdapterHelpText(groups: RootAdapterGroups): string {
  ...
  lines.push("Run 'aekb list' for full command details, or 'aekb <site> --help' to inspect one site.");
  lines.push("Agent tip: use 'aekb <site> --help -f yaml' for all command args/options in one structured response.");
  lines.push('');
  return lines.join('\n');
}
...
function formatUsage(cmd: CliCommand): string {
  const positionalText = formatPositionals(positionals(cmd));
  return `aekb ${cmd.site} ${cmd.name}${positionalText ? ` ${positionalText}` : ''} [options]`;
}

function compactCommand(cmd: CliCommand): Record<string, unknown> {
  return {
    name: cmd.name,
    command: `aekb ${cmd.site} ${cmd.name}`,
...
export function rootHelpData(program: Command, groups: RootAdapterGroups): Record<string, unknown> {
  ...
  return {
    ...
    next: [
      'aekb <site> --help -f yaml',
      'aekb list -f yaml',
      'aekb <site> <command> -f yaml',
    ],
  };
}
...
export function formatSiteHelpText(site: string, commands: readonly CliCommand[]): string {
  const unique = [...new Map(commands.map(cmd => [fullName(cmd), cmd])).values()]
    .sort((a, b) => a.name.localeCompare(b.name));
  const lines: string[] = [
    `Usage: aekb ${site} <command> [args] [options]`,
    '',
    ...
    '',
    `Agent tip: use 'aekb ${site} --help -f yaml' to get all command args/options in one structured response.`,
    '',
  ];
  return lines.join('\n');
}
