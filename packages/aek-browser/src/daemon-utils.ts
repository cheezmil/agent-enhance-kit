...
export const PROFILE_DISCONNECTED_HINT =
  'Open that Chrome profile and make sure the AEK Browser extension is enabled, or choose another profile with aekb profile use <name>.';
...
export type ProfileRouteInput = {
  /** Hard requirement (--profile / AEKB_PROFILE) — never falls back. */
 ...
 * connected profile, AEK Browser uses it automatically" true even when a persisted
...
export function resolveProfileRoute(input: ProfileRouteInput): ProfileRouteResult {
  ...
  if (connected.length > 1) {
    return {
      ...
      errorHint: preferred
        ? 'Run aekb profile list, then update the stale default with aekb profile use <name> or pass --profile <name>.'
        : 'Run aekb profile list, then use aekb --profile <name> ... or aekb profile use <name>.',
    };
  }
  ...
  return {
    ...
    error: 'Extension not connected. Please install the aekb Browser Bridge extension.',
  };
}
