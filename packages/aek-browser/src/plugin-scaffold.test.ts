...
describe('createPluginScaffold', () => {
  ...
  it('creates all expected files', () => {
    const dir = path.join(os.tmpdir(), `aekb-scaffold-${Date.now()}`);
    createdDirs.push(dir);

    const result = createPluginScaffold('my-test', { dir });
    expect(result.name).toBe('my-test');
    expect(result.dir).toBe(dir);
    expect(result.files).toContain('aekb-plugin.json');
  ...
  it('generates ESM package.json', () => {
    ...
    expect(pkg.peerDependencies?.['@cheezmil/aek-browser']).toBeDefined();
  });
  ...
  it('generates a TS sample that matches the current plugin API', () => {
    ...
    expect(tsSample).toContain(`import { cli, Strategy } from '@cheezmil/aek-browser/registry';`);
  ...
  it('documents a supported local install flow', () => {
    ...
    expect(readme).toContain(`aekb plugin install file://${dir}`);
  });
