...
export interface PluginInfo {
  ...
  monorepoName?: string;
  /** Description from aekb-plugin.json. */
  description?: string;
}
...
function cloneRepoToTemp(cloneUrl: string): string {
  const tmpCloneDir = path.join(
    os.tmpdir(),
    `aekb-clone-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}`,
  );
...
export function validatePluginStructure(pluginDir: string): ValidationResult {
  ...
  if (hasTs) {
    ...
    if (!fs.existsSync(pkgJsonPath)) {
      errors.push('Plugin contains .ts files but no package.json. A package.json with "type": "module" and "@cheezmil/aek-browser" peer dependency is required for TS plugins.');
    } else {
      try {
...
function finalizePluginRuntime(pluginDir: string): void {
  // Symlink host AEK Browser so TS plugins resolve '@cheezmil/aek-browser/registry'
...
export function installPlugin(source: string): string | string[] {
  ...
  return withTempClone(parsed.cloneUrl!, (tmpCloneDir) => {
    ...
    if (manifest?.aekb && !checkCompatibility(manifest.aekb)) {
      throw new Error(
        `Plugin requires AEK Browser ${manifest.aekb}, but current version is incompatible.`
      );
    }
...
function installSinglePlugin(
  ...
  if (fs.existsSync(targetDir)) {
    throw new PluginError(`Plugin "${pluginName}" is already installed at ${targetDir}`, 'Use "aekb plugin uninstall" first, or pick a different name.');
  }
...
function installLocalPlugin(localPath: string, name: string): string {
  ...
  if (manifest?.aekb && !checkCompatibility(manifest.aekb)) {
    throw new PluginError(
      `Plugin requires AEK Browser ${manifest.aekb}, but current version is incompatible.`,
      'Upgrade AEK Browser to a compatible version.',
    );
  }
...
function installMonorepo(
  ...
  for (const { name, entry } of pluginsToInstall) {
    ...
    if (entry.aekb && !checkCompatibility(entry.aekb)) {
      log.warn(`Skipping "${name}": requires AEK Browser ${entry.aekb}`);
      continue;
    }
...
function collectUpdatedMonorepoPlugins(
  ...
  for (const [pluginName, entry] of Object.entries(lock)) {
    ...
    if (manifestEntry.aekb && !checkCompatibility(manifestEntry.aekb)) {
      throw new Error(`Sub-plugin "${pluginName}" requires AEK Browser ${manifestEntry.aekb}`);
    }
 ...
 * Reads aekb-plugin.json for description/version when available.
 */
...
function parseSource(
  ...
  if (source.startsWith('file://')) {
    try {
      ...
      return {
        type: 'local',
        localPath,
        name: path.basename(localPath).replace(/^aekb-plugin-/, ''),
      };
    } catch {
      return null;
    }
  ...
  if (githubSubMatch) {
    ...
    const name = repo.replace(/^aekb-plugin-/, '');
    return {
      type: 'git',
  ...
  if (sshUrlMatch) {
    ...
    const name = repoSegment.replace(/^aekb-plugin-/, '');
...
/**
 * Symlink the host AEK Browser package into a plugin's node_modules.
 * This ensures TS plugins resolve '@cheezmil/aek-browser/registry' against
...
function linkHostOpencli(pluginDir: string): void {
  try {
    ...
    const targetLink = path.join(pluginDir, 'node_modules', '@cheezmil', 'aekb');
    ...
    log.debug(`Linked host aekb into plugin: ${targetLink} → ${hostRoot}`);
  } catch (err) {
    log.warn(`Failed to link host aekb into plugin: ${getErrorMessage(err)}`);
  }
}
...
function resolveHostOpencliRoot(startFile = fileURLToPath(import.meta.url)): string {
  ...
  while (true) {
    ...
    if (fs.existsSync(pkgPath)) {
      try {
        const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf-8'));
        if (pkg?.name === '@cheezmil/aek-browser') {
          return dir;
        }
      } catch {
 ...
 * Uses esbuild from the host AEK Browser's node_modules for fast single-file transpilation.
 */
  ...
  try {
    ...
    if (!esbuildBin) {
      log.warn(
        ...
        'Install esbuild (`npm i -g esbuild`) or ensure it is available in the aekb host node_modules.'
      );
      return;
    }
