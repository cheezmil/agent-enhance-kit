  ...
  try {
    if (siteSession !== null) {
      ...
      if (electron) {
        ...
        const manualEndpoint = process.env.AEKB_CDP_ENDPOINT;
        if (manualEndpoint) {
      ...
      // Requirement vs preference: --profile / AEKB_PROFILE route strictly;
...
function exportTraceArtifact(
  ...
  try {
    ...
    if (status === 'failure' && error !== undefined) {
      ...
      process.stderr.write(`AEK Browser trace artifact: ${trace.dir}\n`);
    }
    try {
      onTraceExport?.(trace);
    } catch (err) {
...
function resolveSiteSession(cmd: CliCommand, rawOption?: unknown): SiteSessionMode {
  return normalizeSiteSession('--site-session', rawOption)
    ?? normalizeSiteSession('AEKB_SITE_SESSION', process.env.AEKB_SITE_SESSION)
    ?? cmd.siteSession
    ?? 'ephemeral';
}
...
function resolveBrowserWindowMode(defaultMode: BrowserWindowMode = 'background', rawOption?: unknown): BrowserWindowMode {
  return normalizeWindowMode('--window', rawOption)
    ?? normalizeWindowMode('AEKB_WINDOW', process.env.AEKB_WINDOW)
    ?? defaultMode;
}
