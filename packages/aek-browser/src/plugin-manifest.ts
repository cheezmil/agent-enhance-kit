/**
 * Plugin manifest: reads and validates aekb-plugin.json files.
 *
 * Supports two modes:
...
export interface SubPluginEntry {
  ...
  version?: string;
  description?: string;
  /** Semver range for AEK Browser compatibility (overrides top-level). */
  aekb?: string;
...
export interface PluginManifest {
  ...
  version?: string;
  /** Semver range for AEK Browser compatibility, e.g. ">=1.0.0". */
  aekb?: string;
...
export const MANIFEST_FILENAME = 'aekb-plugin.json';

// ── Read / Validate ─────────────────────────────────────────────────────────

/**
 * Read and parse aekb-plugin.json from a directory.
...
/**
 * Check if the current aekb version satisfies a semver range string.
 *
