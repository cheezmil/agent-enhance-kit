...
const NPM_REGISTRY_URL = 'https://registry.npmjs.org/@cheezmil/aek-browser/latest';
const GITHUB_RELEASES_URL = 'https://api.github.com/repos/cheezmil/aekb/releases?per_page=20';
...
function buildUpdateNotices({ cliVersion, cache, now }: NoticeInputs): NoticeLines {
  ...
  if (cache.latestVersion && isNewer(cache.latestVersion, cliVersion)) {
    lines.cli =
      ...
      `  Run: npm install -g @cheezmil/aek-browser\n`;
  }
  ...
  if (
    ...
    lines.extension =
      ...
      `  Download: https://github.com/cheezmil/aekb/releases\n`;
  }
  return lines;
}
...
function extractLatestExtensionVersionFromReleases(releases: GitHubRelease[]): string | undefined {
  for (const release of releases) {
    for (const asset of release.assets ?? []) {
      const assetMatch = asset.name.match(/^aekb-extension-v(.+)\.zip$/);
...
async function fetchLatestExtensionVersion(): Promise<string | undefined> {
  try {
    ...
    const res = await fetch(GITHUB_RELEASES_URL, {
      ...
      headers: { 'User-Agent': `aekb/${PKG_VERSION}`, Accept: 'application/vnd.github+json' },
    });
    clearTimeout(timer);
  ...
  void (async () => {
    try {
      ...
      const res = await fetch(NPM_REGISTRY_URL, {
        signal: controller.signal,
        headers: { 'User-Agent': `aekb/${PKG_VERSION}` },
      });
      clearTimeout(timer);
      if (!res.ok) return;
 ...
 * Used by `aekb doctor` to report extension updates.
 */
