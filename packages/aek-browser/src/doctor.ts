/**
 * aekb doctor — diagnose browser connectivity.
 *
...
export async function runBrowserDoctor(opts: DoctorOptions = {}): Promise<DoctorReport> {
  ...
  if (daemonFlaky) {
    ...
    issues.push('Daemon is not running. It should start automatically when you run an aekb command.');
  }
  ...
  if (extensionFlaky) {
    ...
    if (health.state === 'profile-required') {
      issues.push(
        ...
        '  Run aekb profile list, then aekb profile use <name>, or pass --profile <name>.',
      );
    } else if (health.state === 'profile-disconnected') {
      issues.push(
        `Selected browser profile is not connected: ${health.status?.contextId ?? 'unknown'}.\n` +
        '  Open that Chrome profile and make sure the AEK Browser extension is enabled.',
      );
    } else {
      issues.push(
        'Daemon is running but the Chrome/Chromium extension is not connected.\n' +
        'If the extension is already installed, try: aekb daemon restart\n' +
        'If the extension is not installed:\n' +
        '  1. Download from https://github.com/cheezmil/aekb/releases\n' +
  ...
  if (extensionConnected && !extensionVersion) {
    issues.push(
      ...
      '  Reload or reinstall the extension from: https://github.com/cheezmil/aekb/releases',
    );
  }
  if (!connectivity.ok) {
  ...
  if (staleDefault && profiles?.length && !profiles.some((p) => p.contextId === staleDefault)) {
    ...
    issues.push(
      ...
      `  ${fallbackNote}\n` +
      '  Refresh it with: aekb profile list, then aekb profile use <name>.',
    );
  }
  ...
  if (extensionVersion && opts.cliVersion && extensionCompatRange) {
    if (!satisfiesRange(opts.cliVersion, extensionCompatRange)) {
      issues.push(
        ...
        '  Update the CLI: npm install -g @cheezmil/aek-browser\n' +
        '  Or download a compatible extension from: https://github.com/cheezmil/aekb/releases',
      );
    }
    ...
    if (extMajor !== cliMajor) {
      issues.push(
        ...
        '  Download the latest extension from: https://github.com/cheezmil/aekb/releases',
      );
    }
  }
  ...
  if (extensionVersion && latestExtensionVersion && isNewerVersion(latestExtensionVersion, extensionVersion)) {
    issues.push(
      ...
      '  Download from: https://github.com/cheezmil/aekb/releases',
    );
  }
...
export function renderBrowserDoctorReport(report: DoctorReport): string {
  const lines = [`aekb v${report.cliVersion ?? 'unknown'} doctor` + ` (${getRuntimeLabel()})`, ''];
