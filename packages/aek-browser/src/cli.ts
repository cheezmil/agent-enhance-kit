 ...
 * the JS interceptor's `window.__aekb_net`) into a consistent shape.
...
async function captureNetworkItems(page: import('./types.js').IPage): Promise<BrowserNetworkItem[]> {
  ...
  const raw = await page.evaluate(`(function(){ var out = window.__aekb_net || []; window.__aekb_net = []; return JSON.stringify(out); })()`) as string;
  try {
    ...
    if (process.env.AEKB_VERBOSE) log.warn(`[network] Failed to parse interceptor buffer: ${typeof raw === 'string' ? raw.slice(0, 200) : String(raw)}`);
    return [];
  }
}
...
const SITEMAP_HINT =
  'Site sitemap available. For navigation context, use the aekb-browser-sitemap skill; treat browser state as truth if it disagrees.';
...
async function resolveBrowserTargetInSession(
  ...
  try {
    ...
    throw new Error(
      ...
      'The Browser Bridge session may have restarted; re-run "aekb tab list" and choose a current target.',
      { cause: err },
    );
  }
...
async function getBrowserPage(
  ...
  const envTimeout = process.env.AEKB_BROWSER_IDLE_TIMEOUT;
...
function getBrowserWindowMode(command: Command | undefined, defaultMode: BrowserWindowMode): BrowserWindowMode {
  ...
  const envRaw = process.env.AEKB_WINDOW;
  if (envRaw !== undefined && envRaw !== '') {
    if (envRaw === 'foreground' || envRaw === 'background') return envRaw;
    throw new Error(`AEKB_WINDOW must be one of: foreground, background. Received: "${envRaw}"`);
  }
  return defaultMode;
}
...
function getBrowserSession(command?: Command): string {
  // The CLI surface is `aekb browser <session> <subcommand>`. main.ts rewrites
  ...
  throw new Error('<session> is a required positional argument: aekb <session> <command>');
}
...
function applyVerbose(opts: { verbose?: boolean }): void {
  if (opts.verbose) process.env.AEKB_VERBOSE = '1';
}
...
export function createProgram(BUILTIN_CLIS: string, USER_CLIS: string): Command {
  ...
  program
    ...
    .action((opts) => {
      ...
      console.log();
      console.log('  aek-b' + ' — available commands');
      console.log();
  ...
  const skillsCmd = program
    .command('skills')
    .description('Read bundled AEK Browser skills');

  skillsCmd
    .command('list')
    .description('List bundled aekb-* skills')
    .option('-f, --format <fmt>', 'Output format: table, json, yaml, md, csv', 'table')
    .action((opts) => {
      const rows = listAekbSkills();
      renderOutput(rows, {
        fmt: opts.format,
        fmtExplicit: !!opts.format,
        columns: ['name', 'description', 'version', 'path'],
        title: 'aekb/skills/list',
        source: 'aekb skills list',
      });
    });

  skillsCmd
    .command('read')
    .description("Print an aekb-* skill's SKILL.md or reference file")
    .argument('<skill>', 'Skill name, or skill/path like aekb-browser/references/foo.md')
    ...
    .action((skill: string, skillPath: string | undefined, opts) => {
      let result: ReturnType<typeof readAekbSkill>;
      try {
        result = readAekbSkill(skill, skillPath ?? '');
      } catch (err) {
...
You can omit the \`browser\`/\`b\` keyword — \`aekb <session> <cmd>\` is an alias for \`aekb browser <session> <cmd>\`.

Examples:
  $ aekb work open https://x.com
  $ aekb browser work open https://x.com
  $ aekb work open https://x.com --window background
  $ aekb work click 12
  $ aekb work state
  $ aekb work bind
  $ aekb work unbind
`);
  ...
  function emitJavaScriptDialogError(message: string): void {
    console.log(JSON.stringify({
      error: {
        ...
        message,
        hint: 'Handle the modal first: aekb dialog accept (or dismiss). Use --text for prompt dialogs.',
      },
    }, null, 2));
  }
  ...
  browser.command('bind')
    .description('Take over the active Chrome tab as this browser session target; subsequent commands operate on that tab')
    .action(browserSessionCommandAction(async ({ session, contextId }) => {
      ...
      console.log(JSON.stringify({ session, bound: true, ...((data && typeof data === 'object') ? data as Record<string, unknown> : { data }) }, null, 2));
    }));
      ...
      if ((!opts.css || typeof opts.css !== 'string') && !locator) {
        console.log(JSON.stringify({
          error: {
            ...
            hint: 'Examples: aekb find --css ".btn.primary"; aekb find --role button --name Save',
          },
        }, null, 2));
        process.exitCode = EXIT_CODES.USAGE_ERROR;
        return;
  ...
  addBrowserTabOption(
    ...
    .action(browserAction(async (page, opts) => {
      ...
      if (max > 0 && html.length > max) {
        console.log(`<!-- aekb: truncated ${max} of ${html.length} chars; re-run without --max (or --max 0) for full -->\n${html.slice(0, max)}`);
        return;
      }
      console.log(html);
    }));
    ...
    if (inputs.length === 0) {
      return {
        error: {
          ...
          hint: 'Example: aekb upload "input[type=file]" ./receipt.pdf',
        },
      };
    }
    const files: string[] = [];
  ...
  browser.command('init')
    ...
    .action(async (name: string) => {
      try {
        ...
        const template = `import { cli, Strategy } from '@cheezmil/aek-browser/registry';
...
cli({
  ...
  example: 'aekb ${site} ${command} -f yaml',
  domain: '${domain}',
...
`;
        ...
        console.log('First time on this site? Run: aekb analyze <url>');
        console.log(`Edit the file to implement your adapter, then run: aekb verify ${name}`);
      } catch (err) {
    ...
    .action(async (name: string, opts: { fixture?: boolean; writeFixture?: boolean; updateFixture?: boolean; strictMemory?: boolean; seedArgs?: string; trace?: string } = {}) => {
      try {
        ...
        try {
          ...
          console.log(`  Executing: aekb ${site} ${command} ${argDisplay}\n`);
  ...
  program
    .command('doctor')
    .description('Diagnose aekb browser bridge connectivity')
  ...
  const pluginCmd = program.command('plugin').description('Manage aekb plugins');
  ...
  pluginCmd
    ...
    .action(async (opts) => {
      ...
      if (plugins.length === 0) {
        console.log('  No plugins installed.');
        console.log('  Install one with: aekb plugin install github:user/repo');
        return;
      }
      if (opts.format === 'json') {
  ...
  pluginCmd
    ...
    .action(async (name: string, opts: { dir?: string; description?: string }) => {
      ...
      try {
        ...
        console.log(`    aekb plugin install file://${result.dir}`);
        console.log(`    aekb ${name} hello`);
      } catch (err) {
        console.error(`Error: ${getErrorMessage(err)}`);
        process.exitCode = EXIT_CODES.GENERIC_ERROR;
      }
  ...
  profileCmd
    ...
    .action(async () => {
      ...
      if (!status) {
        console.log('Daemon is not running. Run aekb doctor after opening Chrome.');
        return;
      }
      if (isDaemonStale(status, PKG_VERSION) || !Array.isArray(status.profiles)) {
        ...
        console.log('Run: aekb daemon restart');
        return;
      }
      if (profiles.length === 0) {
        console.log('No Browser Bridge profiles connected.');
        console.log('Open a Chrome profile with the AEK Browser extension installed, then run aekb profile list again.');
        return;
      }
  ...
  profileCmd
    ...
    .argument('<contextId>', 'Profile contextId from aekb profile list')
  ...
  // ── Built-in: daemon ──────────────────────────────────────────────────────
  const daemonCmd = program.command('daemon').description('Manage the aekb daemon');
  // Snapshot before applyRootSubcommandSummaries() rewrites .description() to a child-name listing.
  ...
  externalCmd
    ...
    .action((opts) => {
      ...
      renderOutput(rows, {
        ...
        title: 'aekb/external/list',
        source: 'aekb external list',
      });
    });
  ...
  // line is accurate: `Usage: aekb browser <session> click [target] [options]`
  // instead of `aekb browser click [target] [options]`. Commander does NOT
  ...
  program.on('command:*', (operands: string[]) => {
    ...
    if (isBinaryInstalled(binary)) {
      console.error(`  Tip: '${binary}' exists on your PATH. Use 'aekb external register ${binary}' to add it as an external CLI.`);
    }
    program.outputHelp();
    process.exitCode = EXIT_CODES.USAGE_ERROR;
  });
...
export function resolveBrowserVerifyInvocation(opts: {
  ...
  if (!fileExists(sourceEntry)) {
    throw new Error(`Could not find aekb entrypoint under ${projectRoot}. Expected built entry from package.json or src/main.ts.`);
  }
