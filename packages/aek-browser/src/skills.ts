...
export interface AekbSkillInfo {
  name: string;
  description: string;
  version: string;
  path: string;
...
export interface AekbSkillReadResult {
  skill: string;
  path: string;
  content: string;
}
...
export function listAekbSkills(packageRoot?: string): AekbSkillInfo[] {
  const skillsRoot = getSkillsRoot(packageRoot);
  if (!fs.existsSync(skillsRoot)) return [];

  return fs.readdirSync(skillsRoot, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && entry.name.startsWith('aekb-'))
    .map((entry) => readSkillInfo(skillsRoot, entry.name))
    .filter((entry): entry is AekbSkillInfo => entry !== null)
    .sort((a, b) => a.name.localeCompare(b.name));
}

export function readAekbSkill(target: string, relpath = '', packageRoot?: string): AekbSkillReadResult {
  const { name, pathInSkill } = parseSkillTarget(target, relpath);
  if (!name.startsWith('aekb-')) {
    throw new ArgumentError(`Unknown AEK Browser skill: ${name}`, 'Run "aekb skills list" to see available AEK Browser skills.');
  }
  ...
  if (relativeToRoot.startsWith('..') || path.isAbsolute(relativeToRoot)) {
    throw new ArgumentError(`Invalid skill path: ${relativePath}`, 'Skill paths must stay inside the selected AEK Browser skill.');
  }
  if (!fs.existsSync(absolutePath) || !fs.statSync(absolutePath).isFile()) {
    throw new ArgumentError(`Skill file not found: ${name}/${relativePath}`, 'Run "aekb skills list <skill>" is not supported yet; read SKILL.md or a known references/... file.');
  }
...
function readSkillInfo(skillsRoot: string, name: string): AekbSkillInfo | null {
...
function normalizeSkillPath(raw: string): string {
  ...
  if (normalized.startsWith('/') || normalized.split('/').some((part) => part === '..')) {
    throw new ArgumentError(`Invalid skill path: ${raw}`, 'Use a path relative to an AEK Browser skill directory.');
  }
