/**
 * aekb micro-daemon — HTTP + WebSocket bridge between CLI and Chrome Extension.
 *
 * Architecture:
 ...
 *   2. Custom header — require X-AEK Browser header (browsers can't send it
 ...
 *
 * Lifecycle:
 *   - Auto-spawned by aekb on first browser command
...
if (!isIgnorableDaemonPortEnv(process.env.AEKB_DAEMON_PORT)) {
  log.error(unsupportedDaemonPortEnvMessage(process.env.AEKB_DAEMON_PORT));
...
function resolveExtensionConnection(contextId?: string, preferredContextId?: string): {
  ...
  if (route.fallbackFrom && !staleDefaultWarned.has(route.fallbackFrom)) {
    ...
    log.warn(
      ...
      `using the only connected profile "${route.contextId}". Update the default with: aekb profile use <name>`,
    );
  }
...
async function handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  // Health-check endpoint — no X-AEK Browser header required.
  ...
  if (!req.headers['x-aekb']) {
    jsonResponse(res, 403, { ok: false, error: 'Forbidden: missing X-AEK Browser header' });
    return;
  }
...
// Listen on all interfaces (0.0.0.0) instead of just 127.0.0.1 so that
// Windows browsers (Edge/Chrome) can reach the daemon through WSL's NAT.
