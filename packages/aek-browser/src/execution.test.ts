...
describe('executeCommand — non-browser timeout', () => {
  ...
  it.each([
    ...
    vi.stubEnv('AEKB_SITE_SESSION', env);
    const cmd = cli({
      site: 'test-execution',
  ...
  it.each(['', ' ', 'Persistent'])('rejects invalid AEKB_SITE_SESSION=%j before hooks or browser setup', async (env) => {
  ...
  it('does not apply AEKB_SITE_SESSION to non-browser commands', async () => {
    vi.stubEnv('AEKB_SITE_SESSION', 'invalid');
    const cmd = cli({
      site: 'test-execution',
  ...
  it('lets env ephemeral override persistent metadata and closes the window on failure', async () => {
    ...
    vi.stubEnv('AEKB_SITE_SESSION', 'ephemeral');
  ...
  it('exports a profile-scoped trace artifact on browser command failure when requested', async () => {
    const baseDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aekb-exec-trace-'));
    const prevConfigDir = process.env.AEKB_CONFIG_DIR;
    process.env.AEKB_CONFIG_DIR = baseDir;
    ...
    try {
      ...
      expect(stderrSpy.mock.calls.flat().join('\n')).not.toContain('___AEKB_TRACE___');
      ...
      if (prevConfigDir === undefined) delete process.env.AEKB_CONFIG_DIR;
      else process.env.AEKB_CONFIG_DIR = prevConfigDir;
  ...
  it('exports a trace receipt on browser command success when trace is on', async () => {
    const baseDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aekb-exec-trace-success-'));
    const prevConfigDir = process.env.AEKB_CONFIG_DIR;
    process.env.AEKB_CONFIG_DIR = baseDir;
    const stderrSpy = vi.spyOn(process.stderr, 'write').mockImplementation(() => true);
    ...
    try {
      ...
      expect(stderr).toContain('AEK Browser trace artifact:');
      const tracesRoot = path.join(baseDir, 'profiles', 'default', 'traces');
      const traceId = fs.readdirSync(tracesRoot)[0];
  ...
  it('keeps the original adapter error when trace export fails', async () => {
    const baseDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aekb-exec-trace-fail-'));
    const blockedPath = path.join(baseDir, 'not-a-dir');
    fs.writeFileSync(blockedPath, 'file');
    const prevConfigDir = process.env.AEKB_CONFIG_DIR;
    process.env.AEKB_CONFIG_DIR = blockedPath;
    const stderrSpy = vi.spyOn(process.stderr, 'write').mockImplementation(() => true);
    const mockPage = {
