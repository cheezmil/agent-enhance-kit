...
export async function ensureUserCliCompatShims(baseDir: string = USER_AEKB_DIR): Promise<void> {
  ...
  // Create node_modules/@cheezmil/aek-browser symlink pointing to the installed package root.
  const aekbRoot = PACKAGE_ROOT;
  const symlinkDir = path.join(baseDir, 'node_modules', '@cheezmil');
  const symlinkPath = path.join(symlinkDir, 'aek-browser');
  try {
    let needsUpdate = true;
    try {
      const existing = await fs.promises.readlink(symlinkPath);
      if (existing === aekbRoot) needsUpdate = false;
    } catch { /* doesn't exist */ }
    if (needsUpdate) {
      ...
      await fs.promises.symlink(aekbRoot, symlinkPath, symlinkType);
    }
  } catch (err) {
  ...
  await Promise.all(files.map(async (file) => {
    ...
    if (file.endsWith('.js') && !file.endsWith('.d.js')) {
      ...
      log.warn(
        ...
        `Run "aekb plugin update ${site}" to re-transpile, or install esbuild.`
      );
    }
  }));
}
