/**
 * Unified error types for AEK Browser.
 *
 ...
 * ## Exit codes
 *
 * AEK Browser follows Unix conventions (sysexits.h) for process exit codes:
 *
 *   0   Success
...
const TRACE_RECEIPT_SYMBOL = Symbol.for('aekb.traceReceipt');
...
export class TimeoutError extends CliError {
  constructor(label: string, seconds: number, hint?: string) {
    super(
      ...
      hint ?? 'Try again, or increase timeout with --timeout <seconds> (or AEKB_BROWSER_COMMAND_TIMEOUT for the global default)',
      EXIT_CODES.TEMPFAIL,
    );
  }
}
...
export function toEnvelope(err: unknown): ErrorEnvelope {
  ...
  // a different copy of @cheezmil/aek-browser (e.g. a plugin with its own
