 ...
 *
 * Inspired by browser-use's multi-layer pruning pipeline, adapted for AEK Browser's
 ...
 *
 * Additional tools:
 *   - scrollToRefJs(ref) — scroll to a data-aekb-ref element
...
export interface DomSnapshotOptions {
  ...
  /** Annotate interactive elements with data-aekb-ref (default true) */
...
/**
 * Generate JS to scroll to an element identified by data-aekb-ref.
...
export function scrollToRefJs(ref: string): string {
  ...
  return `
    (() => {
      const ref = ${safeRef};
      const el = document.querySelector('[data-aekb-ref="' + ref + '"]')
  ...
  return `
    (() => {
      ...
      function extractField(el) {
        ...
        const ref = el.getAttribute('data-aekb-ref') || null;
...
(() => {
  ...
  function walk(el, depth, parentPropagatingRect) {
    ...
    if (tag === 'svg') {
      ...
      if (interactive) {
        interactiveIndex++;
        if (ANNOTATE_REFS) el.setAttribute('data-aekb-ref', '' + interactiveIndex);
  ...
  function walkIframe(el, depth) {
    ...
    try {
      ...
      if (!doc || !doc.body) {
        ...
        lines.push(indent + '|iframe|' + frameLabel + '<iframe' + (attrs ? ' ' + attrs : '') + ' /> (cross-origin, use: AEK Browser browser frames + browser eval --frame <index>)');
        crossOriginIndex++;
        return false;
      }
      iframeCount++;
      ...
      const attrs = serializeAttrs(el);
      const frameLabel = '[F' + crossOriginIndex + ']';
      lines.push(indent + '|iframe|' + frameLabel + '<iframe' + (attrs ? ' ' + attrs : '') + ' /> (blocked, use: AEK Browser browser frames + browser eval --frame <index>)');
      crossOriginIndex++;
      return false;
    }
  }
  ...
  try { window.__aekb_prev_hashes = JSON.stringify(currentHashes); } catch {}
  // Store ref identity map for stale-ref detection by target resolver
  try { window.__aekb_ref_identity = refIdentity; } catch {}
