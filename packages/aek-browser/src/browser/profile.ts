...
export type ProfileSelection = {
   ...
   * or AEKB_PROFILE env): route strictly and fail loud if it is offline.
...
export function resolveProfileSelection(profile?: string): ProfileSelection | undefined {
  ...
  const explicit = normalizeContextId(profile) ?? normalizeContextId(process.env.AEKB_PROFILE);
