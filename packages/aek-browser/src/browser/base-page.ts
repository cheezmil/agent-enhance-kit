...
export abstract class BasePage implements IPage {
  ...
  protected async tryCdpOnResolvedElement(method: 'DOM.focus' | 'DOM.scrollIntoViewIfNeeded'): Promise<boolean> {
    ...
    const markerAttr = 'data-aekb-cdp-target';
  ...
  async setChecked(ref: string, checked: boolean, opts: ResolveOptions = {}): Promise<SetCheckedResult> {
    ...
    if (before?.ok !== true) {
      throw new TargetError({
        ...
        hint: 'Use `aekb state` or `browser find` to pick an input[type=checkbox], input[type=radio], or role=checkbox/switch target.',
      });
    }
    if (before.disabled) {
      throw new TargetError({
  ...
  async uploadFiles(ref: string, files: string[], opts: ResolveOptions = {}): Promise<UploadFilesResult> {
    ...
    const resolved = await runResolve(this, ref, opts);
    const markerAttr = 'data-aekb-upload-target';
    ...
    try {
      ...
      if (!marked) {
        throw new TargetError({
          ...
          hint: 'Use `aekb find --css "input[type=file]"` or inspect `compound` output from state/find.',
        });
      }
  ...
  async drag(
    ...
    try {
      ...
      const endpoints = await this.evaluate(`
        (() => {
          const sourceEl = window.__aekb_drag_source;
      ...
      await this.evaluate('delete window.__aekb_drag_source').catch(() => {});
    }
  }
  ...
  async fillText(ref: string, text: string, opts: ResolveOptions = {}): Promise<FillTextResult> {
    ...
    if (preparation?.ok !== true) {
      throw new TargetError({
        ...
        hint: 'Use `aekb state` to pick an editable target, or use `browser type` for keyboard-like interactions.',
      });
    }
  ...
  async snapshot(opts: SnapshotOptions = {}): Promise<unknown> {
    ...
    try {
      ...
      try {
        const hashes = await this.evaluate('window.__aekb_prev_hashes') as string | null;
  ...
  async installInterceptor(pattern: string): Promise<void> {
    ...
    await this.evaluate(generateInterceptorJs(JSON.stringify(pattern), {
      arrayName: '__aekb_xhr',
      patchGuard: '__aekb_interceptor_patched',
    }));
  }

  async getInterceptedRequests(): Promise<unknown[]> {
    const { generateReadInterceptedJs } = await import('../interceptor.js');
    const result = await this.evaluate(generateReadInterceptedJs('__aekb_xhr'));
    return Array.isArray(result) ? result : [];
  }
