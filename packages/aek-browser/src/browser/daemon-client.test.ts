...
describe('daemon-client', () => {
  ...
  it('fetchDaemonStatus sends the shared status request and returns parsed data', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        headers: expect.objectContaining({ 'X-AeKB': '1' }),
      }),
    );
  });
  ...
  it('rejects AEKB_DAEMON_PORT so CLI and extension cannot split bridge ports', async () => {
    vi.resetModules();
    vi.stubEnv('AEKB_DAEMON_PORT', '19999');
    vi.mocked(fetch).mockResolvedValue({
      ok: true,
      json: () => Promise.resolve({
        ok: true,
    ...
    await expect(freshClient.fetchDaemonStatus()).rejects.toThrow('AEKB_DAEMON_PORT is no longer supported');

    expect(vi.mocked(fetch)).not.toHaveBeenCalled();
  });

  it('tolerates AEKB_DAEMON_PORT when it equals the default port (launchers inject it, #2068)', async () => {
    vi.resetModules();
    vi.stubEnv('AEKB_DAEMON_PORT', '19825');
    const status = {
      ok: true,
      pid: 1,
      uptime: 0,
  ...
  it('sendCommand forwards AEKB_PROFILE as a hard contextId requirement', async () => {
    vi.stubEnv('AEKB_PROFILE', 'work');
    vi.spyOn(Date, 'now').mockReturnValue(1_763_000_000_000);
    vi.mocked(fetch).mockResolvedValue({
      status: 200,
      json: () => Promise.resolve({ id: 'server', ok: true, data: 'ok' }),
  ...
  it('sendCommand forwards the config default as a soft preferredContextId, not a requirement', async () => {
    ...
    const configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aekb-dc-profile-'));
    fs.writeFileSync(
      path.join(configDir, 'browser-profiles.json'),
      JSON.stringify({ version: 1, aliases: {}, defaultContextId: 'zvypsyje' }),
    );
    vi.stubEnv('AEKB_CONFIG_DIR', configDir);
    vi.stubEnv('AEKB_PROFILE', '');
    try {
      vi.mocked(fetch).mockResolvedValue({
        status: 200,
        json: () => Promise.resolve({ id: 'server', ok: true, data: 'ok' }),
  ...
  it('sendCommand uses explicit windowMode before AEKB_WINDOW env fallback', async () => {
    vi.stubEnv('AEKB_WINDOW', 'foreground');
    vi.mocked(fetch).mockResolvedValue({
      status: 200,
      json: () => Promise.resolve({ id: 'server', ok: true, data: 'ok' }),
    } as Response);
  ...
  it('sendCommand does NOT wait when the bridge reports profile_required', async () => {
    ...
    fetchMock.mockResolvedValueOnce({
      ...
      json: () => Promise.resolve({
        ...
        errorHint: 'Run aekb profile list, then aekb profile use <name>.',
      }),
    } as Response);
