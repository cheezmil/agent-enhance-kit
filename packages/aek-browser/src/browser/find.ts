 ...
 * tagged by a prior snapshot: `data-aekb-ref` is set on the element
 * and a fingerprint is written into `window.__aekb_ref_identity`
...
export interface FindEntry {
  ...
  nth: number;
  /**
   * Numeric data-aekb-ref. Find assigns one if the element was not
...
export function buildFindJs(selector: string, opts: FindOptions = {}): string {
  ...
  return `
    (() => {
      ...
      // Ref allocation: reuse \`window.__aekb_ref_identity\` (the same map
      // snapshot populates) as the source of truth. For matched elements that
      // don't already carry a \`data-aekb-ref\`, assign the next free numeric
      // ref and write the fingerprint so the target resolver can verify it on
      // downstream click/type/get calls.
      const identity = (window.__aekb_ref_identity = window.__aekb_ref_identity || {});
      let maxRef = 0;
      for (const k in identity) {
        const n = parseInt(k, 10);
        if (!isNaN(n) && n > maxRef) maxRef = n;
      }
      // Also walk any \`data-aekb-ref\` already in the DOM in case the identity
      ...
      try {
        const tagged = document.querySelectorAll('[data-aekb-ref]');
        for (let t = 0; t < tagged.length; t++) {
          const v = tagged[t].getAttribute('data-aekb-ref');
      ...
      for (let i = 0; i < take; i++) {
        const el = matches[i];
        const refAttr = el.getAttribute('data-aekb-ref');
        let refNum = refAttr != null && /^\\d+$/.test(refAttr) ? parseInt(refAttr, 10) : null;
        if (refNum === null) {
          refNum = ++maxRef;
          try { el.setAttribute('data-aekb-ref', '' + refNum); } catch (_) {}
