 ...
 * 2. Ref path: cascading match levels (see below), using data-aekb-ref
...
export function resolveTargetJs(ref: string, opts: ResolveOptions = {}): string {
  ...
  return `
    (() => {
      ...
      const identity = window.__aekb_ref_identity || {};
      ...
      if (isNumeric) {
        ...
        // snapshot + find populate into \`__aekb_ref_identity\`. Kept inline
        ...
        let el = document.querySelector('[data-aekb-ref="' + ref + '"]');
        ...
        if (!el) {
          ...
          if (recovered) {
            try {
              recovered.setAttribute('data-aekb-ref', ref);
          ...
          return {
            ...
            hint: 'The element may have been removed. Re-run \`aekb state\` to get a fresh snapshot.',
          };
        }
        ...
        // data-aekb-ref attribute on the surviving node.
        const recovered = reidentify(fp);
        if (recovered && recovered !== el) {
          try {
            el.removeAttribute('data-aekb-ref');
            recovered.setAttribute('data-aekb-ref', ref);
            identity[ref] = fingerprintOf(recovered);
          } catch (_) {}
          window.__resolved = recovered;
        ...
        return {
          ...
          hint: 'The page has changed since the last snapshot. Re-run \`aekb state\` to refresh.',
        };
      }
      ...
      // ── CSS selector path (any non-numeric input) ──
      {
        ...
        if (matches.length === 0) {
          return {
            ...
            hint: 'The element may not exist or may be hidden. Re-run \`aekb state\` to check, or use \`aekb find --css\` to explore candidates.',
            matches_n: 0,
          };
        }
        ...
        if (matches.length > 1 && !firstOnMulti) {
          ...
          return {
            ...
            hint: 'Pass --nth <n> (0-based) to pick one, or use a more specific selector. Use \`aekb find --css\` to list all candidates.',
