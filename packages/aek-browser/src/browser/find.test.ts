...
describe('buildFindJs', () => {
  ...
  it('allocates fresh refs for untagged matches (write attribute + identity map)', () => {
    ...
    expect(js).toContain("el.setAttribute('data-aekb-ref'");
    ...
    expect(js).toContain('__aekb_ref_identity');
    expect(js).toContain("identity['' + refNum] = fingerprintOf(el)");
    // Allocation walks both the identity map and any existing data-aekb-ref
    // annotations — guards against collisions after a soft nav.
    expect(js).toContain("document.querySelectorAll('[data-aekb-ref]')");
  });
...
describe('buildSemanticFindJs', () => {
  ...
  it('executes semantic role/name/testid matching and allocates a clickable ref', () => {
    ...
    expect(button.getAttribute('data-aekb-ref')).toBe('1');
    expect((dom.window as any).__aekb_ref_identity['1']).toMatchObject({ tag: 'button', ariaLabel: 'Save expense' });
  });
