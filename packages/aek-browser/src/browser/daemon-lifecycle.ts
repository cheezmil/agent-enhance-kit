...
export async function ensureBrowserBridgeReady(
  ...
  if (isStale) {
    ...
    if (verbose && (process.env.AEKB_VERBOSE || process.stderr.isTTY)) {
    ...
    if (shutdownAccepted) {
      portReleased = await daemonLifecycleHooks.waitForDaemonStop(3000);
    }

    if (!portReleased && typeof stalePid === 'number' && Number.isInteger(stalePid) && stalePid > 0) {
      try {
    ...
    if (!portReleased) {
      throw new BrowserConnectError(
        ...
        '  Run manually: AEK Browser daemon stop',
        'daemon-not-running',
      );
    }
  ...
  } else if (verbose && (process.env.AEKB_VERBOSE || process.stderr.isTTY)) {
    process.stderr.write('⏳ Waiting for Chrome/Chromium extension to connect...\n');
    process.stderr.write('   Make sure Chrome or Chromium is open and the AEK Browser extension is enabled.\n');
  }
...
function browserConnectErrorFromHealth(health: DaemonHealth, contextId?: string): BrowserConnectError {
  if (health.state === 'profile-required') {
    return new BrowserConnectError(
      ...
      'Select one with --profile <name>, AEKB_PROFILE=<name>, or AEK Browser profile use <name>.\n' +
      'Run AEK Browser profile list to see connected profiles.',
      'profile-required',
    );
  }
  if (health.state === 'profile-disconnected') {
    ...
    return new BrowserConnectError(
      ...
      'Open the matching Chrome profile and make sure the AEK Browser extension is enabled, or choose another profile with AEK Browser profile use <name>.',
  ...
  if (health.state === 'no-extension') {
    return new BrowserConnectError(
      ...
      'Make sure Chrome/Chromium is open and the AEK Browser extension is enabled.\n' +
      'If not installed:\n' +
      '  1. Download: https://github.com/cheezmil/AEK Browser/releases\n' +
      '  2. Open chrome://extensions → Developer Mode → Load unpacked',
      'extension-not-connected',
    );
  }
  return new BrowserConnectError(
    'Failed to start AEK Browser daemon',
