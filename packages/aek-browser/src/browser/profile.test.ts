...
describe('profile selection (requirement vs preference)', () => {
  ...
  beforeEach(() => {
    configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aekb-profile-test-'));
  ...
  it('tags AEKB_PROFILE env as explicit', () => {
    vi.stubEnv('AEKB_PROFILE', 'pavmrekj');
  ...
  it('explicit argument beats env beats config default', () => {
    vi.stubEnv('AEKB_PROFILE', 'from-env');
