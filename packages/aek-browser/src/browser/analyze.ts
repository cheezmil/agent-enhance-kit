...
export function analyzeSite(
  ...
  if (antiBot.detected) {
    ...
    next = 'Read the SSR global via `aekb eval "JSON.stringify(window.__INITIAL_STATE__ ?? window.__NUXT__ ?? window.__NEXT_DATA__ ?? window.__APOLLO_STATE__)"` — no API needed.';
  } else if (pattern.pattern === 'C') {
    next = 'No API visible — use SSR HTML scrape (e.g. `aekb extract`) against the rendered page.';
    ...
    next = 'No strong signal. Manually inspect `aekb network --all` and pick a pattern.';
  }
