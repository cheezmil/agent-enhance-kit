...
describe('waitForCaptureJs', () => {
  it('returns a non-empty string', () => {
    ...
    expect(code).toContain('__aekb_xhr');
    expect(code).toContain('resolve');
    expect(code).toContain('reject');
  });

  it('resolves "captured" when __aekb_xhr is populated before deadline', async () => {
    const g = globalThis as unknown as Record<string, unknown>;
    const captured: unknown[] = [];
    g.__aekb_xhr = captured;
  ...
  it('rejects when __aekb_xhr stays empty past deadline', async () => {
    const g = globalThis as unknown as Record<string, unknown>;
    g.__aekb_xhr = [];
    g.window = g;
  ...
  it('resolves immediately when __aekb_xhr already has data', async () => {
    const g = globalThis as unknown as Record<string, unknown>;
    g.__aekb_xhr = [{ data: 'already here' }];
    g.window = g;
    const code = waitForCaptureJs(1000);
