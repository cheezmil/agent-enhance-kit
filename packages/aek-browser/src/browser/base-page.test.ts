...
describe('BasePage annotatedScreenshot', () => {
  it('refreshes DOM refs, captures with a temporary visual overlay, and cleans up', async () => {
    ...
    await expect(page.annotatedScreenshot({ path: '/tmp/aekb.png', annotate: true })).resolves.toBe('shot');

    expect(page.scripts[0]).toContain('const VIEWPORT_EXPAND = 0');
    expect(page.scripts[2]).toContain('__aekb_visual_ref_overlay');
    expect(page.scripts[2]).toContain('[data-aekb-ref]');
    expect(page.scripts[3]).toContain('__aekb_visual_ref_overlay');
    expect(page.screenshotCalls).toEqual([{ path: '/tmp/aekb.png', annotate: false }]);
  });
});
...
describe('BasePage native input routing', () => {
  ...
  it('uploads files through setFileInput using a temporary marker selector', async () => {
    ...
    expect(page.setFileInput).toHaveBeenCalledWith(['/tmp/receipt.pdf'], expect.stringMatching(/data-aekb-upload-target/));
    expect(page.withArgs.at(0)).toMatchObject({ markerAttr: 'data-aekb-upload-target' });
    expect(page.withArgs.at(-1)).toMatchObject({ markerAttr: 'data-aekb-upload-target' });
  });
