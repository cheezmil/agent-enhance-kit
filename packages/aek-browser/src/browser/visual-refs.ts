 ...
 * `data-aekb-ref` attributes produced by the DOM snapshot path so the
 * screenshot labels map back to normal `browser click <ref>` targets.
 */

const OVERLAY_ID = '__aekb_visual_ref_overlay';
...
export function installVisualRefOverlayJs(opts: { maxRefs?: number } = {}): string {
  ...
  return `
    (() => {
      ...
      const refs = Array.from(document.querySelectorAll('[data-aekb-ref]'))
        .map((el) => {
          const rawRef = el.getAttribute('data-aekb-ref') || '';
