 ...
 * Captured responses are pushed to `window.__aekb_intercepted`.
 *
 * @param patternExpr - JS expression resolving to a URL substring to match (e.g. a JSON.stringify'd string)
 * @param opts.arrayName - Global array name for captured data (default: '__aekb_intercepted')
 * @param opts.patchGuard - Global boolean name to prevent double-patching (default: '__aekb_interceptor_patched')
 */
export function generateInterceptorJs(
  patternExpr: string,
  opts: { arrayName?: string; patchGuard?: string } = {},
): string {
  const arr = opts.arrayName ?? '__aekb_intercepted';
  const guard = opts.patchGuard ?? '__aekb_interceptor_patched';
 ...
 */
export function generateReadInterceptedJs(arrayName: string = '__aekb_intercepted'): string {
  return `
    () => {
