...
describe('observation artifact', () => {
  ...
  beforeEach(() => {
    baseDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aekb-trace-'));
  });
  ...
  it('writes artifacts under profile-scoped trace directory', () => {
    ...
    expect(summary).toContain('aekbVersion:');
  ...
  it('builds a compact trace receipt', () => {
    const receipt = buildTraceReceipt({
      traceId: 'trace-1',
      dir: '/tmp/aekb/profiles/work/traces/trace-1',
      summaryPath: '/tmp/aekb/profiles/work/traces/trace-1/summary.md',
      receiptPath: '/tmp/aekb/profiles/work/traces/trace-1/receipt.json',
    ...
    expect(receipt).toMatchObject({
      ...
      aekbVersion: expect.any(String),
      traceId: 'trace-1',
      traceDir: '/tmp/aekb/profiles/work/traces/trace-1',
      receiptPath: '/tmp/aekb/profiles/work/traces/trace-1/receipt.json',
      status: 'failure',
