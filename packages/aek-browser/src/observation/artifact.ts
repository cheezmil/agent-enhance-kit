...
function renderSummary(
  ...
  const lines = [
    '---',
    'schemaVersion: 1',
    `aekbVersion: ${yamlScalar(PKG_VERSION)}`,
    ...
    '---',
    '',
    '# AEK Browser Trace Summary',
    '',
    '## How To Use',
    '',
    '- Start with this summary, then inspect `trace.jsonl` only when the evidence below is insufficient.',
    '- For adapter repair policy and retry limits, use the `aekb-autofix` skill.',
