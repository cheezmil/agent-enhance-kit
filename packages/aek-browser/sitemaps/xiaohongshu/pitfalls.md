...
> **Scope**：本文件只列 **task-executing agent** 跑 sitemap workflow 时会撞的坑。adapter-author 实现层的坑（Pinia store SSR vs XHR 形状差异 / creator-center shadow-DOM publish button / `xsec_token` signed URL 起源）放在 `~/.aek-browser/sites/xiaohongshu/notes.md`，与本文件互补。
...
- workaround: 跑 workflow / adapter 前先 `aek-browser xiaohongshu whoami` 验登录态（# pending: login command MVP, codex task #276）；未登录 → `aek-browser xiaohongshu login`；不要尝试自动填用户名密码（CAPTCHA / 短信 2FA / 滑块 全踩，"human authenticates, machine verifies"）
...
- workaround: note URL 必须来自 feed / search / user adapter 输出，那些 adapter 已经把 signed URL 完整带出；不要自己从 note_id 重建 URL；如果只有 note_id，先跑 `aek-browser xiaohongshu search` / `feed` 拿到 signed URL 再 drill down
- verified_at: 2026-06-04

### pitfall:search_api_returns_empty

- trigger: 走 workflow Best path `aek-browser xiaohongshu search`，adapter 返 0 行或 EmptyResultError
...
- workaround: 发笔记直接 `aek-browser xiaohongshu publish ...`，adapter 内部走 creator host；如果 fallback 需手操，先 `goto creator.xiaohongshu.com/publish/publish?from=menu_left&target=image` 再操作
- verified_at: 2026-06-04
...
- workaround: 不要手 click；`aek-browser xiaohongshu publish` adapter 已用 instance method invocation (`_onPublish` / `onPublish` / `_onSubmit` / `_handlePublish`) 兜底（#1606）；如果 adapter broken 必须手动，用 `aek-browser browser evaluate` 调实例方法而不是 click
- verified_at: 2026-06-04
...
- workaround: 优先用 `aek-browser xiaohongshu publish` adapter（已 prioritize visible title input）；fallback 时按 placeholder text `请输入标题` 加 visible filter (`offsetWidth > 50`) 选可见 input；不要按 nth-child / first match
- verified_at: 2026-06-04
