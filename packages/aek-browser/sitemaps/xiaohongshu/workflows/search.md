...
- 拿到 list 后 drill-down 单 note 走 `aek-browser xiaohongshu note <url>`，不属于本 workflow
...
```yaml
adapter: aek-browser xiaohongshu search
...
直接 `aek-browser xiaohongshu search "<keyword>" --limit 20`。adapter 内部 navigate `/search_result/?keyword=<encoded>` + DOM scrape `section.note-item`。
...
```yaml
on_adapter_fail:
  - adapter_health_update: aek-browser xiaohongshu search -> suspect
  - if AuthRequiredError:
    - recovery: aek-browser xiaohongshu login  # pending: codex task #276 实现该子命令
    - 登录后 retry adapter 一次
  - aek-browser browser state (verify current URL/login wall)
...
agent 中断后醒来按 `aek-browser browser state` URL 判断：

- on `/`，未搜过 → 重新跑 Best path
- on `/search_result/?keyword=<X>` 且 note-item 已渲染 → 从 Fallback step "read_card_meta" 起
- on `/login` 或命中 login wall overlay → 走 Recovery path：`aek-browser xiaohongshu login`（# pending: codex task #276），login 后回 Best path 重跑
...
- adapter `aek-browser xiaohongshu search` 30 天内 fix PR 增多（DOM class 漂） → adapter_health audit 标 suspect
