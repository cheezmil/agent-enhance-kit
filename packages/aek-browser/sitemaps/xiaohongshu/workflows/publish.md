...
- success: `aek-browser xiaohongshu creator-notes` 列表里出现新笔记（title 匹配），或 publish toast `发布成功`

## Best path

```yaml
adapter: aek-browser xiaohongshu publish
...
直接 `aek-browser xiaohongshu publish --title "<title>" "<body>" --images /path/a.jpg,/path/b.jpg [--topics 生活,旅行]`。adapter 内部 navigate creator host + CDP file upload + shadow DOM submit。
...
```yaml
on_adapter_fail:
  - adapter_health_update: aek-browser xiaohongshu publish -> suspect
  - if AuthRequiredError:
    - recovery: aek-browser xiaohongshu login  # pending: codex task #276
    - 登录后 retry adapter 一次
  - aek-browser browser state (verify host + URL)
  ...
  - cross-verify: aek-browser xiaohongshu creator-notes --limit 1 看顶部是否是新发的
estimated_turns: 8
```
...
agent 中断后醒来按 `aek-browser browser state` URL + creator-notes 比对判断：
...
- `aek-browser xiaohongshu creator-notes --limit 5` 顶部出现 title 匹配新 row，或
