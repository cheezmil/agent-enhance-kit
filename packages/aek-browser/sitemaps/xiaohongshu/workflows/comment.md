...
- success: 评论区顶部 / 自己 username 下出现刚发的文本，且 `aek-browser xiaohongshu comments <url>` 输出含该条

## Best path

xhs 现阶段 **没有 cookie-API 形 `aek-browser xiaohongshu comment` adapter**（comments adapter 只读不写）。直接走 browser DOM workflow：
...
flow:
  ...
  - verify: aek-browser xiaohongshu comments <url> --limit 5 看顶部是否是新评论
```

> **Adapter follow-up candidate**：cookie-API 形 `aek-browser xiaohongshu comment <url> "<text>"` 是合理的下一发，发完 v1 sitemap 后开 issue 给 adapter-author。
...
on_action_fail:
  - if URL 含 /login OR overlay "请登录":
    - recovery: aek-browser xiaohongshu login  # pending: codex task #276
  ...
  - if submit button click 后评论未出现 > 5s:
    - 等 10s 后 cross-verify aek-browser xiaohongshu comments，可能是 server lag
...
agent 中断后醒来按 `aek-browser browser state` URL + comments 比对判断：
...
- `aek-browser xiaohongshu comments <url> --limit 5` 输出顶部 row 的 `author` 是 self（whoami 对比） 且 `content` 匹配（normalize whitespace + emoji NFC）
- 评论时间显示 `刚刚` / `1秒前` / `1分钟前`
