...
- `/notifications` → 直接用 adapter `aek-browser xiaohongshu notifications`，无 workflow
- `creator.xiaohongshu.com/creator/notes` → 直接用 adapter `aek-browser xiaohongshu creator-notes`，无 workflow
...
- read a note's full content + comments → 直接用 adapter `aek-browser xiaohongshu note <url>` + `aek-browser xiaohongshu comments <url>`
- read user's notes → 直接用 adapter `aek-browser xiaohongshu user <id>`
