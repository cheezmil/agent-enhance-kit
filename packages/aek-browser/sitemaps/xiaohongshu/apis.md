...
> 引用规则（schema §2.4）：`endpoint_id` 必须存在于 `~/.aek-browser/sites/xiaohongshu/endpoints.json`。本文件只放 endpoint_id + 触发关系 + contract_strength，URL/method/params/response 是 endpoints.json 单一来源。
...
- 跑 `aek-browser browser network` 抓 xhs session 拿到当前 XHR endpoint set，回填 `~/.aek-browser/sites/xiaohongshu/endpoints.json`
