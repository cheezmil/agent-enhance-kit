...
### action:run_search
pre: on /, logged_in
do: aek-browser xiaohongshu search "<keyword>" || (focus input[placeholder*="搜索小红书"] + type + Enter)
post: URL -> /search_result/?keyword=<encoded>; note grid rendered
fail: stays on / | "登录后查看搜索结果" overlay | empty results panel
recover: adapter_health_update: aek-browser xiaohongshu search -> suspect; check whoami; persistent empty -> workflows/search.md Fallback
evidence: aek-browser xiaohongshu search
```
