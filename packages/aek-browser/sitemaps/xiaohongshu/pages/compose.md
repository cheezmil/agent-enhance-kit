...
do: aek-browser xiaohongshu publish --title "..." "..." --images /path/a.jpg,/path/b.jpg ... (走 CDP DOM.setFileInputFiles)
...
evidence: aek-browser xiaohongshu publish
```
...
do: aek-browser xiaohongshu publish --topics 生活,旅行 (adapter 内部处理 #话题 trigger + 选项 click)
...
do: aek-browser xiaohongshu publish ... (adapter 内部走 shadow DOM instance method invocation: `_onPublish` / `onPublish` / `_onSubmit` / `_handlePublish`)
...
evidence: aek-browser xiaohongshu publish; aek-browser xiaohongshu creator-notes (verify new note)
```

```yaml
### action:save_draft
pre: 至少 title 或 images 之一存在
do: aek-browser xiaohongshu publish --draft ... (adapter 内部走 `_onSave` / `_onSaveDraft` / `_onDraft`)
post: redirect /creator/notes/drafts 或 toast "已保存草稿"；可通过 aek-browser xiaohongshu drafts 列出
fail: 同 submit_publish
recover: 同 submit_publish
evidence: aek-browser xiaohongshu publish --draft; aek-browser xiaohongshu drafts
```
