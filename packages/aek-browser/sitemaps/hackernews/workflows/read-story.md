...
- adapter: aek-browser hackernews story <id>
...
如果 adapter 抛 typed error / 空 result（触发 `adapter_health_update: aek-browser hackernews story -> suspect`）：

1. `aek-browser browser open https://news.ycombinator.com/item?id=<id>`
2. action:expand_comment_tree in pages/item.md（如果有折叠）
3. `aek-browser browser state` 拿 DOM
