...
do: aek-browser hackernews story <id> || click `tr.athing[id="<id>"] + tr a[href^="item?id="]`
post: URL → /item?id=<id> AND <title> 含 story 标题
fail: click 落到外部 URL（误点 title link） | athing 不在当前 page
recover: 误点外站立即 back; athing 找不到时 reload /news 或用 adapter 直 fetch; adapter_health_update: aek-browser hackernews story -> suspect
evidence: aek-browser hackernews top --limit 30 + aek-browser browser open
...
do: aek-browser hackernews top --offset <N> || click `a.morelink`
post: URL 加 ?p=<N> 或 ?next=<id>&n=<offset>，新 30 条 stories
fail: morelink 不存在（HN 默认 ~30 条/页）
recover: adapter --limit 跨页透明，DOM 翻页是 worse path; adapter_health_update: aek-browser hackernews top -> suspect
evidence: aek-browser hackernews top --offset 30
