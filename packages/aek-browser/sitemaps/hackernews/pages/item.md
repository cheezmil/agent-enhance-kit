...
do: aek-browser hackernews story <id> (API 拿全树) || click `a.togg`
post: 子 comment 行展开（DOM 多 `<tr class="comtr">`）
fail: togg link 不响应（极少）
recover: 直接走 Firebase `item/<id>.json` 递归 kids; adapter_health_update: aek-browser hackernews story -> suspect
evidence: aek-browser hackernews story <id>
