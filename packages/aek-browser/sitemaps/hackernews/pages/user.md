...
do: aek-browser hackernews user <handle> || click `a[href^="submitted?id="]`
post: URL → /submitted?id=<handle>，story list visible
fail: redirect 404 (handle 不存在)
recover: handle 不存在抛 CommandExecutionError; adapter_health_update: aek-browser hackernews user -> suspect
evidence: aek-browser hackernews user <handle>
