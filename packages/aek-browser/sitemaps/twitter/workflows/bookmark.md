...
```yaml
adapter: aek-browser twitter bookmark
...
`aek-browser twitter bookmark <status-url>` add, `aek-browser twitter unbookmark <status-url>` remove。

## Fallback path

```yaml
on_adapter_fail:
  - adapter_health_update: aek-browser twitter bookmark -> suspect
...
- toast 文本 i18n 漂
- adapter `aek-browser twitter bookmark` 月度 fix 多 → adapter_health audit suspect
