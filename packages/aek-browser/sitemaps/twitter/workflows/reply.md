...
```yaml
adapter: aek-browser twitter reply
...
直接 `aek-browser twitter reply --status-url <url> --text "<content>"`。
...
```yaml
on_adapter_fail:
  - adapter_health_update: aek-browser twitter reply -> suspect
...
- adapter `aek-browser twitter reply` 月度 fix 多 → adapter_health audit 标 suspect
