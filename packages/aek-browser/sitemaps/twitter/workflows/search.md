...
```yaml
adapter: aek-browser twitter search
...
`aek-browser twitter search --query "<keyword>" [--type latest|top|people]`。adapter 内置 queryId bundle parse 三层降级（实现细节见 `notes.md`）。

## Fallback path

```yaml
on_adapter_fail:
  - adapter_health_update: aek-browser twitter search -> suspect
...
- adapter `aek-browser twitter search` queryId bundle parser silent fail (PR #1531 sediment: bundle partial fail 返 `{}` 盖 baked fallback)
