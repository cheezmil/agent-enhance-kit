...
- 带 media 走 adapter `aek-browser twitter post --image-url` 直接路径
...
```yaml
adapter: aek-browser twitter post
...
直接 `aek-browser twitter post --text "<content>"`。adapter 内部 handle COOKIE 注入 / queryId rotation / verify。
...
```yaml
on_adapter_fail:
  - adapter_health_update: aek-browser twitter post -> suspect
  - aek-browser browser state (verify current page)
...
- adapter `aek-browser twitter post` 30 天内 fix PR 增多 → adapter_health audit 标 suspect
