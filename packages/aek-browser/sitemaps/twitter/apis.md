...
> 引用规则（schema §2.4）：`endpoint_id` 必须存在于 `~/.aek-browser/sites/twitter/endpoints.json`。本文件只放 endpoint_id + 触发关系 + contract_strength，URL/method/params/response 是 endpoints.json 单一来源。
...
- 跑 `aek-browser browser network` 抓 twitter session 一次拿到当前 queryId map，全量回写 endpoints.json
