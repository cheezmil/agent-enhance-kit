...
evidence: aek-browser browser type
```

```yaml
### action:submit_modal_post
pre: textarea has content, submit enabled, on modal form (NOT inline)
do: aek-browser twitter post --text "..." || click [data-testid="tweetButton"] (NOT tweetButtonInline)
post: modal dismissed, URL returns to caller page, new post in timeline
fail: modal not dismissing | "Something went wrong" toast
recover: adapter_health_update: aek-browser twitter post -> suspect; close modal + retry; persistent fail -> workflows/post.md Fallback
evidence: aek-browser twitter post
```
...
- 含 media upload 的 post 不在 PoC v1 覆盖：file picker / blob URL 复杂，adapter `aek-browser twitter post --image-url` 已 handle，task agent 直接走 adapter
