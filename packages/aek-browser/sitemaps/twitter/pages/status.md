...
do: aek-browser twitter reply --status-url <url> --text "..." || click [data-testid="tweetButtonInline"]
...
recover: adapter_health_update: aek-browser twitter reply -> suspect; apply pitfall workaround (single-paragraph plain text, escape bullets); long reply -> workflows/post.md + quote
evidence: aek-browser twitter reply
```
