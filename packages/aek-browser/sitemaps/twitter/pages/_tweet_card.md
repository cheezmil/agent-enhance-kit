...
evidence: aek-browser browser click + state
```

```yaml
### action:like_tweet
pre: card visible AND tweet_url known (or extract via article a[href*="/status/"])
do: aek-browser twitter like <tweet-url> || click [data-testid="like"] in card scope
post: card testid like -> unlike, icon red
fail: testid unchanged | login modal | wrong card flipped (scope missing)
recover: adapter_health_update: aek-browser twitter like -> suspect; dom_click in card scope; rollback via [data-testid="unlike"]
evidence: aek-browser twitter like
```
...
evidence: aek-browser browser click
```

```yaml
### action:repost_tweet
pre: card visible AND tweet_url known
do: aek-browser twitter retweet <tweet-url> || click [data-testid="retweet"] in card then [data-testid="retweetConfirm"]
post: card testid retweet -> unretweet
fail: confirm menu missing | testid unchanged
recover: adapter_health_update: aek-browser twitter retweet -> suspect; dom_click confirm flow; rollback via [data-testid="unretweet"] + confirm
evidence: aek-browser twitter retweet
```

```yaml
### action:bookmark_tweet
pre: card visible AND tweet_url known
do: aek-browser twitter bookmark <tweet-url> || click [data-testid="bookmark"] in card scope
post: card testid bookmark -> removeBookmark, toast "Added to your Bookmarks"
fail: testid unchanged | toast missing
recover: adapter_health_update: aek-browser twitter bookmark -> suspect; dom_click + verify toast
evidence: aek-browser twitter bookmark
```
