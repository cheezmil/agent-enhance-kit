...
do: aek-browser twitter follow <handle> || click [data-testid*="-follow"]
post: button text -> "Following", testid -> [data-testid*="-unfollow"]
fail: button text unchanged | login modal | "Follow blocked" toast
recover: adapter_health_update: aek-browser twitter follow -> suspect; dom_click [data-testid*="-follow"]; rollback via unfollow button
evidence: aek-browser twitter follow
```
