...
Gmail desktop mail UI. Prefer `aek-browser gmail ...`: reads use Gmail's own search/navigation and intercept natural responses. This sitemap does not define write actions because no supported browser-session API contract is currently available.
...
- `/mail/u/<account>/#settings/labels` → settings labels, read by `aek-browser gmail labels`
...
- inspect identity → `aek-browser gmail whoami`
