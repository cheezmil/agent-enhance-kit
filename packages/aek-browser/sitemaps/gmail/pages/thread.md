...
recover: adapter_health_update: aek-browser gmail thread -> suspect; expand collapsed messages, then inspect within each message container
...
do: aek-browser gmail attachments <threadId>
post: one row per attachment; EmptyResult when none
fail: attachment cards visible but no rows | wrong message scope
recover: adapter_health_update: aek-browser gmail attachments -> suspect; inspect .aQH within each data-message-id container
