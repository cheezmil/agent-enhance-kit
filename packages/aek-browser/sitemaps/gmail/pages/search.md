...
do: aek-browser gmail search "<gmail-query>" --limit <n>
...
recover: adapter_health_update: aek-browser gmail search -> suspect; confirm input[name="q"] and run browser network while submitting the query
...
do: aek-browser gmail thread <threadId>
...
recover: rerun gmail search for the target, then retry thread; adapter_health_update: aek-browser gmail thread -> suspect
evidence: live result row opened by data-legacy-thread-id
```

## Linked APIs

API details are in `skills/aek-browser-adapter-author/references/site-memory/gmail.md`; no endpoint ids are duplicated here.
