...
workaround: use `aek-browser gmail search`; browser fallback must use native text insertion plus raw native Enter
...
workaround: use `aek-browser gmail thread`; fallback scopes `[data-message-id]` and `.a3s` to the rendered thread
verified_at: 2026-08-25
