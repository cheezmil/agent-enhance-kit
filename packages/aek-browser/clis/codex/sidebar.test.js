...
function fixtureDocument() {
    return el('document', {}, [
        ...
        project('aekb', '/Users/youngcan/aekb', [
            thread({ threadId: 'local:aekb-groups' }, '统一 aekb 二级命令分组', '1 天'),
        ]),
    ]);
}
...
describe('codex sidebar helpers', () => {
    ...
    it('flattens project rows with project filters', () => {
        const projects = collectCodexProjectsFromDocument(fixtureDocument());
        const rows = flattenCodexProjects(projects, { project: 'aekb' });

        expect(rows).toEqual([
            expect.objectContaining({
                Project: 'aekb',
                Index: 1,
                Title: '统一 aekb 二级命令分组',
                Updated: '1 天',
            }),
        ]);
    });
    ...
    it('does not match nested project paths when filtering by a parent label', () => {
        ...
        projects.push({
            index: 3,
            project: 'nested',
            projectPath: '/Users/youngcan/aekb/nested',
            collapsed: false,
            conversations: [
        ...
        expect(rows.map(row => row.Project)).toEqual(['aekb']);
    });
    ...
    it('selects a conversation by index within a project', () => {
        ...
        expect(result).toMatchObject({
            ok: true,
            project: 'aekb',
            conversation: '统一 aekb 二级命令分组',
            threadId: 'local:aekb-groups',
        });
    });
