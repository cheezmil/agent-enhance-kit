import {
  ...
  EmptyResultError,
  TimeoutError,
} from '@cheezmil/aek-browser/errors';
import { log } from '@cheezmil/aek-browser/logger';
...
export function parseJobId(value) {
  ...
  throw new ArgumentError(
    ...
    'Example: AEK Browser midjourney status d5664250-5f1f-4cd0-9637-2ce0153dd30a',
  );
}
...
export function normalizePrompt(value) {
  ...
  if (!prompt) {
    throw new ArgumentError(
      ...
      'Example: AEK Browser midjourney generate "a blue ceramic teapot --ar 1:1"',
    );
  }
  return prompt;
}
...
export async function fetchJobStatuses(page, jobIds) {
  ...
  const payload = await midjourneyJson(page, '/api/job-status', {
    ...
    body: { jobIds, _frontend_source: 'aekb_adapter' },
  });
...
async function fetchMediaThroughPage(page, url, expectedMimePrefix) {
  const transferKey = `aekb_media_${Date.now()}_${Math.random().toString(36).slice(2)}`;
  try {
    let payload;
    try {
...
export async function downloadRenderedVideo(page, jobId, index, kind, outputDir, force = false) {
  ...
  const marked = unwrapEvaluateResult(await page.evaluate((label) => {
    document.querySelectorAll('[data-aekb-video-download]').forEach((node) => node.removeAttribute('data-aekb-video-download'));
...
export async function openImagePanel(page) {
  const markInput = async () => unwrapEvaluateResult(await page.evaluate(() => {
    document.querySelectorAll('[data-aekb-image-input]').forEach((node) => node.removeAttribute('data-aekb-image-input'));
    ...
    active.setAttribute('data-aekb-image-input', '1');
    return true;
  }));
  if (!await markInput()) {
...
async function injectImagesFallback(page, localPaths) {
  ...
  const uploadKey = `aekb_${Date.now()}_${Math.random().toString(36).slice(2)}`;
  await page.evaluate((key) => {
    window[key] = [];
    return true;
  }, uploadKey);
  try {
    ...
    const input = document.querySelector('[data-aekb-image-input="1"]');
...
async function markReferenceTarget(page, sourceUrl, slotLabel) {
  const result = unwrapEvaluateResult(await page.evaluate((url, label) => {
    document.querySelectorAll('[data-aekb-ref-source],[data-aekb-ref-target]').forEach((el) => {
      el.removeAttribute('data-aekb-ref-source');
      el.removeAttribute('data-aekb-ref-target');
    });
    ...
    source.setAttribute('data-aekb-ref-source', '1');
    target.setAttribute('data-aekb-ref-target', '1');
    return { ok: true };
  }, sourceUrl, slotLabel));
...
export async function uploadReferenceLibrary(page, localPaths) {
  ...
  if (typeof page.setFileInput === 'function') {
    try {
      await page.setFileInput(localPaths, '[data-aekb-image-input="1"]');
      uploaded = true;
    } catch (error) {
      const message = errorMessage(error);
...
async function openEndFramePicker(page) {
  const marked = unwrapEvaluateResult(await page.evaluate(() => {
    document.querySelectorAll('[data-aekb-end-frame-picker]').forEach((node) => {
      node.removeAttribute('data-aekb-end-frame-picker');
    });
    ...
    for (let depth = 0; depth < 7 && target; depth += 1, target = target.parentElement) {
      if (String(target.className).includes('cursor-pointer')) {
        target.setAttribute('data-aekb-end-frame-picker', '1');
        return true;
      }
    }
    return false;
  ...
  await page.click('[data-aekb-end-frame-picker="1"]');
  await page.wait({ selector: 'input[type="file"][accept*="image"]', timeout: 10 });
}
...
export async function uploadReferencesToSlot(page, localPaths, slot) {
  ...
  for (const url of selected) {
    ...
    await page.drag('[data-aekb-ref-source="1"]', '[data-aekb-ref-target="1"]');
    await page.wait(0.8);
...
export async function clickComposerSubmit(page) {
  const marked = unwrapEvaluateResult(await page.evaluate(() => {
    document.querySelectorAll('[data-aekb-composer-submit]').forEach((node) => {
      node.removeAttribute('data-aekb-composer-submit');
    });
    const visible = (element) => {
      const rect = element.getBoundingClientRect();
      return rect.width > 0 && rect.height > 0 && getComputedStyle(element).display !== 'none';
    ...
    button.setAttribute('data-aekb-composer-submit', '1');
    return true;
  }));
  if (!marked) throw new CommandExecutionError('Midjourney composer submit control was not found');
  await page.click('[data-aekb-composer-submit="1"]');
}
...
export async function selectSiteSetting(page, anchorText, candidates, targetText) {
  ...
  const target = unwrapEvaluateResult(await page.evaluate((anchorLabel, values, wanted) => {
    ...
    document.querySelectorAll('[data-aekb-setting-target]').forEach((node) => node.removeAttribute('data-aekb-setting-target'));
    ...
    for (let depth = 0; depth < 7 && root; depth += 1, root = root.parentElement) {
      ...
      if (matching.length >= Math.min(2, values.length)) {
        ...
        button.setAttribute('data-aekb-setting-target', '1');
  ...
  if (target.changed) {
    await page.click('[data-aekb-setting-target="1"]');
    await page.wait(0.4);
