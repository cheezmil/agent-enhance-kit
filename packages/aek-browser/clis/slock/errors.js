import { ArgumentError, AuthRequiredError, CommandExecutionError, ConfigError } from '@cheezmil/aek-browser/errors';
...
export function dispatchEvaluateResult(r) {
  switch (r && r.kind) {
    ...
    case 'no-server':
      throw new ConfigError(r.detail, 'Run `aekb slock server-use <slug>` to set the active server.');
    case 'unresolvable':
