...
cli({
    ...
    func: async (page) => {
        ...
        if (process.env.AEKB_VERBOSE) {
            console.error('[aekb:v2ex] Navigating to /mission/daily');
        }
        ...
        for (let i = 0; i < 5; i++) {
            ...
            if (!title?.includes('Just a moment'))
                break;
            if (process.env.AEKB_VERBOSE)
                console.error('[aekb:v2ex] Waiting for Cloudflare...');
        }
    ...
    `);
        if (checkResult.error) {
            if (process.env.AEKB_VERBOSE) {
                console.error(`[aekb:v2ex:debug] Page Title: ${checkResult.debug_title}`);
                console.error(`[aekb:v2ex:debug] Page Body: ${checkResult.debug_body}`);
            }
        ...
        // Perform check in
        if (process.env.AEKB_VERBOSE) {
            console.error(`[aekb:v2ex] Found check-in token: once=${checkResult.once}. Checking in...`);
        }
