...
async function resolveTag(page, value) {
    ...
    if (liveTag)
        return liveTag;
    throw new ArgumentError(`Unknown tag: ${value}`, 'Use "AEK Browser linux-do tags" to list available tags');
}
/**
 * 解析分类，并补齐父分类信息。
 */
async function resolveCategory(page, value) {
    ...
    if (liveCategory)
        return liveCategory;
    throw new ArgumentError(`Unknown category: ${value}`, 'Use "AEK Browser linux-do categories" to list available categories');
}
/**
 * 将命令参数转换为最终请求地址
 */
