import { ArgumentError, CommandExecutionError, EmptyResultError, selectorError } from '@cheezmil/aek-browser/errors';
  ...
  return `
    (function(kinds, click, limit, maxChars, markCandidates) {
      ...
      for (const candidate of candidates) {
        ...
        if (markCandidates) {
          const marker = 'aekb-approval-' + rows.length + '-' + Math.random().toString(36).slice(2);
          candidate.button.setAttribute('data-aekb-approval-id', marker);
          selector = '[data-aekb-approval-id="' + marker + '"]';
        }
        const row = {
