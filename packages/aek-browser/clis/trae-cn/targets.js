...
function normalizeEndpoint(value) {
  const endpoint = String(value || process.env.AEKB_CDP_ENDPOINT || 'http://127.0.0.1:39240').trim();
  if (!endpoint) {
    throw new ArgumentError('Set AEKB_CDP_ENDPOINT, for example http://127.0.0.1:39240');
  }
...
export const targetsCommand = cli({
  ...
  example: 'AEKB_CDP_ENDPOINT=http://127.0.0.1:39240 AEK Browser trae-cn targets -f table',
  strategy: Strategy.LOCAL,
  browser: false,
  args: [
    { name: 'endpoint', type: 'string', required: false, help: 'CDP endpoint (default: AEKB_CDP_ENDPOINT)' },
  ...
  func: async (kwargs) => {
    ...
    const wanted = process.env.AEKB_CDP_TARGET || '';
