...
import { TimeoutError } from '@cheezmil/aek-browser/errors';
import {
  activityScript,
  approveTraePrompts,
  isActivityComplete,
...
export const askCommand = cli({
  ...
  example: 'AEKB_CDP_ENDPOINT=http://127.0.0.1:39240 AEKB_CDP_TARGET=talk AEK Browser trae-cn ask "请只回复 OK" --timeout 120 -f json',
  domain: 'localhost',
    ...
    if (latest?.Text) {
      ...
      if (blocked) {
        throw new TimeoutError(
          'trae-cn ask',
          timeout,
          `Current status=${latestActivity?.Status ?? ''}, approvalPending=${latestActivity?.ApprovalPending ?? ''}, approvalKind=${latestActivity?.ApprovalKind ?? ''}, approvalButton=${latestActivity?.ApprovalButton ?? ''}. Use "AEK Browser trae-cn targets -f json" to find the waiting target, then run "AEKB_CDP_TARGET=<target> AEK Browser trae-cn watch --stream true --duration 300" or "AEK Browser trae-cn approve".`,
        );
      }
      return [latest];
    }
