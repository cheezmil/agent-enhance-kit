...
import { ArgumentError, CommandExecutionError, TimeoutError } from '@cheezmil/aek-browser/errors';
...
describe('trae-cn utils', () => {
  it('extracts user and assistant turns from Trae CN DOM', () => {
    const rows = evaluateInDom(`
      ...
      <section class="chat-turn assistant task" data-role="assistant" data-turn-index="2" data-message-id="a1">
        ...
        <div class="chat-markdown"><p class="chat-markdown-p">TRAE_AEKB_SMOKE_OK</p></div>
    ...
    expect(rows).toEqual([
      ...
      { Role: 'Assistant', Text: 'TRAE_AEKB_SMOKE_OK', TextChars: 18, Truncated: 'no', TurnIndex: '2', MessageId: 'a1' },
    ]);
  });
  ...
  it('detects and clicks terminal approval prompts', () => {
    const dom = new JSDOM(`
      <div role="dialog">
        <p>Trae 请求运行终端命令：printf AEKB_APPROVE</p>
...
describe('trae-cn commands', () => {
  it('keeps the first PR command surface focused on the desktop adapter loop', async () => {
    ...
    expect(setupText).toContain('export AEKB_CDP_TARGET="talk"');
    expect(setupText).toContain('AEK Browser trae-cn approve --approve-kinds terminal,delete -f json');
    expect(setupText).toContain('AEK Browser trae-cn watch --stream true --duration 120 --auto-approve true');
  });
  ...
  it('documents endpoint/target examples for browser commands', () => {
    for (const command of [activityCommand, approveCommand, askCommand, modelCommand, newCommand, readCommand, selectModelCommand, sendCommand, statusCommand, watchCommand]) {
      expect(command.example).toContain('AEKB_CDP_ENDPOINT=http://127.0.0.1:39240');
      expect(command.example).toContain('AEKB_CDP_TARGET=');
    }
    expect(targetsCommand.example).toContain('AEKB_CDP_ENDPOINT=http://127.0.0.1:39240');
  });
  ...
  it('approve fails closed when the native approval click fails', async () => {
    const page = {
      evaluate: vi.fn().mockResolvedValueOnce([
        { Status: 'Detected', Kind: 'terminal', Button: '运行', Prompt: '运行终端命令', Selector: '[data-aekb-approval-id="x"]', Action: 'detected' },
      ]),
