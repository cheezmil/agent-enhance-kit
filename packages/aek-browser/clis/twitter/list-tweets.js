...
import { BROWSER_JSON_SNIFF_FN, throwIfLoginWall } from '@cheezmil/aek-browser/utils';
...
cli({
    ...
    browser: true,
    args: [
        { name: 'listId', positional: true, type: 'string', required: true, help: 'Numeric ID of a Twitter/X list (e.g. from `aekb twitter lists`)' },
    ...
    func: async (page, kwargs) => {
        ...
        if (!listId || !/^\d+$/.test(listId)) {
            throw new CommandExecutionError(`Invalid listId: ${JSON.stringify(kwargs.listId)}. Expected a numeric ID (see \`aekb twitter lists\`).`);
        }
        ...
        // AEK Browser >=1.7.x wraps primitive page.evaluate returns as { session, data: <value> }.
