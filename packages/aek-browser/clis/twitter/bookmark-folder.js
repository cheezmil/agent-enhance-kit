...
cli({
    ...
    access: 'read',
    description: 'Read the tweets inside a single Twitter/X bookmark folder. Get the folder id from `aekb twitter bookmark-folders`.',
    domain: 'x.com',
    strategy: Strategy.COOKIE,
    browser: true,
    args: [
        { name: 'folder-id', positional: true, type: 'string', required: true, help: 'Folder id from `aekb twitter bookmark-folders`.' },
    ...
    func: async (page, kwargs) => {
        ...
        if (!folderId || !FOLDER_ID_PATTERN.test(folderId)) {
            throw new ArgumentError(
                `Invalid folder-id: ${JSON.stringify(kwargs['folder-id'])}. Expected a safe folder ID from \`aekb twitter bookmark-folders\`.`,
            );
        }
