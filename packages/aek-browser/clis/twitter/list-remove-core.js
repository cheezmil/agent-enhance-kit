...
export async function listRemoveUser(page, kwargs) {
        ...
        const uiResult = unwrapBrowserResult(await page.evaluate(`(async () => {
            ...
            try {
                if (!window.__aekbListMutations) {
                    window.__aekbListMutations = [];
                    ...
                    window.fetch = async function(...args) {
                        ...
                        try { resp = await origFetch(...args); } catch (err) {
                            if (/ListAddMember|ListRemoveMember|lists\\/members\\/(create|destroy)/.test(url)) {
                                window.__aekbListMutations.push({ url, method, status: 0, error: String(err), ts: Date.now() });
                            }
                            throw err;
                        }
                        if (/ListAddMember|ListRemoveMember|lists\\/members\\/(create|destroy)/.test(url)) {
                            window.__aekbListMutations.push({ url, method, status: resp.status, ts: Date.now() });
                        }
                        return resp;
                    };
                }
                window.__aekbListMutations.length = 0;
                ...
                return {
                    ...
                    mutationsBefore: window.__aekbListMutations.length,
                };
            } catch (e) {
