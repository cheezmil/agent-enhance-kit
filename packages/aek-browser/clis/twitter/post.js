...
import { CommandExecutionError, TimeoutError } from '@cheezmil/aek-browser/errors';
    ...
    const clickResult = requirePostActionResult(await page.evaluate(`(async () => {
        try {
            ...
            for (const toast of Array.from(document.querySelectorAll('[role="alert"], [data-testid="toast"]'))) {
                if (visible(toast)) toast.setAttribute('data-aekb-before-submit-toast', 'true');
            }
    ...
    const result = requirePostActionResult(await page.evaluate(`(async () => {
        ...
        for (let i = 0; i < ${JSON.stringify(iterations)}; i++) {
            ...
            const toasts = Array.from(document.querySelectorAll('[role="alert"], [data-testid="toast"]'))
                .filter((el) => visible(el) && !el.hasAttribute('data-aekb-before-submit-toast'));
...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (result?.unconfirmed) {
            ...
            throw new TimeoutError('twitter post', SUBMIT_TIMEOUT_MS / 1000, `${result.message} Check \`aekb twitter tweets --limit 1\` before retrying; the post may already be live.`);
        }
        if (!result?.ok) {
