...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (!Number.isInteger(limit) || limit <= 0) {
            throw new ArgumentError('twitter following --limit must be a positive integer', 'Example: AEK Browser twitter following @elonmusk --limit 200');
        }
        const rawUser = String(kwargs.user ?? '').trim();
        let targetUser = normalizeScreenName(rawUser);
        if (rawUser && !targetUser) {
            throw new ArgumentError('twitter following user must be a valid Twitter/X handle', 'Example: AEK Browser twitter following @elonmusk --limit 200');
        }
        ...
        if (!targetUser) {
            throw new ArgumentError('twitter following user cannot be empty', 'Example: AEK Browser twitter following @elonmusk --limit 200');
        }
