...
export async function listAddUser(page, kwargs) {
        ...
        if (!listId || !/^\d+$/.test(listId)) {
            throw new ArgumentError(`Invalid listId: ${JSON.stringify(kwargs.listId)}. Expected numeric ID.`, 'Example: AEK Browser twitter list-add 123456789 alice');
        }
        if (!username) {
            throw new ArgumentError('twitter list-add username is required', 'Example: AEK Browser twitter list-add 123456789 alice');
        }
        ...
        // AEK Browser >=1.7.x wraps page.evaluate return values as { session, data }.
        ...
        // Don't unwrap listsData: AEK Browser spreads GraphQL response to top-level + adds session;
