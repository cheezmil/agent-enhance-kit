import { ArgumentError, AuthRequiredError, EmptyResultError, CommandExecutionError, TimeoutError } from '@cheezmil/aek-browser/errors';
...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (rawUser && !targetUser) {
            throw new ArgumentError('twitter followers user must be a valid Twitter/X handle', 'Example: AEK Browser twitter followers @elonmusk --limit 100');
        }
        ...
        if (!targetUser) {
            throw new ArgumentError('twitter followers user cannot be empty', 'Example: AEK Browser twitter followers @elonmusk --limit 100');
        }
