 ...
 *
 * Usage:
 *   AEK Browser twitter download elonmusk --limit 50 --output ./twitter
 *   AEK Browser twitter download --tweet-url https://x.com/xxx/status/123 --output ./twitter
 */
import { cli, Strategy } from '@cheezmil/aek-browser/registry';
import { ArgumentError, AuthRequiredError, CliError, CommandExecutionError, EmptyResultError } from '@cheezmil/aek-browser/errors';
import { formatCookieHeader } from '@cheezmil/aek-browser/download';
import { downloadMedia } from '@cheezmil/aek-browser/download/media-download';
import {
...
cli({
    ...
    func: async (page, kwargs) => {
        try {
            ...
            if (!username) {
                throw new ArgumentError('twitter download username must be a valid Twitter/X handle', 'Example: AEK Browser twitter download @jack --limit 20');
            }
