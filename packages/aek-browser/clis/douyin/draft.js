...
async function prepareCustomCoverInput(page) {
    ...
    for (let attempt = 0; attempt < COVER_INPUT_WAIT_ATTEMPTS; attempt += 1) {
      ...
      document
        .querySelectorAll('[data-aekb-cover-input="1"]')
        .forEach((el) => el.removeAttribute('data-aekb-cover-input'));
      target.setAttribute('data-aekb-cover-input', '1');
      return { ok: true, selector: '[data-aekb-cover-input="1"]' };
    }`));
