...
function requirePositiveInt(value, label, defaultValue, maxValue) {
    ...
    if (!Number.isInteger(parsed) || parsed <= 0) {
        throw new ArgumentError(`${label} must be a positive integer`, `Example: AEK Browser tiktok creator-videos --${label} ${defaultValue}`);
    }
    if (parsed > maxValue) {
        throw new ArgumentError(`${label} must be <= ${maxValue}`, `Example: AEK Browser tiktok creator-videos --${label} ${maxValue}`);
    }
    return parsed;
}
...
function requireCursor(value) {
    ...
    if (!/^\d+$/.test(text)) {
        throw new ArgumentError('cursor must be a non-negative integer string', 'Example: AEK Browser tiktok creator-videos --cursor 0');
    }
    const cursor = Number(text);
    if (!Number.isSafeInteger(cursor)) {
        throw new ArgumentError('cursor must be a safe integer', 'Example: AEK Browser tiktok creator-videos --cursor 0');
    }
    return cursor;
}
