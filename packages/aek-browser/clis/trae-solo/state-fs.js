...
function resolveStateDb(args) {
    ...
    if (!fs.existsSync(db)) {
        throw new CommandExecutionError(
            ...
            'List valid workspace ids with `aekb trae-solo workspaces-list`.',
        );
    }
    return db;
}
...
cli({
    ...
    func: async (args) => {
        ...
        if (val === null) {
            throw new CommandExecutionError(`Key not found: ${key}`, 'List available keys with `aekb trae-solo state-keys`.');
        }
