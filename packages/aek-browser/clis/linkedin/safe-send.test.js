...
function makeFakePage(probe) {
  ...
  return {
    ...
    evaluate: vi.fn(async (script) => {
      ...
      if (text.includes('__AEKB_LINKEDIN_PROBE__')) return probe;
      if (text.includes('__AEKB_LINKEDIN_FOCUS_COMPOSER__')) return { ok: true, composerText: '' };
      if (text.includes('__AEKB_LINKEDIN_READ_COMPOSER__')) return { ok: true, composerText };
      if (text.includes('__AEKB_LINKEDIN_CLICK_SEND__')) return { ok: true, sent: true };
      return undefined;
    }),
