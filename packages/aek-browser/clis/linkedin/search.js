...
async function enrichJobDetails(page, jobs) {
    ...
    for (let i = 0; i < jobs.length; i++) {
        const job = jobs[i];
        console.error(`[aekb:linkedin] Fetching details ${i + 1}/${jobs.length}: ${job.title}`);
        if (!job.url) {
            const reason = 'no url';
            console.error(`[aekb:linkedin] Skipping detail for "${job.title}": ${reason}`);
        ...
        return { description, applyUrl: applyLink?.href || '' };
            ...
            console.error(`[aekb:linkedin] Detail fetch failed for ${job.url}: ${reason}`);
            enriched.push({ ...job, description: null, apply_url: null, detail_error: reason });
        }
    }
    return enriched;
