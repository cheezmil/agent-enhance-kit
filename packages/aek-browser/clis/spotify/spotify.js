...
async function getToken() {
    ...
    if (!tokens)
        throw new CliError('AUTH_REQUIRED', 'Not authenticated. Run: AEK Browser spotify auth');
    if (!tokens.access_token || !tokens.refresh_token || !(tokens.expires_at > 0)) {
        throw new CliError('AUTH_CORRUPTED', 'Token file is corrupted. Run: AEK Browser spotify auth');
    }
