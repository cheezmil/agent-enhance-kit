  ...
  if (RESERVED_PATH_ROOTS.has(username.toLowerCase())) {
    throw new ArgumentError(
      ...
      username.toLowerCase() === 'pin'
        ? 'Pass the board this pin lives on, e.g. janedoe/my-board (`aekb pinterest pin <id>` reports it)'
...
export async function resolveBoardTarget(page, raw) {
  ...
  if (!url) {
    throw new ArgumentError(`No board with id "${trimmed}"`, 'Check the id with `aekb pinterest user-boards <username>`');
  }
...
export async function resolveSection(page, boardId, sectionValue, sourceUrl) {
  ...
  if (sections.length === 0) {
    throw new ArgumentError(`Board has no sections, so --section "${wanted}" cannot be used`, 'Create one first with `aekb pinterest board-section-create`');
  }
...
export async function movePinToSection(page, pinId, boardId, sectionId, sourceUrl) {
  try {
    ...
    throw new CommandExecutionError(
      ...
      `The pin is on the board root — move it with \`aekb pinterest pin-update ${pinId} --board <board> --section ${sectionId}\``,
    );
  }
}
