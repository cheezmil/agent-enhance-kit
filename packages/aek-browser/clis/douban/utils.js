 ...
 */
import { ArgumentError, CliError, EmptyResultError } from '@cheezmil/aek-browser/errors';
...
async function ensureDoubanReady(page) {
  ...
  `);
    if (state?.blocked) {
        throw new CliError('AUTH_REQUIRED', 'Douban requires a logged-in browser session before these commands can load data.', 'Please sign in to douban.com in the browser that AEK Browser reuses, then rerun the command.');
    }
}
