...
describe('weibo read adapters Browser Bridge envelopes', () => {
  it('unwraps comments, feed, hot, search, and favorites array payloads', async () => {
    ...
    await expect(getRegistry().get('weibo/hot').func(
      makePage([envelope([{ rank: 1, word: 'aekb', hot_value: 1, category: '', label: '', url: 'https://s.weibo.com/weibo?q=aekb' }])]),
      { limit: 1 },
    )).resolves.toHaveLength(1);

    await expect(getRegistry().get('weibo/search').func(
      makePage([envelope([{ id: 'm1', title: 'AEK Browser', author: 'a', time: '', url: 'https://weibo.com/1/m1' }])]),
      { keyword: 'aekb', limit: 1 },
    )).resolves.toEqual([{ rank: 1, id: 'm1', title: 'AEK Browser', author: 'a', time: '', url: 'https://weibo.com/1/m1' }]);
