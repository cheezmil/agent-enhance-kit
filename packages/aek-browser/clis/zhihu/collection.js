...
function validatePositiveInt(value, name) {
  ...
  if (!Number.isInteger(n) || n <= 0) {
    throw new ArgumentError(`zhihu collection --${name} must be a positive integer`, 'Example: AEK Browser zhihu collection 83283292 --limit 20');
  }
  return n;
}
...
function validateNonNegativeInt(value, name) {
  const n = Number(value);
  if (!Number.isInteger(n) || n < 0) {
    throw new ArgumentError(`zhihu collection --${name} must be a non-negative integer`, 'Example: AEK Browser zhihu collection 83283292 --offset 0');
  }
  return n;
}
...
cli({
  ...
  func: async (page, kwargs) => {
    ...
    if (!/^\d+$/.test(collectionId)) {
      throw new ArgumentError('Collection ID must be numeric', 'Example: AEK Browser zhihu collection 83283292');
    }
