...
function requireQuery(value) {
    ...
    if (!query) {
        throw new ArgumentError('zhihu search query must not be empty', 'Example: AEK Browser zhihu search codex');
    }
    return query;
}
...
function requireType(value) {
    ...
    if (!TYPES.includes(type)) {
        throw new ArgumentError(`zhihu search --type must be one of: ${TYPES.join(', ')}`, 'Example: AEK Browser zhihu search codex --type answer');
    }
    return type;
}
