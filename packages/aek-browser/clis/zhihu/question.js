...
import { AuthRequiredError, CliError } from '@cheezmil/aek-browser/errors';
...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (!/^\d+$/.test(questionId)) {
            throw new CliError('INVALID_INPUT', 'Question ID must be numeric', 'Example: AEK Browser zhihu question 123456789');
        }
        ...
        if (sort !== 'default' && sort !== 'created') {
            throw new CliError('INVALID_INPUT', 'Sort must be one of: default, created', 'Example: AEK Browser zhihu question 123456789 --sort created');
        }
