 ...
 *
 * Usage:
 *   AEK Browser web read --url "https://www.anthropic.com/research/..." --output ./articles
 *   AEK Browser web read --url "https://..." --download-images false
 */
import { cli, Strategy } from '@cheezmil/aek-browser/registry';
import { downloadArticle } from '@cheezmil/aek-browser/download/article-download';
      ...
      (() => {
        ...
        const collectEmptyContainers = (root, scope, baseUrl) => {
          ...
          root.querySelectorAll?.(likely).forEach((el) => {
            if (scope === 'main' && el.closest?.('[data-aekb-iframe-source]')) return;
        ...
        const buildFrameSection = (frameBody, desc, fallbackLabel) => {
          ...
          section.setAttribute('data-aekb-iframe-source', desc.src);
...
const command = cli({
    ...
    func: async (page, kwargs, debug = false) => {
        ...
        const shouldDiagnose = boolish(kwargs.diagnose) || debug || !!process.env.AEKB_VERBOSE;
