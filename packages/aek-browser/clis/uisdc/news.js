...
function normalizeLimit(value) {
    ...
    if (!Number.isInteger(limit) || limit <= 0) {
        throw new ArgumentError('limit must be a positive integer', `Example: AEK Browser uisdc news --limit ${DEFAULT_LIMIT}`);
    }
    if (limit > MAX_LIMIT) {
        throw new ArgumentError(`limit must be <= ${MAX_LIMIT}`, `Example: AEK Browser uisdc news --limit ${MAX_LIMIT}`);
    }
    return limit;
}
