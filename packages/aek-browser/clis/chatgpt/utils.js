 ...
 * Cross-platform: works on Linux/macOS/Windows via AEK Browser's CDP browser automation.
 */

import { htmlToMarkdown } from '@cheezmil/aek-browser/utils';
import { ArgumentError, AuthRequiredError, CommandExecutionError, TimeoutError } from '@cheezmil/aek-browser/errors';
...
function debugChatGPTModel(message) {
    if (process?.env?.AEKB_CHATGPT_MODEL_DEBUG) {
...
function buildComposerLocatorScript() {
    const markerAttr = 'data-aekb-chatgpt-composer';
    return `
      const isVisible = (el) => {
...
export function requireNonEmptyPrompt(prompt, commandName) {
    ...
    if (!text) {
        throw new ArgumentError(
            ...
            `Example: AEK Browser ${commandName} "hello"`,
        );
    }
    return text;
}
...
export function parseChatGPTConversationId(value) {
    ...
    if (/^https?:\/\//i.test(raw)) {
        ...
        throw new ArgumentError(
            ...
            'Example: AEK Browser chatgpt detail https://chatgpt.com/c/123e4567-e89b-12d3-a456-426614174000',
        );
    }
    ...
    throw new ArgumentError(
        'chatgpt detail requires a conversation id or chatgpt.com /c/<id> URL',
        'Example: AEK Browser chatgpt detail 123e4567-e89b-12d3-a456-426614174000',
    );
}
...
export function parseChatGPTProjectId(value) {
    ...
    if (/^https?:\/\//i.test(raw) || raw.startsWith('/')) {
        ...
        throw new ArgumentError(
            ...
            'Example: AEK Browser chatgpt project-file-add report.pdf --id 12345678',
        );
    }
