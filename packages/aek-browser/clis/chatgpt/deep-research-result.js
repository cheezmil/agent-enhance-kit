...
export const deepResearchResultCommand = cli({
    ...
    func: async (page, kwargs) => {
        ...
        const timeout = requirePositiveInt(
            ...
            'Example: AEK Browser chatgpt deep-research-result <id> --wait true --timeout 600',
        );
        const stableSeconds = requireNonNegativeInt(
            Number(kwargs.stable ?? 6),
            'chatgpt deep-research-result --stable',
            'Example: AEK Browser chatgpt deep-research-result <id> --wait true --stable 6',
        );
        ...
        if (currentUrl.startsWith(targetUrl)) {
            await page.goto(`${CHATGPT_URL}/?aekb_dr_result=${Date.now()}`, { waitUntil: 'none' });
            await page.wait(1);
        }
