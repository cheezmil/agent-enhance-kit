...
describe('yahoo search', () => {
  ...
  it('rejects empty query, invalid limit, and invalid page before navigation', async () => {
    ...
    await expect(command.func(page, { keyword: 'aekb', limit: 8 })).rejects.toMatchObject({ code: 'ARGUMENT' });
    await expect(command.func(page, { keyword: 'aekb', limit: 5, page: 0 })).rejects.toMatchObject({ code: 'ARGUMENT' });
  ...
  it('decodes Yahoo redirect URLs and assigns listing rank', async () => {
    const page = createPageMock({
      ...
      data: [[
        'AEK Browser',
        'https://r.search.yahoo.com/_ylt=x/RU=https%3A%2F%2Fgithub.com%2Fcheezmil%2Faekb/RK=2/RS=x',
        'CLI browser tooling',
      ]],
    });

    await expect(command.func(page, { keyword: 'aekb', limit: 1, page: 2 })).resolves.toEqual([{
      rank: 8,
      title: 'AEK Browser',
