...
import { ArgumentError, CommandExecutionError, EmptyResultError, getErrorMessage } from '@cheezmil/aek-browser/errors';
...
function normalizeLimit(value) {
    ...
    if (!Number.isInteger(limit) || limit <= 0) {
        throw new ArgumentError('limit must be a positive integer', `Example: AEK Browser aibase news --limit ${DEFAULT_LIMIT}`);
    }
    if (limit > MAX_LIMIT) {
        throw new ArgumentError(`limit must be <= ${MAX_LIMIT}`, `Example: AEK Browser aibase news --limit ${MAX_LIMIT}`);
    }
    return limit;
}
