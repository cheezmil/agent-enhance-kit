...
function validateAskModelValue(value) {
    if (!value) {
        throw new ArgumentError(
            ...
            'Use "AEK Browser gemini models" to list available values.'
        );
    }
...
export const askCommand = cli({
    ...
    args: [
        ...
        { name: 'model', type: 'string', required: false, help: 'Gemini model to use (e.g. "2.5-flash"). Use "AEK Browser gemini models" to list available values.' },
    ...
    func: async (page, kwargs) => {
        ...
        if (hasThinking) {
            ...
            if (thinkingValue !== 'standard' && thinkingValue !== 'extended') {
                throw new ArgumentError(
                    ...
                    'Run `aekb gemini models` to see available thinking levels.',
                );
            }
        }
        ...
        if (hasModel) {
            ...
            if (!availableIds.includes(modelValue)) {
                throw new ArgumentError(
                    ...
                    'Use "AEK Browser gemini models" to see available values.'
                );
            }
        ...
        if (hasThinking) {
            ...
            if (targetModelThinking) {
                ...
                if (!targetModelThinking.includes(thinkingValue)) {
                    ...
                    throw new ArgumentError(
                        ...
                        `Model '${targetModelId}' supports: ${availableForModel}. Run \`aekb gemini models\` for all models.`,
                    );
                }
                ...
                throw new ArgumentError(
                    ...
                    `Available thinking values: ${availableList}. Run \`aekb gemini models\` for details.`,
                );
            }
            ...
            if (!selected) {
                ...
                hintParts.push('Run `aekb gemini models` for details.');
                throw new ArgumentError(
