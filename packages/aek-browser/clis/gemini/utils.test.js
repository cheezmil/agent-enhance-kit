...
describe('sanitizeGeminiResponseText', () => {
    it('strips a prompt echo only when it appears as a prefixed block', () => {
        const prompt = 'Reply with the word aekb';
        const value = `Reply with the word aekb\n\naekb`;
        expect(sanitizeGeminiResponseText(value, prompt)).toBe('aekb');
    });
    it('does not strip prompt text that appears later in a legitimate answer', () => {
        const prompt = 'aekb';
        const value = 'You asked about aekb, and aekb is the right keyword here.';
