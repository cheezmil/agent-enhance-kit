...
describe('validateAskModelValue', () => {
    ...
    it('mentions aekb gemini models in error messages', () => {
        try { validateAskModelValue('pro'); } catch (e) {
            expect(e.message).toContain('AEK Browser gemini models');
        }
...
describe('gemini ask orchestration', () => {
    ...
    it('rejects aliases with error message mentioning canonical ids and aekb gemini models', async () => {
        const page = createPageMock();
    ...
    it('includes available models and suggests aekb gemini models in invalid-model error', async () => {
        const page = createPageMock();
        mocks.ensureGeminiPage.mockResolvedValue(undefined);
        setupDiscoveryWithoutThinking(page);
