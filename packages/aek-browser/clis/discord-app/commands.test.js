...
import { CommandExecutionError, EmptyResultError } from '@cheezmil/aek-browser/errors';
...
function createRoutePage({ route, rows = [] }) {
    return {
        ...
        evaluate: vi.fn(async (script) => {
            if (script.includes('__aekbDiscordRouteState')) {
                return {
                    url: route.url,
                    route: {
            ...
            if (script.includes('__aekbDiscordReadMessages')) return rows;
...
describe('discord-app list row validation', () => {
    it('unwraps Browser Bridge envelopes for server rows', async () => {
        const page = {
            evaluate: vi.fn().mockResolvedValue({
                ...
                data: [{ Server: 'AEK Browser', guild_id: '111', url: 'https://discord.com/channels/111' }],
            }),
        };

        await expect(listDiscordServers(page)).resolves.toEqual([
            { Server: 'AEK Browser', guild_id: '111', url: 'https://discord.com/channels/111' },
        ]);
    });
    ...
    it('typed-fails server rows missing stable guild identity', async () => {
        const page = {
            evaluate: vi.fn().mockResolvedValue([{ Server: 'AEK Browser', url: 'https://discord.com/channels/111' }]),
        };
...
describe('discord-app targeted reads', () => {
    ...
    it('read unwraps Browser Bridge evaluate envelopes before shape validation', async () => {
        ...
        const page = {
            ...
            evaluate: vi.fn(async (script) => {
                ...
                if (script.includes('__aekbDiscordReadMessages')) {
                    return {
                        session: 'site:discord-app',
    ...
    it('resolves visible channel names from the current sidebar', async () => {
        const page = {
            evaluate: vi.fn(async (script) => {
                if (script.includes('__aekbDiscordListChannels')) {
