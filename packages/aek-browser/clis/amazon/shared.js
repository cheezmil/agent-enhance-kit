import { ArgumentError, CommandExecutionError } from '@cheezmil/aek-browser/errors';
...
const AMAZON_RANKING_SPECS = {
    bestsellers: {
        ...
        invalidInputHint: 'Example: AEK Browser amazon bestsellers https://www.amazon.com/Best-Sellers/zgbs',
    },
    new_releases: {
        ...
        invalidInputHint: 'Example: AEK Browser amazon new-releases https://www.amazon.com/gp/new-releases',
    },
    movers_shakers: {
        ...
        invalidInputHint: 'Example: AEK Browser amazon movers-shakers https://www.amazon.com/gp/movers-and-shakers',
    },
};
...
export function buildProductUrl(input) {
    ...
    if (!asin) {
        throw new ArgumentError('amazon product expects an ASIN or product URL', 'Example: AEK Browser amazon product B0FJS72893');
    }
...
export function buildDiscussionUrl(input) {
    const asin = extractAsin(input);
    if (!asin) {
        throw new ArgumentError('amazon discussion expects an ASIN or product URL', 'Example: AEK Browser amazon discussion B0FJS72893');
    }
    const host = amazonHostFromInput(input);
...
export function buildChallengeHint(action) {
    return [
        ...
        'If you are using CDP, set AEKB_CDP_TARGET=amazon.com and avoid parallel Amazon commands against the same browser target.',
    ].join(' ');
}
