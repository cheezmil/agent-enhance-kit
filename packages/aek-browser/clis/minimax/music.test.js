...
import { ArgumentError, AuthRequiredError, CommandExecutionError, ConfigError, TimeoutError } from '@cheezmil/aek-browser/errors';
import './music.js';
import {
    MINIMAX_API_KEY_VAR,
    cleanupAudioFile,
...
function tempDir() {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'aekb-minimax-'));
    tempDirs.push(dir);
    return dir;
}
