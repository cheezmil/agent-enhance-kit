...
describe('duckduckgo search', () => {
  ...
  it('rejects empty query and out-of-range pagination before navigation', async () => {
    ...
    await expect(command.func(page, { keyword: 'aekb', limit: 11 })).rejects.toMatchObject({ code: 'ARGUMENT' });
    await expect(command.func(page, { keyword: 'aekb', limit: 5, offset: 5 })).rejects.toMatchObject({ code: 'ARGUMENT' });
  ...
  it('decodes DuckDuckGo redirect URLs and assigns listing rank', async () => {
    const page = createPageMock([
      [
        'AEK Browser',
        '/l/?uddg=https%3A%2F%2Fgithub.com%2Fcheezmil%2Faekb',
        'CLI browser tooling',
        'github.com/cheezmil/aekb',
        '',
        'web',
      ],
    ]);

    await expect(command.func(page, { keyword: 'aekb', limit: 1 })).resolves.toEqual([{
      rank: 1,
      title: 'AEK Browser',
      url: 'https://github.com/cheezmil/aekb',
      snippet: 'CLI browser tooling',
      displayUrl: 'github.com/cheezmil/aekb',
      icon: '',
      resultType: 'web',
    }]);
  });
  ...
  it('executes the DOM extractor, filters ads, and returns canonical rows', async () => {
    ...
    await expect(command.func(page, { keyword: 'aekb', limit: 5 })).resolves.toEqual([{
      rank: 1,
  ...
  it('unwraps browser envelopes for paginated extraction', async () => {
    ...
    const result = await command.func(page, { keyword: 'aekb', limit: 1, offset: 10 });
  ...
  it('fails typed instead of returning [] for malformed extraction payloads', async () => {
    ...
    await expect(command.func(page, { keyword: 'aekb', limit: 1 })).rejects.toMatchObject({
      code: 'COMMAND_EXEC',
