import { ArgumentError, AuthRequiredError, CommandExecutionError } from '@cheezmil/aek-browser/errors';
...
export function parseSearchLimit(input) {
    ...
    if (!Number.isFinite(parsed) || parsed < 1) {
        throw new ArgumentError('1688 search --limit must be a positive integer', 'Example: AEK Browser 1688 search "桌面置物架" --limit 20');
    }
...
export function buildSearchUrl(query) {
    ...
    if (!normalized) {
        throw new ArgumentError('1688 search query cannot be empty', 'Example: AEK Browser 1688 search "桌面置物架" --limit 20');
    }
...
export function buildDetailUrl(input) {
    ...
    if (!offerId) {
        throw new ArgumentError('1688 item expects an offer URL or offer ID', 'Example: AEK Browser 1688 item 887904326744');
    }
...
export function resolveStoreUrl(input) {
    ...
    if (!normalized) {
        throw new ArgumentError('1688 store expects a store URL or member ID', 'Example: AEK Browser 1688 store https://yinuoweierfushi.1688.com/');
    }
    ...
    throw new ArgumentError('1688 store expects a store URL or member ID', 'Example: AEK Browser 1688 store b2b-22154705262941f196');
}
export function canonicalizeStoreUrl(input) {
    ...
    if (!host) {
        throw new ArgumentError('Invalid 1688 store URL', 'Example: AEK Browser 1688 store https://yinuoweierfushi.1688.com/');
    }
    return `https://${host}`;
}
...
export function buildCaptchaHint(action) {
    return [
        ...
        'If you run AEK Browser via CDP, set AEKB_CDP_TARGET=1688.com or a more specific 1688 host before retrying.',
    ].join(' ');
}
...
export async function gotoAndReadState(page, url, settleMs = 2500, action = 'page') {
    try {
        ...
        if (message.includes('Inspected target navigated or closed')
            ...
            throw new CommandExecutionError(`1688 ${action} navigation lost the current browser target`, `${buildCaptchaHint(action)} If CDP is attached to a stale or blocked tab, open a fresh 1688 tab and point AEKB_CDP_TARGET at that tab.`);
        }
        throw error;
    }
}
