...
function validateInstagramNoteArgs(kwargs) {
    if (kwargs.content === undefined) {
        throw new ArgumentError('Argument "content" is required.', 'Provide a note text, for example: AEK Browser instagram note "hello"');
    }
}
function normalizeInstagramNoteContent(kwargs) {
    const content = String(kwargs.content ?? '').trim();
    if (!content) {
        throw new ArgumentError('Instagram note content cannot be empty.', 'Provide a non-empty note text, for example: AEK Browser instagram note "hello"');
    }
