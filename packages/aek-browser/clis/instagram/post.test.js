...
import { AuthRequiredError, CommandExecutionError } from '@cheezmil/aek-browser/errors';
...
function createTempImage(name = 'demo.jpg', bytes = Buffer.from([0xff, 0xd8, 0xff, 0xd9])) {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'aekb-instagram-post-'));
    tempDirs.push(dir);
...
function createTempVideo(name = 'demo.mp4', bytes = Buffer.from('video')) {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'aekb-instagram-post-video-'));
    tempDirs.push(dir);
    const filePath = path.join(dir, name);
    fs.writeFileSync(filePath, bytes);
    return filePath;
...
afterAll(() => {
    ...
    delete process.env.AEKB_INSTAGRAM_CAPTURE;
});
...
describe('instagram post registration', () => {
    ...
    it('prefers the private route by default and returns without touching UI upload steps when private publish succeeds', async () => {
        ...
        const evaluate = vi.fn(async (js) => {
            ...
            if (js.includes('window.location?.pathname'))
                return { ok: true };
            if (js.includes('const data = Array.isArray(window[') && js.includes('__aekb_ig_protocol_capture'))
                return { data: [], errors: [] };
            return { ok: true };
        });
    ...
    it('falls back to the UI route when the default private route fails safely before publishing', async () => {
        ...
        const evaluate = vi.fn(async (js) => {
            ...
            if (js.includes('window.location?.pathname'))
                return { ok: true };
            if (js.includes('data-aekb-ig-upload-index'))
                return { ok: true, selectors: ['[data-aekb-ig-upload-index="0"]'] };
        ...
        expect(privateSpy).toHaveBeenCalledTimes(1);
        expect(page.setFileInput).toHaveBeenCalledWith([imagePath], '[data-aekb-ig-upload-index="0"]');
        expect(result).toEqual([
            {
                status: '✅ Posted',
                detail: 'Single image post shared successfully',
    ...
    it('falls back to the UI route for mixed-media posts when the private route fails safely before publishing', async () => {
        ...
        expect(privateSpy).toHaveBeenCalledTimes(1);
        expect(page.setFileInput).toHaveBeenCalledWith([imagePath, videoPath], '[data-aekb-ig-upload-index="0"]');
        expect(result).toEqual([
            {
                status: '✅ Posted',
    ...
    it('uploads a single image, fills caption, and shares the post', async () => {
        ...
        const page = createPageMock(withInitialDialogDismiss([
            { ok: true },
            { ok: true, selectors: ['[data-aekb-ig-upload-index="0"]'] },
            { ok: true },
            { ok: true },
            { ok: false },
            { ok: true, label: 'Next' },
    ...
    it('uploads multiple images as a carousel and shares the post', async () => {
        ...
        expect(page.setFileInput).toHaveBeenCalledWith([firstImagePath, secondImagePath], '[data-aekb-ig-upload-index="0"]');
        expect(result).toEqual([
            {
                status: '✅ Posted',
    ...
    it('installs and dumps protocol capture when AEKB_INSTAGRAM_CAPTURE is enabled', async () => {
        process.env.AEKB_INSTAGRAM_CAPTURE = '1';
        const imagePath = createTempImage('capture-enabled.jpg');
        const evaluate = vi.fn(async (js) => {
            if (js.includes('__aekb_ig_protocol_capture') && js.includes('PATCH_GUARD'))
                return { ok: true };
            if (js.includes('const data = Array.isArray(window[') && js.includes('__aekb_ig_protocol_capture')) {
                return { data: [], errors: [] };
            }
        ...
        expect(evaluateCalls.some((js) => js.includes('__aekb_ig_protocol_capture') && js.includes('PATCH_GUARD'))).toBe(true);
        expect(evaluateCalls.some((js) => js.includes('const data = Array.isArray(window[') && js.includes('__aekb_ig_protocol_capture'))).toBe(true);
        expect(result).toEqual([
            {
                status: '✅ Posted',
                detail: 'Single image post shared successfully',
    ...
    it('falls back to browser-side file injection when the extension does not support set-file-input', async () => {
        ...
        const evaluate = vi.fn(async (js) => {
            ...
            if (js.includes('data-aekb-ig-upload-index'))
                return { ok: true, selectors: ['[data-aekb-ig-upload-index="0"]'] };
            if (js.includes('__aekbInstagramUpload_') && js.includes('] = [];'))
                return { ok: true };
    ...
    it('forces a fresh home reload before retrying after an upload-stage error', async () => {
        ...
        const evaluate = vi.fn(async (js) => {
            ...
            if (js.includes('const hasPreviewUi =')) {
                ...
                return gotoUrls.some((url) => url.includes('__aekb_reset='))
                    ? { ok: true, state: 'preview' }
                    : { ok: false, state: 'failed', detail: 'Something went wrong. Please try again.' };
            }
            if (js.includes('button[aria-label="Close"]'))
        ...
        expect(gotoUrls.some((url) => url.includes('__aekb_reset='))).toBe(true);
        expect(result).toEqual([
            {
                status: '✅ Posted',
                detail: 'Single image post shared successfully',
    ...
    it('re-resolves the upload input when the tagged selector goes stale before setFileInput runs', async () => {
        ...
        const setFileInput = vi.fn()
            .mockRejectedValueOnce(new Error('No element found matching selector: [data-aekb-ig-upload-index="0"]'))
    ...
    it('tries the next upload input when the first candidate never opens the preview', async () => {
        ...
        const page = createPageMock(withInitialDialogDismiss([
            { ok: true },
            { ok: true, selectors: ['[data-aekb-ig-upload-index="0"]', '[data-aekb-ig-upload-index="1"]'] },
            { ok: true },
            { ok: false },
            { ok: false },
            { ok: false },
        ...
        expect(setFileInput).toHaveBeenNthCalledWith(1, [imagePath], '[data-aekb-ig-upload-index="0"]');
        expect(setFileInput).toHaveBeenNthCalledWith(2, [imagePath], '[data-aekb-ig-upload-index="1"]');
        expect(result).toEqual([
            {
                status: '✅ Posted',
                detail: 'Single image post shared successfully',
