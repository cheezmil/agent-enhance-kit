...
import { Page as BrowserPage } from '@cheezmil/aek-browser/browser/page';
...
function buildSafeTempVideoPath(filePath) {
    ...
    return path.join(os.tmpdir(), `aekb-instagram-video-real${ext}`);
}
    ...
    (() => {
      ...
      for (const root of roots) {
        ...
        for (const input of inputs) {
          ...
          input.setAttribute('data-aekb-reel-upload-index', String(index));
          selectors.push('[data-aekb-reel-upload-index="' + index + '"]');
          index += 1;
        }
      }
