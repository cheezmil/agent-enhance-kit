...
export async function resolveInstagramPrivatePublishConfig(page) {
    ...
    for (let attempt = 0; attempt < INSTAGRAM_PRIVATE_CONFIG_RETRY_BUDGET; attempt += 1) {
        try {
            ...
            await page.goto(`${INSTAGRAM_HOME_URL}?__aekb_private_probe=${Date.now()}`);
...
function buildPrivateNormalizedImagePath(filePath) {
    ...
    return path.join(os.tmpdir(), `aekb-instagram-private-${parsed.name}-${crypto.randomUUID()}${parsed.ext || '.png'}`);
}
...
function runSwiftJsonScript(script, args, stage) {
    return path.join(os.tmpdir(), `aekb-instagram-${crypto.randomUUID()}.swift`);
...
function buildPrivateVideoCoverPath(filePath) {
    const parsed = path.parse(filePath);
    return path.join(os.tmpdir(), `aekb-instagram-private-video-cover-${parsed.name}-${crypto.randomUUID()}.jpg`);
}
function buildPrivateStoryVideoPath(filePath) {
    const parsed = path.parse(filePath);
    return path.join(os.tmpdir(), `aekb-instagram-story-video-${parsed.name}-${crypto.randomUUID()}${parsed.ext || '.mp4'}`);
}
