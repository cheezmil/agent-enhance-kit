...
const DEFAULT_CAPTURE_VAR = '__aekb_ig_protocol_capture';
const DEFAULT_CAPTURE_ERRORS_VAR = '__aekb_ig_protocol_capture_errors';
...
export function buildInstallInstagramProtocolCaptureJs(captureVar = DEFAULT_CAPTURE_VAR, captureErrorsVar = DEFAULT_CAPTURE_ERRORS_VAR) {
    ...
    (() => {
      ...
      XMLHttpRequest.prototype.setRequestHeader = function(name, value) {
        try {
          this.__aekb_headers = this.__aekb_headers || {};
          this.__aekb_headers[String(name)] = String(value);
        } catch {}
        return origSetRequestHeader.apply(this, arguments);
      };
      XMLHttpRequest.prototype.send = function(body) {
        this.addEventListener('load', () => {
          if (!shouldCapture(this.__aekb_url)) return;
          try {
            window[CAPTURE_VAR].push({
              kind: 'xhr',
              url: String(this.__aekb_url || ''),
              method: String(this.__aekb_method || 'GET').toUpperCase(),
              requestHeaders: this.__aekb_headers || {},
...
export async function dumpInstagramProtocolCaptureIfEnabled(page) {
    if (process.env.AEKB_INSTAGRAM_CAPTURE !== '1')
        return;
