...
async function findUploadSelectors(page, mediaItems) {
    ...
    ((includesVideo) => {
      ...
      document.querySelectorAll('[data-aekb-ig-upload-index]').forEach((el) => el.removeAttribute('data-aekb-ig-upload-index'));
      const selectors = ordered.map((input, index) => {
        input.setAttribute('data-aekb-ig-upload-index', String(index));
        return '[data-aekb-ig-upload-index="' + index + '"]';
      });
...
function extractSelectorIndex(selector) {
    const match = selector.match(/data-aekb-ig-upload-index="(\d+)"/);
    if (!match)
        return null;
...
async function injectImageViaBrowser(page, imagePaths, selector) {
    ...
    const chunkKey = `__aekbInstagramUpload_${Date.now()}_${Math.random().toString(36).slice(2)}`;
...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        const protocolCaptureEnabled = process.env.AEKB_INSTAGRAM_CAPTURE === '1';
