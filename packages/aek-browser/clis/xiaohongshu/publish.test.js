...
import { CommandExecutionError, ArgumentError } from '@cheezmil/aek-browser/errors';
...
describe('xiaohongshu publish', () => {
    ...
    it('uses native insertText for contenteditable title fields when available', async () => {
        ...
        const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aekb-xhs-publish-'));
        ...
        const page = createConditionalPageMock((code) => {
            ...
            if (code.includes('const sels =') && code.includes('for (const sel of sels)'))
                return true;
            if (code.includes('__aekb_xhs_fill_phase') && code.includes('"locate"')) {
            ...
            if (code.includes('__aekb_xhs_fill_phase') && code.includes('"prepare"'))
                return { ok: true };
            if (code.includes('__aekb_xhs_fill_phase') && code.includes('"verify"')) {
                return code.includes('[contenteditable="true"][placeholder*="标题"]')
    ...
    it('falls back to in-page insertion when contenteditable native insertText fails', async () => {
        ...
        const page = createConditionalPageMock((code) => {
            ...
            if (code.includes('__aekb_xhs_fill_phase') && code.includes('"prepare"'))
                return { ok: true };
            if (code.includes('__aekb_xhs_fill_phase') && code.includes('"apply"')) {
                return code.includes('[contenteditable="true"][placeholder*="标题"]')
    ...
    it('attaches topics via Enter to accept the inline suggestion (shadow-DOM dropdown)', async () => {
        ...
        const page = createConditionalPageMock((code) => {
            ...
            if (code.includes('__aekb_xhs_topic_marker_count')) {
                markerChecks += 1;
...
describe('xiaohongshu publish 文字配图 validation', () => {
    ...
    it('rejects .gif appended images in text-image mode', async () => {
        const cmd = getCmd();
        const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aekb-xhs-cardgif-'));
    ...
    it('throws ArgumentError for invalid image paths before navigating', async () => {
        ...
        const page = createPageMock([]);
        await expect(
            cmd.func(page, { title: 't', content: 'c', images: '/tmp/aekb-xhs-missing.jpg' })
        ).rejects.toThrow(ArgumentError);
        expect(page.goto).not.toHaveBeenCalled();
    });
    ...
    it('throws ArgumentError for unsupported image extensions before navigating', async () => {
        const cmd = getCmd();
        const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aekb-xhs-badext-'));
...
describe('xiaohongshu publish 文字配图 flow', () => {
    ...
    function createTextImagePage({ insertText, capture }) {
        return createConditionalPageMock((code) => {
            ...
            if (code.includes('__aekb_xhs_click_label')) {
                capture.clicks.push(code.match(/wantLabel:\s*"([^"]+)"/)?.[1] ?? 'unknown');
                return { ok: true };
            }
            // focus active card
            if (code.includes('__aekb_xhs_focus_card'))
                return { ok: true };
            // new-card render poll after 再写一张
            if (code.includes('__aekb_xhs_card_count'))
                return { ok: true, count: 9, activeEmpty: true };
            // 预览图片 step readiness poll after 生成图片
            if (code.includes('__aekb_xhs_preview_ready'))
                return { ok: true };
            // verify a card is non-empty
            if (code.includes('__aekb_xhs_card_text'))
    ...
    it('types one card, generates, and lands on the editor', async () => {
        ...
        const mediaCountCode = page.evaluate.mock.calls.map((args) => String(args[0]))
            .find((code) => code.includes('__aekb_xhs_composer_media_count'));
    ...
    it('appends uploaded images after generating cards (text-image + images)', async () => {
        ...
        const insertText = vi.fn().mockResolvedValue(undefined);
        const setFileInput = vi.fn().mockResolvedValue(undefined);
        const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aekb-xhs-cardimg-'));
        ...
        const page = createConditionalPageMock((code) => {
            ...
            if (code.includes('hasTitleInput') && code.includes('hasVideoSurface'))
                return { state: 'image_surface', hasTitleInput: false, hasImageInput: true, hasVideoSurface: false };
            if (code.includes('__aekb_xhs_click_label')) return { ok: true };
            if (code.includes('__aekb_xhs_focus_card')) return { ok: true };
            if (code.includes('__aekb_xhs_card_count')) return { ok: true, count: 9, activeEmpty: true };
            if (code.includes('__aekb_xhs_preview_ready')) return { ok: true };
            if (code.includes('__aekb_xhs_card_text')) return { ok: true, text: 'x' };
            if (code.includes('__aekb_xhs_composer_media_count')) return { ok: true, count: 2 };
            ...
            if (code.includes('__aekb_xhs_fill_phase') && code.includes('"locate"')) return { ok: true, sel: 'x', kind: 'input' };
            if (code.includes('__aekb_xhs_fill_phase') && code.includes('"apply"')) return { ok: true, actual: code.includes('标题') ? 't' : 'c' };
    ...
    it('fails when a card stays empty after typing', async () => {
        ...
        const page = createConditionalPageMock((code) => {
            ...
            if (code.includes('__aekb_xhs_card_count')) return { ok: true, count: 9, activeEmpty: true };
            if (code.includes('__aekb_xhs_preview_ready')) return { ok: true };
            if (code.includes('__aekb_xhs_card_text')) return { ok: false, text: '' }; // stays empty
            return null;
        }, { insertText });
        await expect(
    ...
    it('selects a non-default card style on the preview step', async () => {
        ...
        const page = createConditionalPageMock((code) => {
            ...
            if (code.includes('__aekb_xhs_composer_media_count')) return { ok: true, count: 1 };
            if (code.includes('const sels =') && code.includes('for (const sel of sels)')) return true;
            if (code.includes('__aekb_xhs_fill_phase') && code.includes('"locate"')) return { ok: true, sel: 'x', kind: 'input' };
            if (code.includes('__aekb_xhs_fill_phase') && code.includes('"apply"')) return { ok: true, actual: code.includes('标题') ? 't' : 'c' };
            if (code.includes('xhs-publish-btn')) return { ok: true, via: 'method', name: '_onPublish' };
    ...
    it('fails when generated cards do not appear as composer media', async () => {
        ...
        const page = createConditionalPageMock((code) => {
            ...
            if (code.includes('__aekb_xhs_preview_ready')) return { ok: true };
            if (code.includes('__aekb_xhs_card_text')) return { ok: true, text: 'x' };
            if (code.includes('__aekb_xhs_composer_media_count')) return { ok: true, count: 0 };
            if (code.includes('const sels =') && code.includes('for (const sel of sels)')) return true;
            return null;
        }, { insertText });
