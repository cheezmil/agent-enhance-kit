 ...
 *
 * Usage:
 *   AEK Browser antigravity serve --port 8082
 *   ANTHROPIC_BASE_URL=http://localhost:8082 claude
 */
import { createServer } from 'node:http';
import { CDPBridge } from '@cheezmil/aek-browser/browser/cdp';
import { resolveElectronEndpoint } from '@cheezmil/aek-browser/launcher';
import { EXIT_CODES, getErrorMessage } from '@cheezmil/aek-browser/errors';
// ─── Helpers ─────────────────────────────────────────────────────────
...
export async function startServe(opts = {}) {
    ...
    const envTimeoutSeconds = parseEnvTimeout('AEKB_ANTIGRAVITY_TIMEOUT', 120);
    ...
    async function ensureConnected() {
        ...
        // If multiple editor windows are open, set AEKB_CDP_TARGET to the window title.
        if (process.env.AEKB_CDP_TARGET) {
            console.error(`[serve] Using AEKB_CDP_TARGET=${process.env.AEKB_CDP_TARGET}`);
        }
        ...
        console.error(`[serve] Connecting via CDP (target pattern: "${process.env.AEKB_CDP_TARGET}")...`);
        cdp = new CDPBridge();
        try {
    ...
    `);
        if (!hasUI) {
            console.error('[serve] ⚠️  Warning: chat UI elements not found in this target. Try setting AEKB_CDP_TARGET to the correct window title.');
        }
        return page;
    }
