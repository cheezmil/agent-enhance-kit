...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (!taskId) {
            throw new CliError('CONFIG', 'task uuid required', 'Pass the work item uuid from AEK Browser ones my-tasks or the URL.');
        }
        ...
        if (!Number.isFinite(hoursHuman) || hoursHuman <= 0 || hoursHuman > 1000) {
            throw new CliError('CONFIG', 'hours must be a positive number (hours)', 'Example: AEK Browser ones worklog <taskUUID> 2 --team <teamUUID>');
        }
