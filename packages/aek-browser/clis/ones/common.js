...
function throwIfOnesPeekBusinessError(apiPath, parsed) {
    ...
    throw new CliError('FETCH_ERROR', `ONES ${apiPath}: ${detail}`, '若 query 不合法会返回 ServerError；可试 AEK Browser ones tasks（空 must）或检查筛选器文档。响应全文可用 -v 或临时打日志。');
}
...
export async function resolveOnesUserUuid(page, opts) {
    ...
    if (!u || typeof u.uuid !== 'string') {
        throw new CliError('FETCH_ERROR', 'Could not read current user uuid from users/me', 'Set ONES_USER_ID or ensure Chrome is logged in; try: AEK Browser ones me -f json');
    }
...
export async function onesFetchInPage(page, apiPath, options = {}) {
    ...
    if (!r.ok) {
        ...
        const hint = r.status === 401
            ? '在 Chrome 中打开 ONES 并登录；或先执行 AEK Browser ones login 后按提示 export ONES_USER_ID / ONES_AUTH_TOKEN；并确认 ONES_BASE_URL 与浏览器地址一致。'
