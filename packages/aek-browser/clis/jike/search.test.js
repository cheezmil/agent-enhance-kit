...
describe('jike search API pagination', () => {
    it('preserves the seven-column contract and maps structured posts', async () => {
        ...
        await expect(command().func(page, { query: 'AEK Browser', limit: 1 })).resolves.toEqual([{
            id: 'post-1',
            author: 'Alice',
            content: 'hello world',
            likes: 4,
        ...
        expect(page.goto).toHaveBeenCalledWith('https://web.okjike.com/search?q=AEK%20Browser');
    ...
    it('does not validate an unused cursor after satisfying --limit', async () => {
        ...
        await expect(command().func(page, { query: 'AEK Browser', limit: 1 })).resolves.toHaveLength(1);
    });
    ...
    it.each([
        ...
        await expect(command().func(page, { query: 'AEK Browser', limit: 20 })).rejects.toBeInstanceOf(ErrorType);
    });
    ...
    it('rejects malformed posts and cursors instead of returning partial rows', async () => {
        ...
        ])), { query: 'AEK Browser', limit: 20 })).rejects.toBeInstanceOf(CommandExecutionError);

        await expect(command().func(makePage(identity, apiPage([], 'bad-cursor')), {
            query: 'AEK Browser', limit: 20,
    ...
    it('rejects repeated cursors and page-budget exhaustion without returning accumulated rows', async () => {
        ...
        await expect(command().func(makePage(
            ...
            apiPage([], cursor),
        ), { query: 'AEK Browser', limit: 20 })).rejects.toMatchObject({
            code: 'COMMAND_EXEC',
            message: expect.stringContaining('repeated cursor'),
        });

        const pages = Array.from({ length: 50 }, (_, index) => apiPage([], { skip: index + 1 }));
        await expect(command().func(makePage(identity, ...pages), { query: 'AEK Browser', limit: 20 }))
            .rejects.toMatchObject({ code: 'COMMAND_EXEC', message: expect.stringContaining('exceeded 50 pages') });
    });

    it.each([
        [{ query: '', limit: 20 }, ArgumentError],
        [{ query: 'AEK Browser', limit: 0 }, ArgumentError],
        [{ query: 'AEK Browser', limit: 1.5 }, ArgumentError],
        [{ query: 'AEK Browser', limit: '20' }, ArgumentError],
