...
import { CliError, EmptyResultError } from '@cheezmil/aek-browser/errors';
...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (state.kind !== 'notebook') {
            throw new CliError('NOTEBOOKLM_OPEN_FAILED', `NotebookLM notebook "${notebookId}" did not open in the adapter session`, 'Run `aekb notebooklm list -f json` first and pass a valid notebook id.');
        }
        ...
        if (!current) {
            throw new EmptyResultError('AEK Browser notebooklm open', 'NotebookLM notebook metadata was not found after navigation.');
        }
        return [current];
    },
});
