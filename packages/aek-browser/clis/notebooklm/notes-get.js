...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (state.kind !== 'notebook') {
            throw new EmptyResultError('AEK Browser notebooklm notes-get', 'No NotebookLM notebook is open in the adapter session. Run `aekb notebooklm open <notebook>` first.');
        }
        ...
        if (listed) {
            throw new EmptyResultError('AEK Browser notebooklm notes-get', `Note "${query}" is listed in Studio, but AEK Browser currently reads note content only from the visible note editor. Open that note in NotebookLM, then retry.`);
        }
        throw new EmptyResultError('AEK Browser notebooklm notes-get', `Note "${query}" was not found in the current notebook.`);
    },
});
