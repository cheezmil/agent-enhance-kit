import { ArgumentError, AuthRequiredError, CliError, CommandExecutionError } from '@cheezmil/aek-browser/errors';
...
function ensureNotebookUuid(candidate) {
    if (!NOTEBOOK_UUID_RE.test(candidate)) {
        throw new CliError('NOTEBOOKLM_INVALID_NOTEBOOK', `NotebookLM notebook id "${candidate}" is not a valid UUID`, `Pass a notebook id from \`aekb notebooklm list\` or a full notebook URL like ${NOTEBOOKLM_HOME_URL}notebook/<uuid>.`);
    }
    return candidate;
}
export function parseNotebooklmNotebookTarget(value) {
    ...
    if (!normalized) {
        throw new CliError('NOTEBOOKLM_INVALID_NOTEBOOK', 'NotebookLM notebook id is required', 'Pass a notebook id from `aekb notebooklm list` or a full notebook URL.');
    }
    if (/^https?:\/\//i.test(normalized)) {
        ...
        if (!parseTrustedNotebooklmUrl(parsed.href)) {
            throw new CliError('NOTEBOOKLM_INVALID_NOTEBOOK', 'NotebookLM notebook URL must be a NotebookLM URL', 'Pass a notebook id from `aekb notebooklm list` or a full NotebookLM notebook URL.');
        }
        ...
        try {
            ...
            if (error instanceof CliError)
                throw error;
            throw new CliError('NOTEBOOKLM_INVALID_NOTEBOOK', 'NotebookLM notebook URL contains an invalid encoded id', 'Pass a notebook id from `aekb notebooklm list` or a full NotebookLM notebook URL.');
        }
    }
...
export function getNotebooklmAuthuser() {
    const v = process.env.AEKB_NOTEBOOKLM_AUTHUSER;
