...
cli({
    ...
    func: async (page) => {
        ...
        if (state.kind !== 'notebook') {
            throw new EmptyResultError('AEK Browser notebooklm source-list', 'No NotebookLM notebook is open in the adapter session. Run `aekb notebooklm open <notebook>` first.');
        }
        ...
        if (domRows.length > 0)
            return domRows;
        throw new EmptyResultError('AEK Browser notebooklm source-list', 'No NotebookLM sources were found on the current page.');
    },
});
