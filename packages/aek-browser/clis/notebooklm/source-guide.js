...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (state.kind !== 'notebook') {
            throw new EmptyResultError('AEK Browser notebooklm source-guide', 'No NotebookLM notebook is open in the adapter session. Run `aekb notebooklm open <notebook>` first.');
        }
        ...
        if (rows.length === 0) {
            throw new EmptyResultError('AEK Browser notebooklm source-guide', 'No NotebookLM sources were found on the current page.');
        }
        ...
        if (!matched) {
            throw new EmptyResultError('AEK Browser notebooklm source-guide', `Source "${query}" was not found in the current notebook.`);
        }
        const guide = await getNotebooklmSourceGuideViaRpc(page, matched).catch(() => null);
        if (guide)
            return [guide];
        throw new EmptyResultError('AEK Browser notebooklm source-guide', `NotebookLM guide was not available for source "${matched.title}".`);
    },
});
