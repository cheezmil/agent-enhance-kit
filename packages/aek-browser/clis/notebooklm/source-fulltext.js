...
cli({
    ...
    func: async (page, kwargs) => {
        ...
        if (state.kind !== 'notebook') {
            throw new EmptyResultError('AEK Browser notebooklm source-fulltext', 'No NotebookLM notebook is open in the adapter session. Run `aekb notebooklm open <notebook>` first.');
        }
        ...
        if (rows.length === 0) {
            throw new EmptyResultError('AEK Browser notebooklm source-fulltext', 'No NotebookLM sources were found on the current page.');
        }
        ...
        if (!matched) {
            throw new EmptyResultError('AEK Browser notebooklm source-fulltext', `Source "${query}" was not found in the current notebook.`);
        }
        const fulltext = await getNotebooklmSourceFulltextViaRpc(page, matched.id).catch(() => null);
        if (fulltext)
            return [fulltext];
        throw new EmptyResultError('AEK Browser notebooklm source-fulltext', `NotebookLM fulltext was not available for source "${matched.title}".`);
    },
});
