    ...
    func: async (args) => {
        ...
        try {
            resp = await fetch(url, {
                headers: {
                    'User-Agent': 'AEK Browser/1.0 (+https://github.com/cheezmil/AEK Browser)',
        ...
        if (!page || page.missing) {
            throw new EmptyResultError('wikipedia page', `No article "${title}" on ${lang}.wikipedia.org. Try \`aekb wikipedia search\` first.`);
        }
