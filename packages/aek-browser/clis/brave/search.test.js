...
describe('brave search', () => {
  ...
  it('rejects empty query, invalid limit, and invalid offset before navigation', async () => {
    ...
    await expect(command.func(page, { keyword: 'aekb', limit: 19 })).rejects.toMatchObject({ code: 'ARGUMENT' });
    await expect(command.func(page, { keyword: 'aekb', limit: 5, offset: -1 })).rejects.toMatchObject({ code: 'ARGUMENT' });
  ...
  it('unwraps browser envelopes and returns ranked HTTPS rows', async () => {
    const page = createPageMock({
      session: 'site:brave',
      data: [['AEK Browser', 'https://github.com/cheezmil/AEK Browser', 'CLI browser tooling']],
    });

    await expect(command.func(page, { keyword: 'aekb', limit: 1, offset: 1 })).resolves.toEqual([{
      rank: 19,
      title: 'AEK Browser',
      url: 'https://github.com/cheezmil/AEK%20Browser',
