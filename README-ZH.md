...
AI Agent 增强提效工具 — **跨平台支持**：Windows · Linux · macOS
...
| 包 | 说明 |
|----|------|
| [`aek`](packages/aek) | 全家桶 CLI——统一的 `aek` 入口，聚合所有模块（网页搜索、MCP、Skill 同步、任务管理） |
...
| [`aek-prompt-manager`](packages/aek-prompt-manager) | 向 AI coding agent 的全局提示词文件注入平台化提示词片段（Claude Code、Codex、OpenCode、Hermes、Cursor、Cline 等），双源（patch/map）支持 17 个工具，通过 `aek pm` 命令 |
...
2. Clone 你 fork 的仓库：（包在 npm 已发布，但是 AI 读取源码后，因为源码内有预置 skill 和 prompt，所以能给你更好的自定义使用教程，）
   ```bash
...
基于实际使用测试（截至 2026-08-21），不同场景下排行不同：

- **仅中文内容**：**Tavily** > **Exa** ≈ **Serper**
- **仅国际内容**：**Exa** > **Tavily** ≈ **Serper**
- **中英文内容都搜索**：**Tavily** > **Exa** ≈ **Serper**

所以 **Tavily**、**Exa**、**Serper** 三者可随时一起使用。不同 agent 对不同搜索提供商的喜好不同，不同的搜索关键词在不同提供商上得到的效果也不一样——总之这三个都属于 T0 级别。

*此排行仅覆盖值得排名的提供商。未列入的供应商未参与排名，原因是其对 agent 使用效果不理想，浪费 token、浪费时间，没有必要排名。*

*该完整排行反映个人实际测试体验，具有时效性，可能随时间变化。*
