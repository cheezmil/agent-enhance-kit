...
| Package | Description |
|---------|-------------|
| [`aek`](packages/aek) | All-in-one CLI — unified `aek` entry that assembles every module (web search, MCP, skill sync, task manager) |
...
| [`aek-prompt-manager`](packages/aek-prompt-manager) | Inject platform-specific prompt fragments into AI coding tools' global prompt files (Claude Code, Codex, OpenCode, Hermes, Cursor, Cline, ...) via `aek pm` — dual-source (patch/map), platform-aware, 17 tools supported |
