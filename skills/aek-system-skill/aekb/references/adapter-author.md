# references/adapter-author.md

你是要给一个站点写 adapter 的 agent。这份 skill 目标：简单站点争取 **30 分钟内从零到通过 `aekb browser verify`**；复杂、私有协议或写操作站点以证据完整和安全为先，不为了时限猜接口。

全程用现有工具：`aekb browser *` / `aekb doctor` / `aekb browser init` / `aekb browser verify`。没有新命令。
...
| `PAGE_FETCH` | internal-unstable | 只能在页面上下文 `fetch` 才能复用 same-origin/session/runtime | `aekb browser eval fetch(...)` 非空；必须解释为什么避不开内部接口 |
...
实测：`PAGE_FETCH` / `INTERCEPT` 的 fix 频率约为 `PUBLIC_API` 的 7-8 倍，`UI_SELECTOR` 跟 `COOKIE_API` 同档。详细 ladder 推导、`api_candidates` 证据怎么填、booking #1680 等反例见 [`references/adapter/strategy-selection.md`](references/adapter/strategy-selection.md)。
...
START
  ...
  ▼
┌──────────────────────────┐
│ aekb doctor 通？      │── no ──→ 修桥接（doctor 输出里的提示）
└──────────────────────────┘
  │ yes
  ▼
┌────────────────────────────────────────────────────┐
│ 读站点记忆：                                        │
│   1. ~/.aek/browser/system/sites/<site>/endpoints.json         │
│   2. ~/.aek/browser/system/sites/<site>/notes.md               │
│   3. references/adapter/site-memory/<site>.md               │
└────────────────────────────────────────────────────┘
...
│ → references/adapter/deep-recon.md                 │  intent matrix → 因果 diff → 候选账本 → contract gate
└────────────────────────────────────────────┘
...
└──────────────────────────┘
  ...
  ▼
┌──────────────────────────┐
│ aekb browser init      │  生成 ~/.aek/browser/system/clis/<site>/<name>.js 骨架
│ 复制最像的邻居 adapter    │
│ 改 name / URL / 映射三处  │
└──────────────────────────┘
  │
  ▼
┌──────────────────────────┐
│ aekb browser verify    │── 失败 ──→ autofix skill，用 --trace retain-on-failure 回对应步骤
└──────────────────────────┘
  │ 成功
  ▼
┌──────────────────────────┐
...
└──────────────────────────┘
  ...
  ▼
┌──────────────────────────┐
│ 回写 ~/.aek/browser/system/sites/   │  endpoints / field-map / notes / fixtures
└──────────────────────────┘
  │
  ▼
DONE
...
```
[ ] 1. aekb doctor 返回 "Everything looks good"
[ ] 2. 读站点记忆：
       [ ] ~/.aek/browser/system/sites/<site>/endpoints.json 存在？里面有想要的 endpoint？
       [ ] references/adapter/site-memory/<site>.md 存在？看"已知 endpoint"节
       [ ] 命中后：**跳到第 5（endpoint 验证） + 第 7（字段核对）**，不能直接跳第 9 写 adapter
       [ ] memory 写入超过 30 天（看 `verified_at`）→ 当作过期，按冷启动走 Step 3 → 4
[ ] 3. 侦察（site-recon.md）：
       [ ] **首选**：`aekb browser analyze <url>` 一步拿 pattern + 反爬 + 最近 adapter + next step
...
[ ] 9. 写 adapter（adapter-template.md）：
       [ ] aekb browser init <site>/<name>
       [ ] 找同站点或同类型最像的 adapter，cp 过来
       [ ] 改 name / URL / 字段映射
[ ] 10. aekb browser verify <site>/<name>
        [ ] 首轮通过后立刻 `--write-fixture` 生成 `~/.aek/browser/system/sites/<site>/verify/<cmd>.json` 种子
        [ ] 手改种子：加 `patterns`（URL / 日期 / ID 格式）+ `notEmpty`（核心字段）+ 收紧 `rowCount`
        [ ] 再跑一次 `aekb browser verify <site>/<name>`，确认 ✓ matches fixture
[ ] 11. 字段值 vs 网页肉眼比对（别只看 "Adapter works!"）
[ ] 12. 回写站点记忆（**verify 通过 + 肉眼比对对得上之后**，schema 见 `references/adapter/site-memory.md`）：
        ...
        [ ] `verify/<cmd>.json`：**必填。** `aekb browser verify` 的期望值（args / rowCount / columns / types / patterns / notEmpty），Step 10 已经让你生成了，这里只是 checklist
...
| 文件 | 什么时候翻 |
|------|----------|
| `references/adapter/coverage-matrix.md` | 动手前做"是否在范围内"自测 |
| `references/adapter/site-recon.md` | Step 3 定站点类型 |
| `references/adapter/api-discovery.md` | Step 4 找 endpoint |
| `references/adapter/deep-recon.md` | 复杂无文档站：动作归因、jsluice 候选扩展、合同证明、读写安全与交付净账 |
| `references/adapter/strategy-selection.md` | Step 6 填 strategy note 之前：契约模型 + 实测 fix 频率 + `api_candidates` 证据用法 + 反例 |
| `references/adapter/field-conventions.md` | Step 7 查已知字段代号 |
| `references/adapter/field-decode-playbook.md` | Step 7 字段不在词典时 |
| `references/adapter/output-design.md` | Step 8 命名 / 类型 / 顺序 |
| `references/adapter/adapter-template.md` | Step 9 文件结构 + 活例子 `convertible.js` |
| `references/adapter/site-memory.md` | 总览：in-repo 种子 + 本地 `~/.aek/browser/system/sites/` 的两层结构 |
| `references/adapter/site-memory/<site>.md` | Step 2 读站点公共知识（eastmoney / xueqiu / bilibili / tonghuashun 已铺） |
| `references/adapter/success-rate-pitfalls.md` | Step 7 / 11 踩坑前翻：11 种"verify 能过但数据是错的"静默失败（含 aria-label locale-dependence） |
| `references/adapter/jsdom-fixture-pattern.md` | 当 adapter 走 `page.evaluate` 内 DOM 抽取、且 mocked-evaluate 单测漏 silent bug 时——把 HTML 冻进 `clis/<site>/__fixtures__/` 用 JSDOM 跑（含 fixture 创建 mandatory `awk 'NF>0'` 收紧 + reverse-validate 纪律） |
| `references/adapter/typed-errors.md` | 写 `func` 主体之前必读：5 类 typed error 落点表（ArgumentError / EmptyResultError / CommandExecutionError / AuthRequiredError / TimeoutError）+ 三大 silent anti-pattern（silent-clamp / sentinel-row / generic CliError）的反例修法 |

---

## 关键约定

- adapter 只引 `@cheezmil/aek-browser/registry` + `@cheezmil/aek-browser/errors`，不用第三方
...
- 已知失败按 [`references/adapter/typed-errors.md`](references/adapter/typed-errors.md) 5-classification 抛对应 typed error；**不要** silent `return []`，**不要** silent `return [{sentinel}]`，**不要** `Math.max/min` silent clamp 外部参数
- 写私人 adapter 用 `~/.aek/browser/system/clis/<site>/<name>.js`（免 build）；要提 PR 才 copy 到 `clis/<site>/<name>.js`
...
- **调试过程中的原始 dump / 抓包 / HTML 样本只能短暂落在系统 `/tmp/` 或受控 cache，任务结束删除。只有通过 `site-memory.md` 数据分级、准备长期保留的公开/合成/已脱敏样本才进入 `~/.aek/browser/system/sites/<site>/fixtures/`。严禁在 repo 根目录、`clis/<site>/` 或当前工作目录留 `.dbg-*.html / raw-*.json / sample.*`。**
- **JSDOM unit-test fixture（`clis/<site>/__fixtures__/<command>.html`）是上面那条的例外**——它是有意 commit 进 repo 的 review artifact，不是临时 dump。但因此 quality bar 要更高：必须按 `references/adapter/jsdom-fixture-pattern.md` 的 5 步做完（含 mandatory `awk 'NF>0'` 空白行收紧），并 reverse-validate 一道证明 regression guard 真能挂。

---

## 卡住了

- 诊断类：`aekb doctor` → 看 `notes.md` → 搜 autofix skill
