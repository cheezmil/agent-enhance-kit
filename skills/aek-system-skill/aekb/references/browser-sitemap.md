# references/browser-sitemap.md

Use this skill when `aekb browser open` or `aekb browser analyze` reports `sitemap.available: true`, or when the user asks you to use a site's sitemap.
...
1. Run or reuse `aekb browser <session> state` to know the current page.
...
3. Prefer the workflow's **Best path**. If it names an adapter such as `aekb twitter post`, use that before raw browser actions.
...
```text
~/.aek/browser/system/sites/<site>/sitemap/    # local overlay
...
1. Find the local workflow file under `~/.aek/browser/system/sites/<site>/sitemap/workflows/` whose `Best path` references the adapter command.
