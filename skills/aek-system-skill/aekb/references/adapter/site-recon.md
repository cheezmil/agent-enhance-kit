...
```json
{
  ...
  "recommended_next_step": "Pick the most specific JSON endpoint from `aekb browser network` and try a bare Node fetch with cookies; escalate to browser-context fetch only if blocked."
}
```
...
**数据是 SPA / 异步加载时，`wait time 2` 可能不够**。改用 `aekb browser wait xhr '/api/path-fragment'` 直接等具体接口到场，比盲 `wait time 5` 更稳。
...
- 先看 `aekb browser analyze` 输出的 `api_candidates[]`：`verdict=likely_data` 的条目才是真候选；`verdict=noise`（analytics / beacon / personalization）不能算 API 信号
