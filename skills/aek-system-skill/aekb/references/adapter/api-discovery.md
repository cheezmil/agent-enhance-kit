...
`aekb browser analyze <url>` 的 `anti_bot` 字段给答案；手查看 cookies 也行：
...
```bash
aekb browser eval "fetch('https://<target-subdomain>/api/...', {credentials:'include'}).then(r=>r.status).catch(e=>'cors:'+e.message)"
```
...
2. 用 `aekb browser open https://<target-subdomain>/`，让页面在目标 subdomain 本身打开，再 fetch 相对路径
...
如果是冷启动，先看 `aekb browser analyze <url>` 里的 `api_candidates`：
...
capture 会持久化到 `~/.aek/browser/system/cache/browser-network/<session>.json`（默认 TTL 24h），所以 `--detail` 即使跨多条其他命令也还在。
...
```bash
# 滚到底（虚拟列表）
aekb browser eval "window.scrollTo(0, document.body.scrollHeight)"
aekb browser wait time 2
aekb browser network

# 点某个按钮
aekb browser click <N>
aekb browser wait time 2
aekb browser network
```
...
```bash
aekb browser eval "Object.keys(window).filter(k=>k.startsWith('__'))"
```
...
```bash
aekb browser eval "JSON.stringify(window.__NEXT_DATA__).slice(0, 3000)"
```
...
```bash
aekb browser eval "[...document.querySelectorAll('script[src]')].map(s=>s.src).filter(s=>!/\\.(css|png|jpg|svg|woff|mp4)$/.test(s)&&!/googletagmanager|crazyegg|sentry|doubleclick|amazon-adsystem|cloudflare/.test(s))"
```
...
```bash
aekb browser eval "(async()=>{const s=[...document.querySelectorAll('script[src]')].map(e=>e.src).find(s=>/main|app|index|bundle|chunk/.test(s));if(!s)return'no bundle';const t=await fetch(s).then(r=>r.text());const patterns=['baseURL','baseUrl','BASE_URL','apiHost','apiBase','API_HOST','API_BASE'];const hits=[];for(const p of patterns){let i=-1;while((i=t.indexOf(p,i+1))>-1&&hits.length<5)hits.push(t.slice(Math.max(0,i-5),i+80));}return hits})()"
```
...
```bash
aekb browser eval "fetch('https://push2.eastmoney.com/api/qt/clist/get?fs=m:1+t:2&pn=1&pz=5&fltt=2&fid=f3&po=1&fields=f2,f3,f12,f14').then(r=>r.json())"
```
...
```bash
# 当前页加 .json 试
aekb browser eval "fetch(location.pathname.replace(/\\/$/,'')+'.json').then(r=>r.ok?r.json():'no')"
```
...
```bash
aekb browser eval "document.cookie.split('; ').map(x=>x.slice(0,x.indexOf('='))).filter(Boolean)"
```
...
```bash
aekb browser eval "Object.keys(localStorage).filter(k=>/token|auth|jwt|bearer|csrf/i.test(k))"
```
...
```bash
aekb browser eval "(async()=>{const s=[...document.querySelectorAll('script[src]')].map(e=>e.src).find(s=>/main|app|bundle/.test(s));const t=await fetch(s).then(r=>r.text());const m=[...t.matchAll(/Bearer\\s+[\\w-]{20,}/g)];return {count:m.length,positions:m.slice(0,3).map(x=>x.index)}})()"
```
...
```bash
# Pinia
aekb browser eval "typeof __pinia !== 'undefined' ? Object.keys(__pinia.state.value) : 'no pinia'"

# 只调用已证明是 read-only 的 store action（每个站点具体 action 名要查）
aekb browser eval "window.__pinia.state.value.someStore.someMethod({...})"
```
